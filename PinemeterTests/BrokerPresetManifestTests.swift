...
final class BrokerPresetManifestTests: XCTestCase {
    ...
    private func makeManifestData(
        ...
        object.merge(metadata) { _, metadataValue in metadataValue }
    ...
    func test_decode_manifestWithoutRevisionMetadataRemainsCompatible() throws {
        let manifest = try BrokerPresetManifest.decode(from: makeManifestData(presets: []))

        XCTAssertNil(manifest.manifestRevision)
        XCTAssertNil(manifest.publishedAt)
        XCTAssertNil(manifest.summary)
    }

    func test_decode_manifestRevisionMetadataIsValidatedAndSanitized() throws {
        let data = try makeManifestData(
            presets: [],
            metadata: [
                "manifest_revision": 14,
                "published_at": "2026-09-20T12:34:56Z",
                "summary": "Changed\u{0007} rules\n" + String(repeating: "x", count: 300),
            ]
        )

        let manifest = try BrokerPresetManifest.decode(from: data)

        XCTAssertEqual(manifest.manifestRevision, 14)
        XCTAssertNotNil(manifest.publishedAt)
        XCTAssertFalse(try XCTUnwrap(manifest.summary).contains("\u{0007}"))
        XCTAssertFalse(try XCTUnwrap(manifest.summary).contains("\n"))
        XCTAssertEqual(
            manifest.summary?.unicodeScalars.count,
            BrokerPresetManifest.maxSummaryScalars
        )
    }

    func test_decode_invalidManifestRevisionMetadataIsIgnoredIndependently() throws {
        let data = try makeManifestData(
            presets: [],
            metadata: [
                "manifest_revision": -1,
    ...
    func test_decode_modelSpecWithAccountIDOrUnknownProviderSkipsOnlyThatEntry() throws {
        let badAccount = makeEntry(name: "Bad Account", rules: [
            "models": ["gpt-test": [
                "quota": ["provider": "claude_account", "account_id": "local-secret"]
            ]]
        ])
        let badProvider = makeEntry(name: "Bad Provider", rules: [
            "models": ["gpt-test": ["quota": ["provider": "future-provider"]]]
        ])
        let badEffort = makeEntry(name: "Bad Effort", rules: [
            "models": ["gpt-test": [
                "effort": ["levels": ["Ultra High"], "default_label": "Default"]
            ]]
        ])
        let valid = makeEntry(name: "Valid", rules: [
            "models": ["gpt-test": [
                "quota": ["provider": "chatgpt", "label_contains": "codex weekly"]
            ]]
        ])

        let manifest = try BrokerPresetManifest.decode(
            from: makeManifestData(presets: [badAccount, badProvider, badEffort, valid])
        )

        XCTAssertEqual(manifest.presets.map(\.name), ["Valid"])
        XCTAssertEqual(
            manifest.presets.first?.rules.models["gpt-test"]?.quota,
    ...
    func test_routingUpdateRecoverySurvivesRestartAndProtectsNewEdits() throws {
        var settings = makeSettings()
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        let baseline = settings.activeProfileRules
        settings.policy.roles["planning"] = [BrokerCandidate(route: .auto, model: "before-update-edit")]
        let previous = settings.policy.ruleSet
        var update = BrokerAgentProfile.balanced
        update.rules.roles["planning"] = [BrokerCandidate(route: .auto, model: "updated-model")]
        settings.updateRemotePresets([update])
        settings.applyProfile(id: update.id)
        settings = try JSONDecoder().decode(BrokerSettings.self, from: JSONEncoder().encode(settings))
        XCTAssertTrue(settings.canUndoRoutingUpdate)
        settings.applyProfile(id: BrokerAgentProfile.maxQualityID)
        XCTAssertFalse(settings.undoRoutingUpdate())
        settings.applyProfile(id: update.id)
        XCTAssertTrue(settings.canUndoRoutingUpdate)
        settings.policy.roles["planning"] = [BrokerCandidate(route: .auto, model: "new-edit")]
        XCTAssertFalse(settings.undoRoutingUpdate())
        XCTAssertEqual(settings.policy.roles["planning"]?.first?.model, "new-edit")
        ...
        let legacy = try JSONDecoder().decode(BrokerSettings.self, from: JSONEncoder().encode(settings))
        XCTAssertNil(legacy.routingRecovery)
        XCTAssertFalse(legacy.canUndoRoutingUpdate)
    }

    func test_popoverRecoveryDismissalPersistsUntilNextUpdateOrUndo() throws {
        var settings = makeSettings()
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        var update = BrokerAgentProfile.balanced
        update.rules.roles["planning"] = [BrokerCandidate(route: .auto, model: "updated-model")]
        ...
        settings.applyProfile(id: update.id)
        XCTAssertTrue(settings.canUndoRoutingUpdate)
        XCTAssertFalse(settings.isRoutingRecoveryDismissedInPopover)

        settings.isRoutingRecoveryDismissedInPopover = true
        let saved = try JSONEncoder().encode(settings)
        settings = try JSONDecoder().decode(BrokerSettings.self, from: saved)
        XCTAssertTrue(settings.isRoutingRecoveryDismissedInPopover, "dismissal survives a restart")
        XCTAssertTrue(settings.canUndoRoutingUpdate, "settings keeps offering Undo Update")

        var legacyJSON = try XCTUnwrap(
            JSONSerialization.jsonObject(with: saved) as? [String: Any]
        )
        legacyJSON.removeValue(forKey: "routing_recovery_dismissed_in_popover")
        let legacy = try JSONDecoder().decode(
            BrokerSettings.self,
            from: JSONSerialization.data(withJSONObject: legacyJSON)
        )
        XCTAssertFalse(
            legacy.isRoutingRecoveryDismissedInPopover,
            "a save written before dismissal existed shows the banner"
        )
        XCTAssertTrue(legacy.canUndoRoutingUpdate)

        var second = update
        second.rules.roles["planning"] = [BrokerCandidate(route: .auto, model: "second-update-model")]
        settings.updateRemotePresets([second])
        settings.applyProfile(id: second.id)
        XCTAssertFalse(settings.isRoutingRecoveryDismissedInPopover, "a new update re-announces itself")
    ...
    func test_routingUpdateSummaryUsesPublishedChangesAndLoadedBaseline() {
        var settings = makeSettings()
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        let original = settings.policy.roles["planning"]?.first?.model ?? ""
        var rules = BrokerAgentProfile.balanced.rules
        rules.roles["planning"] = [BrokerCandidate(route: .auto, model: "future-model", effort: .low)]
        let update = BrokerAgentProfile(id: BrokerAgentProfile.balancedID, name: "Balanced", detail: "", symbolName: "leaf", isBuiltIn: false, rules: rules)
        settings.updateRemotePresets([update])
        settings.policy.roles["planning"] = [BrokerCandidate(route: .auto, model: "unsaved-model")]
        let summary = settings.routingUpdateChanges.joined(separator: "\n")
        XCTAssertTrue(summary.contains("New: auto/future-model (low)"))
        XCTAssertTrue(summary.contains(original))
        XCTAssertFalse(summary.contains("unsaved-model"))
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        XCTAssertTrue(settings.routingUpdateChanges.isEmpty)
    }
    ...
    func test_applyProfile_onRemotePreset_preservesMachineState() {
        ...
        remoteRules.models = ["gpt-test": BrokerModelSpec(
        ...
        settings.policy.t3.ignoredInstances = ["removed"]
        settings.policy.usageLanes = [
            "codex/gpt-local": .claudeAccount(
                accountId: "account-1", labelContains: nil, isPrimary: nil
            )
        ]
        let localLanes = settings.policy.usageLanes
        ...
        XCTAssertEqual(settings.policy.t3.ignoredInstances, ["removed"])
        XCTAssertEqual(settings.policy.usageLanes, localLanes)
        XCTAssertNotNil(settings.policy.models["gpt-test"])
    }
    ...
    func test_bundledManifest_loadsAndContainsBalancedNoOpus() throws {
        ...
        XCTAssertNil(manifest.presets.first { $0.name == "Astra" })
    }

    /// The broker contract tells an agent to pass one exact candidate id for an
    /// explicit Fable review. `decideOverride` matches that id literally against
    /// the configured role chains, so a preset that lists Fable under some other
    /// id makes every explicit Fable review fail "not configured" instead of
    /// running Fable. A preset with no Fable at all is allowed to fail closed.
    func test_bundledManifest_everyFablePresetConfiguresTheContractFableCandidate() throws {
        let manifest = try loadBundledManifest()
        let contractCandidate = "auto/claude-fable-5-1"
        XCTAssertTrue(
            BrokerSetupPrompt.text(port: 43117, origin: .pasteboard).contains("`\(contractCandidate)`"),
            "the setup prompt must name the same candidate the presets configure"
        )

        for preset in manifest.presets {
            let candidates = preset.rules.roles.values.flatMap { $0 }
            guard candidates.contains(where: { $0.model.contains("fable") }) else { continue }
            XCTAssertTrue(
                candidates.contains { $0.id == contractCandidate },
                "\(preset.name) lists Fable but not as \(contractCandidate), "
                    + "so an explicit Fable review override cannot resolve"
            )
        }
    }

    func test_bundledManifest_everyPresetDeclaresAModelSpecForEveryGPTCandidate() throws {
        let manifest = try loadBundledManifest()
        ...
        for preset in manifest.presets {
            ...
            for model in models {
                ...
                guard case .chatGPT = preset.rules.models[model]?.quota else {
                    XCTFail("\(preset.name)/\(model) has no portable ChatGPT quota")
                    continue
                }
            }
            let encoded = try JSONEncoder().encode(preset.rules)
            XCTAssertFalse(String(decoding: encoded, as: UTF8.self).contains("account_id"))
            XCTAssertFalse(String(decoding: encoded, as: UTF8.self).contains("bound_account_id"))
        }
    }
...
final class BrokerPresetManifestAppModelTests: XCTestCase {
    private func makeAppModel(
        presetManifestService: (any PresetManifestServiceProtocol)?,
    ...
    func test_refreshPresetManifest_notModified_updatesLastCheckedOnlyAndClearsError() async {
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("BrokerPresetManifestAppModelTests-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: directory) }
        let historyStore = BrokerManifestHistoryStore(storeDirectory: directory)
        let appModel = makeAppModel(presetManifestService: fake, historyStore: historyStore)
        appModel.settings.broker.remotePresets = [existing]
        appModel.settings.broker.presetManifest.lastError = "stale error"
        let source = appModel.settings.broker.presetManifest.sources[0]
        let seeded = try? await historyStore.seedIfNeeded(
            sourceID: source.id,
        ...
        XCTAssertNil(appModel.settings.broker.presetManifest.lastError)
        XCTAssertNotNil(appModel.settings.broker.presetManifest.lastCheckedAt)
        let history = await historyStore.load(sourceID: source.id, urlString: source.urlString)
        XCTAssertEqual(history.revisions.map(\.id), seeded?.map(\.id), "a 304 must not append history")
    }

...
final class GeminiUsageServiceTests: XCTestCase {
    ...
    func test_httpClientKeepsAPIKeyOutOfURLAndPreventsPersistentCaching() async throws {
        let sourceURL = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("Pinemeter/Services/GeminiUsageService.swift")
        let source = try String(contentsOf: sourceURL, encoding: .utf8)
        XCTAssertTrue(source.contains("init(configuration: URLSessionConfiguration = .ephemeral)"))

        let configuration = URLSessionConfiguration.default
        configuration.urlCache = .shared
        configuration.protocolClasses = [GeminiRequestURLProtocol.self]
        let apiKey = try GeminiAPIKey("not-a-real-key")
        GeminiRequestURLProtocol.handler = { request in
            XCTAssertNil(request.url?.query)
            XCTAssertTrue(request.value(forHTTPHeaderField: "x-goog-api-key") == apiKey.value)
            XCTAssertFalse(request.url?.absoluteString.contains(apiKey.value) ?? true)
        ...
        defer { GeminiRequestURLProtocol.handler = nil }

        let client = GeminiHTTPClient(configuration: configuration)
        XCTAssertNil(configuration.urlCache)
        XCTAssertEqual(configuration.requestCachePolicy, .reloadIgnoringLocalCacheData)

        let _: GeminiUsageAPIResponse = try await client.request(
    ...
    override func tearDown() async throws {
        // Keychain cleanup only. The status write this triggers lands in the
        // isolated suite `TestSafeDefaults` hands out under XCTest,
        // not in the real defaults domain.
...
private final class GeminiRequestURLProtocol: URLProtocol {

...
final class InstructionAuditServiceTests: XCTestCase {
    ...
    private var completeContract: String {
        ...
        A fresh explicit operator instruction may pass one `override_candidate`. Require `source: human-override`. Never infer, persist, or reuse it. Never bypass Pinemeter for an override.
        """
    }
    ...
    func test_lineWrappedDirectivesStillCountAsPresent() {
        ...
        Accept only `native` with `agent`, `t3` with `t3-dispatch`, and `codex`
        with `codex-exec`.
        ...
        Require `source: human-override`. Never infer or persist it. Never bypass Pinemeter for an override.
        """

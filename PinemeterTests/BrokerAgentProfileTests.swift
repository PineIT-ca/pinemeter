...
final class BrokerAgentProfileTests: XCTestCase {
    ...
    func test_builtInProfiles_offerStandardWorkToCodexCallers() {
        for profile in BrokerAgentProfile.builtIns {
            let routes = Set(profile.rules.roles["standard", default: []].map(\.route))
            XCTAssertFalse(routes.isDisjoint(with: [.codex, .t3]), profile.name)
        }
    }

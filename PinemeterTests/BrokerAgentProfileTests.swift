...
final class BrokerAgentProfileTests: XCTestCase {
    func test_routingSemanticChangesKeepCandidateOrderEffortAndThresholdFieldsTyped() {
        var old = BrokerRuleSet.default
        old.roles["planning"] = [
            BrokerCandidate(route: .native, model: "model-a", effort: .high),
            BrokerCandidate(route: .codex, model: "model-b", effort: .low),
        ]
        var new = old
        new.roles["planning"] = [
            BrokerCandidate(route: .codex, model: "model-b", effort: .medium),
            BrokerCandidate(route: .native, model: "model-a", effort: .high),
        ]
        new.thresholds.weeklyPct = 80
        new.allowForcedDegraded["planning"] = false
        new.agentModelAliases["model-a"] = "agent-model-a"

        let changes = makeSettings().routingChanges(from: old, to: new)

        XCTAssertEqual(changes.first, .role(BrokerRoutingRoleChange(
            role: "planning",
            previous: old.roles["planning"]!,
            proposed: new.roles["planning"]!,
            proposedUsesDefaultRules: false
        )))
        XCTAssertTrue(changes.contains(.thresholds([
            BrokerRoutingThresholdChange(
                field: .weeklyPct,
                previousValue: "85%",
                proposedValue: "80%"
            ),
        ])))
        XCTAssertTrue(changes.contains(.callerPolicy))
        XCTAssertTrue(changes.contains(.connections))
    }

    func test_revisionSemanticChangesNameDefaultRuleProvenance() {
        var old = BrokerRuleSet.default
        old.roles["planning"] = [BrokerCandidate(route: .native, model: "old-model")]
        let proposed = BrokerRuleSet.default

        let changes = makeSettings().routingChanges(
            from: old,
            to: proposed,
            defaultedRoles: ["planning"]
        )

        guard case .role(let roleChange) = changes.first else {
            return XCTFail("Expected a typed role change")
        }
        XCTAssertEqual(roleChange.role, "planning")
        XCTAssertTrue(roleChange.proposedUsesDefaultRules)
        XCTAssertEqual(roleChange.proposed, BrokerPolicy.bundledDefault.roles["planning"])
    }

    func test_routingUpdateSemanticChangesComparePublishedRulesToPinnedBaseline() {
        var settings = makeSettings()
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        let baseline = settings.activeProfileRules!
        var published = BrokerAgentProfile.balanced
        published.rules.roles["planning"] = [
            BrokerCandidate(route: .auto, model: "published-model", effort: .low),
        ]
        settings.updateRemotePresets([published])
        settings.policy.roles["planning"] = [
            BrokerCandidate(route: .auto, model: "unsaved-model"),
        ]

        guard case .role(let change) = settings.routingUpdateSemanticChanges.first else {
            return XCTFail("Expected a typed role change")
        }
        XCTAssertEqual(change.previous, baseline.roles["planning"])
        XCTAssertEqual(change.proposed.first?.model, "published-model")
        XCTAssertFalse(change.previous.contains { $0.model == "unsaved-model" })
        XCTAssertFalse(change.proposed.contains { $0.model == "unsaved-model" })
    }
    ...
    func test_restoreRulesPreservesExplicitOrderAndUndoRestoresUnsavedRulesExactly() throws {
        var settings = makeSettings()
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        let baseline = settings.activeProfileRules
        settings.policy.roles["planning"] = [BrokerCandidate(route: .t3, instance: "claudeAgent", model: "claude-fable-5")]
        let previous = settings.policy.ruleSet
        let local = settings.policy.t3Instances
        let lanes = settings.policy.usageLanes
        var revision = BrokerRuleSet.default
        revision.roles["planning"] = [
            BrokerCandidate(route: .t3, instance: "claudeAgent", model: "claude-fable-5"),
            BrokerCandidate(route: .native, model: "claude-fable-5"),
        ]
        revision.thresholds.weeklyPct = 1
        XCTAssertTrue(settings.restoreProfileRules(id: BrokerAgentProfile.balancedID, rules: revision))
        XCTAssertEqual(settings.policy.roles["planning"], revision.roles["planning"])
        XCTAssertEqual(settings.routingRecovery?.previousBaseline, baseline)
        XCTAssertEqual(settings.policy.t3Instances, local)
        XCTAssertEqual(settings.policy.usageLanes, lanes)
        XCTAssertTrue(settings.undoRoutingUpdate())
        XCTAssertEqual(settings.policy.ruleSet, previous)
        XCTAssertEqual(settings.activeProfileRules, baseline)
        XCTAssertEqual(settings.policy.t3Instances, local)
        XCTAssertEqual(settings.policy.usageLanes, lanes)
    }

    func test_restoreRulesRejectsInvalidRolesAndUnknownInstancesWithoutMutation() {
        var settings = makeSettings()
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        let original = settings
        let invalidChains: [[BrokerCandidate]] = [
            [],
            [BrokerCandidate(route: .auto, model: "unconfigured-model")],
            [BrokerCandidate(route: .t3, instance: "missing", model: "claude-fable-5"),
             BrokerCandidate(route: .native, model: "claude-fable-5")],
            [BrokerCandidate(route: .t3, instance: "claudeAgent", model: "unsupported-model")],
        ]
        for chain in invalidChains {
            var rules = BrokerRuleSet.default
            rules.roles["planning"] = chain
            XCTAssertFalse(settings.restoreProfileRules(id: BrokerAgentProfile.balancedID, rules: rules))
            XCTAssertEqual(settings, original)
        }
        XCTAssertFalse(settings.restoreProfileRules(id: BrokerAgentProfile.maxQualityID, rules: .default))
        XCTAssertEqual(settings, original)
        XCTAssertFalse(settings.restoreProfileRules(id: BrokerAgentProfile.balancedID, rules: settings.policy.ruleSet))
        XCTAssertEqual(settings, original)
    }

    func test_ordinaryUpdateStillUsesExistingUndoSemantics() {
        var settings = makeSettings()
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        let baseline = settings.activeProfileRules
        settings.policy.thresholds.weeklyPct = 63
        let edited = settings.policy.ruleSet
        var published = BrokerAgentProfile.balanced
        published.rules.thresholds.weeklyPct = 54
        settings.updateRemotePresets([published])
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        XCTAssertNil(settings.routingRecovery?.origin)
        XCTAssertTrue(settings.undoRoutingUpdate())
        XCTAssertEqual(settings.policy.ruleSet, edited)
        XCTAssertEqual(settings.activeProfileRules, baseline)
    }

    func test_restoreRulesFillsAbsentRolesButRetainsExistingUndoConsent() {
        var settings = makeSettings()
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        let before = settings.policy.ruleSet
        var rules = BrokerRuleSet.default
        rules.roles.removeValue(forKey: "planning")
        rules.thresholds.weeklyPct = 1
        XCTAssertTrue(settings.restoreProfileRules(id: BrokerAgentProfile.balancedID, rules: rules))
        XCTAssertEqual(settings.policy.roles["planning"], BrokerPolicy.bundledDefault.roles["planning"])
        settings.policy.thresholds.weeklyPct = 2
        XCTAssertFalse(settings.undoRoutingUpdate())
        XCTAssertTrue(settings.undoRoutingUpdate(discardingEdits: true))
        XCTAssertEqual(settings.policy.ruleSet, before)
    }

    func test_restoreAndUndoPreserveAllExcludedSettingsBytesAndReplaceOnlyRecovery() throws {
        var settings = makeSettings()
        settings.port = 43219
        settings.networkAccess = .network
        settings.apiKeyMode = .none
        settings.recheckReminderEnabled = false
        settings.routingUpdateNotificationsEnabled = false
        settings.lastRoutingUpdateNotifiedFingerprint = "notified-fingerprint"
        settings.seenRemotePresetIDs = [BrokerAgentProfile.maxQualityID]
        settings.appliedRoutingMigrations.insert("fixture-migration")
        settings.profiles = [BrokerAgentProfile(name: "Saved local profile", rules: .default)]
        settings.remotePresets = [BrokerAgentProfile.maxQuality]
        settings.presetManifest.sources = [BrokerPresetManifestSource(
            urlString: "https://example.com/local.json",
            lastCheckedAt: Date(timeIntervalSince1970: 1_700_000_000)
        )]
        settings.presetManifest.sources[0].cachedPresets = [BrokerAgentProfile.maxQuality]
        settings.presetManifest.sources[0].etag = "local-etag"
        settings.presetManifest.sources[0].lastError = "Previous check failed"
        settings.presetManifest.sources[0].cachedAgentSetup = BrokerAgentSetupNotice(
            revision: 7, changedAt: "2026-09-20", summary: "Stored setup notice"
        )
        settings.presetManifest.isEnabled = false
        settings.applyProfile(id: BrokerAgentProfile.balancedID)
        var first = BrokerRuleSet.default
        first.thresholds.weeklyPct = 66
        XCTAssertTrue(settings.restoreProfileRules(id: BrokerAgentProfile.balancedID, rules: first))
        settings.policy.roles["planning"] = [BrokerCandidate(route: .t3, instance: "claudeAgent", model: "claude-fable-5")]
        let previous = settings.policy.ruleSet
        let baseline = settings.activeProfileRules
        let excluded = try excludedSettingsBytes(settings)
        var second = BrokerRuleSet.default
        second.thresholds.weeklyPct = 55
        XCTAssertTrue(settings.restoreProfileRules(id: BrokerAgentProfile.balancedID, rules: second))
        XCTAssertEqual(settings.routingRecovery?.previousRules, previous)
        XCTAssertEqual(try excludedSettingsBytes(settings), excluded)
        XCTAssertTrue(settings.undoRoutingUpdate())
        XCTAssertEqual(settings.policy.ruleSet, previous)
        XCTAssertEqual(settings.activeProfileRules, baseline)
        XCTAssertEqual(try excludedSettingsBytes(settings), excluded)
        XCTAssertNil(settings.routingRecovery)
    }

    private func excludedSettingsBytes(_ settings: BrokerSettings) throws -> Data {
        let encoder = JSONEncoder()
        var json = try XCTUnwrap(JSONSerialization.jsonObject(with: encoder.encode(settings)) as? [String: Any])
        for key in ["active_profile_id", "active_profile_rules", "routing_recovery", "routing_recovery_dismissed_in_popover"] {
            json.removeValue(forKey: key)
        }
        var policy = try XCTUnwrap(json["policy"] as? [String: Any])
        for key in ["roles", "thresholds", "callers", "allow_forced_degraded", "agent_model_aliases", "models"] {
            policy.removeValue(forKey: key)
        }
        var t3 = try XCTUnwrap(policy["t3"] as? [String: Any])
        t3.removeValue(forKey: "instance_by_model")
        t3.removeValue(forKey: "default_instance")
        policy["t3"] = t3
        json["policy"] = policy
        return try JSONSerialization.data(withJSONObject: json, options: [.sortedKeys])
    }

    func test_legacyAndMalformedOptionalRecoveryMetadataDoNotResetAppSettings() async throws {
        let suite = "BrokerAgentProfileTests-recovery-\(UUID().uuidString)"
        ...
        settings.broker = makeSettings()
        settings.broker.applyProfile(id: BrokerAgentProfile.balancedID)
        var revision = BrokerRuleSet.default
        revision.thresholds.weeklyPct = 42
        XCTAssertTrue(settings.broker.restoreProfileRules(id: BrokerAgentProfile.balancedID, rules: revision))
        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .iso8601
        let original = try XCTUnwrap(JSONSerialization.jsonObject(with: encoder.encode(settings)) as? [String: Any])
        let metadataVariants: [[String: Any]] = [
            [:], ["origin": "future-origin"],
            ["origin": 42, "source": ["wrong": true], "revisionFingerprint": [1, 2]],
        ]
        for metadata in metadataVariants {
            var json = original
            var broker = try XCTUnwrap(json["broker"] as? [String: Any])
            var recovery = try XCTUnwrap(broker["routing_recovery"] as? [String: Any])
            for key in ["origin", "source", "revisionFingerprint"] { recovery.removeValue(forKey: key) }
            recovery.merge(metadata) { _, new in new }
            broker["routing_recovery"] = recovery
            json["broker"] = broker
            defaults.set(try JSONSerialization.data(withJSONObject: json), forKey: "app_settings")
            let loaded = await repository.load()
            XCTAssertEqual(loaded.refreshInterval, 600)
            XCTAssertFalse(loaded.hasNotificationsEnabled)
            XCTAssertEqual(loaded.broker.policy, settings.broker.policy)
            XCTAssertEqual(loaded.broker.routingRecovery?.previousRules, settings.broker.routingRecovery?.previousRules)
            XCTAssertEqual(loaded.broker.routingRecovery?.previousBaseline, settings.broker.routingRecovery?.previousBaseline)
            XCTAssertEqual(try excludedSettingsBytes(loaded.broker), try excludedSettingsBytes(settings.broker))
        }
    }
    ...
    func test_ruleSet_carriesRulesAndNotMachineState() {
        ...
        XCTAssertEqual(rules.models, policy.models)
    ...
    func test_ruleSet_export_stripsAccountIDsFromPortableModelMetadata() throws {
        var rules = BrokerRuleSet.default
        rules.models["claude-future"] = BrokerModelSpec(
            quota: .claudeAccount(
                accountId: "secret-account", labelContains: "Team", isPrimary: nil
            )
        )

        let json = String(data: try JSONEncoder().encode(rules), encoding: .utf8) ?? ""

        XCTAssertFalse(json.contains("secret-account"))
        XCTAssertFalse(json.contains("account_id"))
    }
    ...
    func test_applyRules_leavesInstancesIgnoreListAndUsageLanesUntouched() {
        ...
        XCTAssertEqual(policy.models, BrokerAgentProfile.conserveQuota.rules.models)
    }

    func test_builtInProfiles_offerStandardWorkAsAutomaticModelChoices() {
        for profile in BrokerAgentProfile.builtIns {
            let chain = profile.rules.roles["standard", default: []]
            let candidates = profile.rules.roles.values.flatMap { $0 }
            XCTAssertTrue(chain.allSatisfy { $0.route == .auto }, profile.name)
            // Standard work must always be able to reach the Codex pool from
            // the standard chain itself, whichever GPT tier leads it.
            XCTAssertTrue(chain.contains { $0.model.hasPrefix("gpt-") }, profile.name)
            XCTAssertTrue(candidates.contains { $0.model == "gpt-5.6-sol" }, profile.name)
            XCTAssertTrue(candidates.contains { $0.model == "claude-fable-5-1" }, profile.name)
            XCTAssertFalse(candidates.contains { $0.model == "claude-fable-5" }, profile.name)
        }
    }
    ...
    func test_profile_encodeDecodeRoundTripsEveryRule() throws {
        var rules = BrokerAgentProfile.conserveQuota.rules
        rules.models["gpt-future"] = BrokerModelSpec(
            quota: .chatGPT(labelContains: "codex weekly"),
            effort: BrokerEffortCapability(levels: [BrokerEffort(rawValue: "ultra")!])
        )
        rules.roles["standard"] = [
            BrokerCandidate(
                route: .auto, model: "gpt-future", effort: BrokerEffort(rawValue: "ultra")
            )
        ]
        var profile = BrokerAgentProfile(name: "Round Trip", rules: rules)
        ...
        var rules = BrokerRuleSet(
            roles: [
                "review": [
                    ...
                    BrokerCandidate(route: .t3, model: "other"),
                    BrokerCandidate(route: .t3, model: "unmapped"),
    ...
    func test_unknownInstanceReferences_automaticRulesNeedNoInstanceRows() {
        ...
        XCTAssertEqual(
            ...
            [],
            "automatic model choices resolve from detected capability at pick time"
        )

        settings.policy.t3Instances.append(T3InstanceConfig(id: "codex", name: "Codex"))
        XCTAssertEqual(
            settings.unknownInstanceReferences(in: settings.policy.ruleSet), [],
            "adding a legacy mapped row does not change automatic rules"
        )
    }
    ...
    func test_builtIns_areAllMarkedBuiltInAndCarryEveryShippedRole() {
        let shippedRoles: Set<String> = [
            "planning", "design", "research", "review", "explore",
            "verification", "heavy", "standard", "execution",
        ]
        XCTAssertEqual(Set(BrokerPolicy.bundledDefault.roles.keys), shippedRoles)
        for profile in BrokerAgentProfile.builtIns {
    ...
    func test_builtIns_carryTheExpectedExploreAndVerificationChains() {
        func signature(_ profile: BrokerAgentProfile, role: String) -> [String] {
            profile.rules.roles[role, default: []].map {
                "\($0.model):\($0.effort?.rawValue ?? "default")"
            }
        }

        XCTAssertEqual(
            signature(BrokerAgentProfile.balanced, role: "explore"),
            ["claude-haiku-4-5-20251001:default", "gpt-5.6-terra:medium", "claude-sonnet-5:low"]
        )
        XCTAssertEqual(
            signature(BrokerAgentProfile.balanced, role: "verification"),
            ["claude-sonnet-5:high", "gpt-5.6-terra:high", "claude-haiku-4-5-20251001:default"]
        )
        XCTAssertEqual(
            signature(BrokerAgentProfile.conserveQuota, role: "explore"),
            ["claude-haiku-4-5-20251001:default", "gpt-5.6-luna:medium", "gpt-5.6-terra:medium"]
        )
        XCTAssertEqual(
            signature(BrokerAgentProfile.conserveQuota, role: "verification"),
            ["gpt-5.6-terra:medium", "claude-sonnet-5:medium", "claude-haiku-4-5-20251001:default"]
        )
        XCTAssertEqual(
            signature(BrokerAgentProfile.maxQuality, role: "explore"),
            ["claude-sonnet-5:medium", "gpt-5.6-terra:high", "claude-haiku-4-5-20251001:default"]
        )
        XCTAssertEqual(
            signature(BrokerAgentProfile.maxQuality, role: "verification"),
            ["claude-opus-5-5:high", "claude-sonnet-5:high", "gpt-5.6-sol:high"]
        )
        XCTAssertEqual(
            signature(BrokerAgentProfile.offloadExecution, role: "explore"),
            ["gpt-5.6-terra:medium", "claude-haiku-4-5-20251001:default", "claude-sonnet-5:low"]
        )
        XCTAssertEqual(
            signature(BrokerAgentProfile.offloadExecution, role: "verification"),
            ["gpt-5.6-terra:high", "claude-sonnet-5:high", "claude-haiku-4-5-20251001:default"]
        )
    }
    ...
    func test_maxQuality_refusesDegradedFallbackForEveryJudgmentRoleExceptReview() {
        let rules = BrokerAgentProfile.maxQuality.rules
        XCTAssertEqual(BrokerAgentProfile.judgmentRoles, ["design", "planning", "research", "review"])
        // `review` exhausts its three quota pools often enough that the hard
        // stop dead-ends the completion gate. A degraded pick is confirmed
        // with the user before dispatch, so it is a choice, not a downgrade.
        XCTAssertEqual(
            rules.allowForcedDegraded["review"], true,
            "review must offer a confirmable degraded pick instead of dead-ending"
        )
    ...
    /// A ceiling gates dispatch, not spend: in-flight threads and Codex CLI use
    /// outside the broker keep consuming the same window, so real usage
    /// overshoots whatever the ceiling is. The ChatGPT window is the one every
    /// role competes for, so Max Quality must not raise it above the shipped
    /// default and spend the reserve that absorbs the overshoot.
    func test_maxQuality_keepsAReserveOnTheSharedChatGPTWindow() {
        let thresholds = BrokerAgentProfile.maxQuality.rules.thresholds

        XCTAssertEqual(thresholds.chatgptWeeklyPct, 90)
        XCTAssertLessThanOrEqual(
            thresholds.chatgptWeeklyPct, BrokerThresholds.default.chatgptWeeklyPct,
            "execution and standard reach this same window through gpt-5.6-sol, so raising "
                + "the ceiling here starves them"
        )
        // The Claude ceilings deliberately stay higher: two accounts sit behind
        // them, so exhausting one is not the same kind of event.
        XCTAssertGreaterThan(
            thresholds.fableWeeklyPct, BrokerThresholds.default.fableWeeklyPct
        )
        XCTAssertGreaterThan(
            thresholds.weeklyPct, BrokerThresholds.default.weeklyPct
        )
    }
    ...
    func test_decodeExported_acceptsAFileThisAppExported() throws {
        var settings = makeSettings()
        settings.policy.thresholds.sessionPct = 61
        settings.policy.models["gpt-future"] = BrokerModelSpec(
            quota: .chatGPT(labelContains: "codex weekly"),
            effort: BrokerEffortCapability(levels: [BrokerEffort(rawValue: "ultra")!])
        )
        ...
        XCTAssertEqual(decoded.rules.models["gpt-future"], settings.policy.models["gpt-future"])
    }
    ...
    /// `addCandidate`/`addRole` seed the first sorted *chain* model; alias-only
    /// legacy ids trail the list. Pinning it here means the seed changing is a
    /// test failure, not a stale comment.
    func test_knownModelIDs_seedsTheFirstChainModelNotTheLegacyAlias() {
        // The legacy `claude-fable-5` alias key sorts before `claude-fable-5-1`
        // but is alias-only, so chain models must still lead the list.
        XCTAssertEqual(
            BrokerPolicyEditorView.knownModelIDs(in: .bundledDefault).first,
            "claude-fable-5-1"
        )
        let choices = BrokerPolicyEditorView.knownModelIDs(in: .bundledDefault)
        let chainModels = Set(BrokerPolicy.bundledDefault.roles.values.flatMap { $0 }.map(\.model))
        XCTAssertEqual(Array(choices.prefix(chainModels.count)), chainModels.sorted())
        XCTAssertTrue(choices.dropFirst(chainModels.count).contains("claude-fable-5"))
    }

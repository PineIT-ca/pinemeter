...
final class BrokerAgentProfileTests: XCTestCase {
    ...
    /// The shipped seed routes some judgment work to `claude_secondary`, which no
    /// Mac has until the user adds it — so a fresh install's own rules produce
    /// the caption, and it must name exactly that instance.
    func test_unknownInstanceReferences_freshInstallNamesTheUnaddedSeedInstance() {

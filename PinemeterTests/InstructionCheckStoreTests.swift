...
//  PinemeterTests
//
//  The record behind the Instructions tab's "last checked" line. It is the
...
final class InstructionCheckStoreTests: XCTestCase {
    ...
    func test_batchesSharingARunIDMergeByPathAndLastWins() async throws {
        ...
        await store.record(
            ...
            runID: "run-1", caller: nil, at: checkedAt
        )
        await store.record(
            report: report([("c.md", .conflict, ["clash"]), ("a.md", .pass, [])]),
            runID: "run-1", caller: nil, at: checkedAt.addingTimeInterval(30)
        )
    ...
    func test_aDifferentOrAbsentRunIDReplacesRatherThanAccumulates() async throws {
        let store = makeStore()
        await store.record(report: report([("a.md", .pass, [])]), runID: "run-1", caller: nil, at: checkedAt)
        await store.record(report: report([("b.md", .pass, [])]), runID: "run-2", caller: nil, at: checkedAt)
        ...
        await store.record(report: report([("c.md", .pass, [])]), runID: nil, caller: nil, at: checkedAt)
        await store.record(report: report([("d.md", .pass, [])]), runID: nil, caller: nil, at: checkedAt)
    ...
    func test_aRecordFromTheFutureIsNotMergedInto() async throws {
        ...
        await store.record(report: report([("now.md", .pass, [])]), runID: "stable",
                           caller: nil, at: checkedAt.addingTimeInterval(-60))
        let latest = await store.latest()
    ...
    func test_findingsAreCappedPerSourceAndPerMessage() async throws {
        ...
        await store.record(
            ...
            runID: nil, caller: nil, at: checkedAt
        )
    ...
    func test_aWriteFailureLeavesTheStoreOnItsLastGoodRecord() async throws {
        ...
        await store.record(report: report([("a.md", .pass, [])]), runID: nil, caller: nil, at: checkedAt)

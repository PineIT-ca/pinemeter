...
final class AppSettingsTests: XCTestCase {
    ...
    func test_decodingSavedNativeOnlyStandardRole_addsCodexRoutesOnce() throws {
        ...
        XCTAssertEqual(routes, [.native, .t3, .codex])
    ...
    func test_decodingSaveWithoutMigrationRecord_movesReviewOntoTheOpusChain() throws {
        ...
        XCTAssertEqual(settings.policy.roles["review"]?.first?.id, "native/claude-opus-5")
        XCTAssertEqual(settings.policy.roles["review"]?.first?.effort, .xhigh)
        XCTAssertTrue(
        ...
        XCTAssertEqual(settings.policy.roles["execution"]?.map(\.id), ["t3/gpt-5.6-sol"])
    }
    ...
    func test_decodingSaveWithMigrationAlreadyRecorded_leavesAUserEditedReviewChainAlone() throws {
        ...
        XCTAssertEqual(settings.policy.roles["review"]?.map(\.id), ["native/claude-haiku-4-5-20251001"])
    }

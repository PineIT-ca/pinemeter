...
//  PinemeterTests
//
//  The Broker window's non-pixel behaviour: chain re-ranking (the edit that
...
final class BrokerRoutingEditorTests: XCTestCase {
    ...
    private let threeCandidateChain = [
        ...
        BrokerCandidate(route: .t3, instance: "claude_secondary", model: "claude-sonnet-5"),
    ...
    @MainActor
    func test_modelChoices_collapseRouteDuplicates() {
        let chain = [
            BrokerCandidate(route: .native, model: "claude-fable-5", effort: .xhigh),
            BrokerCandidate(route: .t3, model: "claude-fable-5", effort: .high),
            BrokerCandidate(route: .native, model: "claude-sonnet-5"),
        ]
        let choices = BrokerPolicyEditorView.modelChoices(in: chain)
        XCTAssertEqual(choices.map(\.id), ["auto/claude-fable-5", "auto/claude-sonnet-5"])
        XCTAssertEqual(choices.first?.effort, .xhigh)
    }

...
final class MenuBarIconRendererTests: XCTestCase {
    ...
    func test_popoverWidthFitsActualTwoAndThreeBarLayouts() {
        ...
        XCTAssertEqual(QuotaChartLayout.popoverWidth(for: nineBars), 764)
        XCTAssertEqual(QuotaChartLayout.popoverWidth(for: (1...40).map { bar("Quota \($0)") }), 764)
    }

    func test_resetTextDoesNotWidenQuotaColumns() {
        let bar = MenuBarQuotaBar(
            label: "Claude 5h",
            percentage: 20,
            status: .safe,
            detail: "Mon at 6:30pm\nin 4d 6h 15m",
            heading: "5h"
        )

        XCTAssertEqual(QuotaChartLayout.columnWidth(for: bar), 72)
        XCTAssertEqual(QuotaChartLayout.popoverWidth(for: [bar, bar, bar]), 284)
    }

    func test_resetLabelAccessibilityPreservesSpokenUnitsAndLocalTime() {
        XCTAssertEqual(
            QuotaResetLabel(detail: "Fri at 8:30pm\nin 4d 6h 15m").accessibilityDescription,
            "Resets Fri at 8:30pm, in 4 days 6 hours 15 minutes"
        )
        XCTAssertEqual(QuotaResetLabel(detail: "in 1d 1h 1m").accessibilityDescription,
                       "Resets in 1 day 1 hour 1 minute")
        XCTAssertEqual(QuotaResetLabel(detail: "now").accessibilityDescription, "Resets now")
    }

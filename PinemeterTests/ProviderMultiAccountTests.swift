...
final class ProviderMultiAccountTests: XCTestCase {
    ...
    private static let cookieC = "__Secure-next-auth.session-token=cookie-account-c"
    private static let userA = "user-aaaaaaaaaaaaaaaaaaaaaaaa"
    private static let userB = "user-bbbbbbbbbbbbbbbbbbbbbbbb"
    private static let userC = "user-cccccccccccccccccccccccc"
    ...
    func test_importAndSaveChatGPTSessionCookie_retainsAccountsFromOtherBrowserSources() async throws {
        let answerA: MultiChatGPTUsageServiceStub.Answer = (
            usage(34),
            identity(userId: Self.userA, email: "a@example.com")
        )
        let answerB: MultiChatGPTUsageServiceStub.Answer = (
            usage(12),
            identity(userId: Self.userB, email: "b@example.com")
        )
        let usageService = MultiChatGPTUsageServiceStub(
            byCookie: [Self.cookieA: answerA, Self.cookieB: answerB],
            byAccount: [
                ChatGPTAccount.primaryKeychainAccount: .success(answerA),
                Self.userB: .success(answerB),
            ]
        )
        let sessionRepository = MultiChatGPTSessionRepositoryFake()
        try await sessionRepository.save(
            ChatGPTSession(sessionCookie: Self.cookieA),
        ...
        let appModel = makeAppModel(
            ...
            importService: SessionKeyImportServiceStub(
                result: .failure(SessionKeyImportError.noSessionKeyFound),
                allChatGPTResults: [
                    ImportedChatGPTSessionCookie(
                        cookieHeader: Self.cookieA,
        ...
        XCTAssertEqual(Set(appModel.settings.chatGPTAccounts.map(\.id)), Set([Self.userA, Self.userB]))
        XCTAssertEqual(
            appModel.settings.chatGPTAccounts.first { $0.id == Self.userB }?.profileLabel,
            "Safari Profile"
        )
        let retained = try await sessionRepository.load(account: Self.userB)
        XCTAssertEqual(retained.sessionCookie, Self.cookieB)
    }
    ...
    func test_refreshAdditionalChatGPTAccounts_storesFreshUsageUnderChangedIdentity() async throws {
        let usageService = MultiChatGPTUsageServiceStub(byAccount: [
            Self.userB: .success((usage(47), identity(userId: Self.userC, email: "c@example.com")))
        ])
        let appModel = makeAppModel(
            usageService: usageService,
            sessionRepository: MultiChatGPTSessionRepositoryFake(),
        ...
        XCTAssertNil(appModel.chatGPTAccountUsage[Self.userB])
        XCTAssertEqual(appModel.chatGPTAccountUsage[Self.userC]?.rows.first?.usedPercent, 47)
        XCTAssertEqual(appModel.settings.chatGPTAccounts.first { !$0.isPrimary }?.id, Self.userC)
    }

    func test_refreshAdditionalChatGPTAccounts_waitsForOfflineRetriesConcurrently() async throws {
        let sessionRepository = MultiChatGPTSessionRepositoryFake()
        try await sessionRepository.save(ChatGPTSession(sessionCookie: Self.cookieB), account: Self.userB)
        try await sessionRepository.save(ChatGPTSession(sessionCookie: Self.cookieC), account: Self.userC)
        let httpClient = AlwaysOfflineChatGPTHTTPClientStub()
        let usageService = ChatGPTUsageService(
            httpClient: httpClient,
            sessionRepository: sessionRepository,
            networkRetryDelay: .milliseconds(200)
        )
        let appModel = makeAppModel(
            usageService: usageService,
            sessionRepository: sessionRepository,
        ...
        appModel.settings.chatGPTAccounts = [
            ChatGPTAccount(id: Self.userA, label: "a@example.com", keychainAccount: ChatGPTAccount.primaryKeychainAccount),
            ChatGPTAccount(id: Self.userB, label: "b@example.com", keychainAccount: Self.userB),
            ChatGPTAccount(id: Self.userC, label: "c@example.com", keychainAccount: Self.userC),
        ]

        await appModel.refreshAdditionalChatGPTAccounts()

        let cookieHeaders = await httpClient.cookieHeaders
        XCTAssertEqual(cookieHeaders.count, 4, "Each offline account retries exactly once")
        XCTAssertEqual(
            Set(cookieHeaders.prefix(2)),
            Set([Self.cookieB, Self.cookieC]),
            // This is the ONLY assertion that distinguishes the task group from
            // the old serial loop: a serial run also yields two requests per
            // cookie and four in total, but its first two entries are the same
            // cookie twice. The 200 ms retry delay is the starvation margin.
            "Every account must start before any account begins its retry"
        )
        XCTAssertEqual(cookieHeaders.filter { $0 == Self.cookieB }.count, 2)
        XCTAssertEqual(cookieHeaders.filter { $0 == Self.cookieC }.count, 2)
        XCTAssertEqual(
            appModel.chatGPTAccountErrors[Self.userB],
            ChatGPTUsageError.networkUnavailable.localizedDescription
        )
        XCTAssertEqual(
            appModel.chatGPTAccountErrors[Self.userC],
            ChatGPTUsageError.networkUnavailable.localizedDescription
        )
    }

    func test_cancelledChatGPTRefreshDoesNotSurfaceDashboardErrors() async {
        let usageService = MultiChatGPTUsageServiceStub(byAccount: [
            ChatGPTAccount.primaryKeychainAccount: .failure(CancellationError()),
            Self.userB: .failure(CancellationError()),
        ])
        let appModel = makeAppModel(
            usageService: usageService,
            sessionRepository: MultiChatGPTSessionRepositoryFake(),
        ...
        XCTAssertNil(appModel.chatGPTAccountErrors[Self.userB])
        XCTAssertEqual(appModel.chatGPTUsageData?.rows.first?.usedPercent, 34)
        XCTAssertEqual(appModel.chatGPTAccountUsage[Self.userB]?.rows.first?.usedPercent, 12)
    }
    ...
    /// Reconnecting while offline: the browser holds a rotated cookie for the
    /// one connected account, nothing validates, and retaining the stored
    /// session pushes the cookie count past the fallback's one-cookie limit.
    /// The import must still fall back and save the rotated cookie, which is
    /// the whole point of reconnecting.
    func test_importAndSaveChatGPTSessionCookie_savesARotatedCookieWhenNothingValidates() async throws {
        let sessionRepository = MultiChatGPTSessionRepositoryFake()
        try await sessionRepository.save(
            ChatGPTSession(sessionCookie: Self.cookieA),
            account: ChatGPTAccount.primaryKeychainAccount
        ...
        let stored = try await sessionRepository.load(account: ChatGPTAccount.primaryKeychainAccount)
        XCTAssertEqual(stored.sessionCookie, Self.cookieB, "the rotated cookie replaces the stale one")
        let account = try XCTUnwrap(appModel.settings.chatGPTAccounts.first)
        XCTAssertEqual(account.id, Self.userA)
        XCTAssertEqual(account.customLabel, "Work GPT")
    }

    /// A retained session must never be described by the account's own label:
    /// that label is the provider-reported email, and the description is
    /// logged and persisted as `profileLabel`.
    func test_importAndSaveChatGPTSessionCookie_doesNotDescribeARetainedSessionByItsEmail() async throws {
        let answerB: MultiChatGPTUsageServiceStub.Answer = (
            usage(12),
            identity(userId: Self.userB, email: "b@example.com")
        )
            ...
            usageService: MultiChatGPTUsageServiceStub(
                ...
                byAccount: [
                    ChatGPTAccount.primaryKeychainAccount: .success(
                        (usage(34), identity(userId: Self.userA, email: "a@example.com"))
                    ),
                    Self.userB: .success(answerB),
                ]
            ),
        ...
        // No profileLabel, so the retained session has only the email to fall
        // back on -- exactly the case that must not use it.
        appModel.settings.chatGPTAccounts = [
            ChatGPTAccount(
                id: Self.userA,
                label: "a@example.com",
        ...
        let retained = try XCTUnwrap(appModel.settings.chatGPTAccounts.first { $0.id == Self.userB })
        XCTAssertEqual(retained.profileLabel, AppModel.retainedSessionSourceDescription)
        XCTAssertNotEqual(retained.profileLabel, "b@example.com")
    }
    ...
    func test_removeChatGPTAccount_skipsAnUnreadableSuccessorAndPromotesTheNextLoadableAccount() async throws {
        let usageService = MultiChatGPTUsageServiceStub(byAccount: [
            ChatGPTAccount.primaryKeychainAccount: .success((
                usage(21),
                identity(userId: Self.userC, email: "c@example.com", plan: "plus")
            ))
        ])
        let sessionRepository = MultiChatGPTSessionRepositoryFake()
        try await sessionRepository.save(
        ...
        let promoted = try XCTUnwrap(appModel.settings.chatGPTAccounts.first { $0.isPrimary })
        XCTAssertEqual(promoted.id, Self.userC)
        XCTAssertEqual(promoted.label, "c@example.com")
        XCTAssertEqual(promoted.planType, "plus")
        XCTAssertEqual(promoted.profileLabel, "Safari Profile")
        XCTAssertEqual(promoted.customLabel, "Side")
        XCTAssertTrue(appModel.settings.chatGPTAccounts.contains { $0.id == Self.userB })
        let savedPrimary = try await sessionRepository.load(account: ChatGPTAccount.primaryKeychainAccount)
        XCTAssertEqual(savedPrimary.sessionCookie, Self.cookieC)
    }

    func test_removeChatGPTAccount_clearsProviderWhenNoSuccessorSessionLoads() async throws {
        let sessionRepository = MultiChatGPTSessionRepositoryFake()
        try await sessionRepository.save(
            ChatGPTSession(sessionCookie: Self.cookieA),
            account: ChatGPTAccount.primaryKeychainAccount
        ...
        XCTAssertFalse(appModel.settings.isChatGPTUsageShown)
        let storedAccounts = await sessionRepository.storedAccounts
...
private actor AlwaysOfflineChatGPTHTTPClientStub: ChatGPTHTTPClientProtocol {

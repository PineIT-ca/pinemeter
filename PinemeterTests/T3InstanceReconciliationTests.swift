...
final class T3InstanceReconciliationTests: XCTestCase {
    ...
    func test_reconcile_bundledSeeds_keepTheirDisplayNamesAfterATersDiscoveryScan() {
        ...
        let discovered = [
            ...
            DiscoveredT3Instance(instanceId: "claude_secondary", driver: "claudeAgent", displayName: "AU", installed: true),
        ...
        XCTAssertEqual(
            policy.t3Instances.first { $0.id == "claude_secondary" }?.name, "AU",
            "an instance the seed does not ship takes the name discovery reported"
        )
    }
    ...
    func test_reconcile_seedFableLaneResolution_neverChangesWhenOnlySecondaryClaudeInstanceIsInstalled() {
        // The CR-03 blowout scenario: only claude_secondary (the secondary
        ...
        _ = policy.reconcileDiscoveredT3Instances([
            DiscoveredT3Instance(instanceId: "claude_secondary", driver: "claudeAgent", installed: true, modelSlugs: ["claude-fable-5"])
        ])
    ...
    func test_reconcile_everyCreatedRow_hasNilBoundAccountId() {
        ...
        _ = policy.reconcileDiscoveredT3Instances([
            DiscoveredT3Instance(instanceId: "claude_secondary", driver: "claudeAgent", installed: true)
        ])

        XCTAssertNil(policy.t3Instances.first { $0.id == "claude_secondary" }?.boundAccountId)
    }

    // MARK: - Account binding applicability

    func test_supportsAccountBinding_claudeAgentDriver_isTrue() {
        let instance = T3InstanceConfig(
            id: "claude_secondary",
            name: "Claude (second account)",
            driver: T3InstanceConfig.claudeAgentDriver
        )
        XCTAssertTrue(instance.supportsAccountBinding)
    }

    /// A manual row discovery has never confirmed keeps the control, since it
    /// may still turn out to be a Claude lane.
    func test_supportsAccountBinding_unknownDriver_isTrue() {
        let instance = T3InstanceConfig(id: "manual-instance-1", name: "New Instance")
        XCTAssertTrue(instance.supportsAccountBinding)
    }

    func test_supportsAccountBinding_nonClaudeDrivers_isFalse() {
        for driver in ["codex", "opencode", "cursor"] {
            let instance = T3InstanceConfig(id: driver, name: driver, driver: driver)
            XCTAssertFalse(instance.supportsAccountBinding, "\(driver) must not offer an account binding")
        }
    }

    func test_showsAccountPicker_nonClaudeDriverWithoutBinding_isHidden() {
        let instance = T3InstanceConfig(id: "codex", name: "Codex", driver: "codex")
        XCTAssertFalse(BrokerInstancesPane.showsAccountPicker(for: instance))
    }

    /// A binding written before the driver was known stays clearable.
    func test_showsAccountPicker_nonClaudeDriverWithBinding_staysVisible() {
        let instance = T3InstanceConfig(
            id: "codex", name: "Codex", boundAccountId: "acct-123", driver: "codex"
        )
        XCTAssertTrue(BrokerInstancesPane.showsAccountPicker(for: instance))
    }

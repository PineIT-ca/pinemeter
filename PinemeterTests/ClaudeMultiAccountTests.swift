...
final class ClaudeMultiAccountTests: XCTestCase {
    ...
    private static let org3UUIDString = "33333333-3333-3333-3333-333333333333"
    ...
    func test_connectClaudeAccounts_routesConcurrentValidationThroughController() async throws {
        let org1 = organization(id: 1, uuid: Self.org1UUIDString, name: "Acme")
        let org2 = organization(id: 2, uuid: Self.org2UUIDString, name: "Personal")
        let usageService = SuspendedConnectionUsageService(
            organizationsByKey: [Self.key1: [org1], Self.key2: [org2]],
            usage: makeUsageData(percentage: 22)
        )
        let appModel = AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
        ...
        let connection = Task {
            try await appModel.connectClaudeAccounts(importedKeys: [
                ImportedSessionKey(value: Self.key1, sourceDescription: "Chrome Profile 1"),
                ImportedSessionKey(value: Self.key2, sourceDescription: "Chrome Profile 15"),
            ])
        }

        await usageService.waitUntilBothValidationsEntered()
        let validationCount = await usageService.fetchOrganizationsCallCount
        XCTAssertEqual(validationCount, 2)
        await usageService.releaseValidations()

        let result = try await connection.value
        XCTAssertEqual(result.importedCount, 2)
        XCTAssertEqual(appModel.settings.claudeAccounts.count, 2)
    }
    ...
    func test_usageQuotaBars_subscriptionResetModeChangesOnlyPrimaryClaudeDetail() {
        let appModel = AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
            usageService: MultiAccountUsageServiceStub(
        ...
        appModel.usageData = UsageData(
            sessionUsage: UsageLimit(utilization: 37, resetAt: resetAt),
            weeklyUsage: UsageLimit(utilization: 52, resetAt: resetAt.addingTimeInterval(86_400)),
            sonnetUsage: nil,
            lastUpdated: Date(timeIntervalSince1970: 0)
        )

        appModel.settings.subscriptionResetAnnouncementMode = .timeRemaining
        let remainingBar = appModel.usageQuotaBars[0]
        appModel.settings.subscriptionResetAnnouncementMode = .localResetTime
        let localBar = appModel.usageQuotaBars[0]

        XCTAssertEqual(localBar.detail, SubscriptionResetAnnouncementMode.localResetTime.resetAnnouncement(for: resetAt))
        XCTAssertNotEqual(localBar.detail, remainingBar.detail)
        XCTAssertEqual(localBar.label, remainingBar.label)
        XCTAssertEqual(localBar.heading, remainingBar.heading)
        XCTAssertEqual(localBar.owner, remainingBar.owner)
        XCTAssertEqual(localBar.percentage, remainingBar.percentage)
        XCTAssertEqual(localBar.status, remainingBar.status)
        XCTAssertEqual(localBar.renameTarget, remainingBar.renameTarget)
        XCTAssertEqual(localBar.colorScheme, remainingBar.colorScheme)
    }

    func test_usageQuotaBars_subscriptionResetModeAppliesAcrossEveryProviderAndOnlyChangesDetail() {
        let appModel = AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
            usageService: MultiAccountUsageServiceStub(
        ...
        let baseReset = Date(timeIntervalSince1970: 4_102_444_800)
        let primaryUsage = makeUsageData(
            percentage: 11,
            sessionResetAt: baseReset,
            weeklyResetAt: baseReset.addingTimeInterval(1_000),
            fableResetAt: baseReset.addingTimeInterval(2_000)
        )
        let secondaryUsage = makeUsageData(
            percentage: 22,
            sessionResetAt: baseReset.addingTimeInterval(3_000),
            weeklyResetAt: baseReset.addingTimeInterval(4_000),
            fableResetAt: baseReset.addingTimeInterval(5_000)
        )
        let chatGPTReset = baseReset.addingTimeInterval(6_000)
        let geminiReset = baseReset.addingTimeInterval(7_000)
        ...
        appModel.usageData = primaryUsage
        appModel.claudeAccountUsage[Self.org2UUIDString] = secondaryUsage
        ...
        appModel.chatGPTUsageData = ChatGPTUsageData(
            rows: [
                .init(
                    label: "5 hour",
                    usedPercent: 41,
                    resetAt: chatGPTReset,
                    menuBarRole: .chatGPT5h
                ),
                .init(label: "Codex Tasks", usedPercent: 17, resetAt: nil),
            ],
            lastUpdated: Date(timeIntervalSince1970: 0)
        )
        ...
        let resetDates: [Date?] = [
            primaryUsage.sessionUsage.resetAt,
            primaryUsage.weeklyUsage.resetAt,
            primaryUsage.fableUsage?.resetAt,
            secondaryUsage.sessionUsage.resetAt,
            secondaryUsage.weeklyUsage.resetAt,
            secondaryUsage.fableUsage?.resetAt,
            chatGPTReset,
            nil,
            geminiReset,
        ]
        ...
        appModel.settings.subscriptionResetAnnouncementMode = .localResetTime
        let localBars = appModel.usageQuotaBars
        XCTAssertEqual(localBars.count, resetDates.count)
        XCTAssertEqual(
            localBars.map(\.detail),
            resetDates.map { resetAt in
                resetAt.map { SubscriptionResetAnnouncementMode.localResetTime.resetAnnouncement(for: $0) }
            }
        )

        appModel.settings.subscriptionResetAnnouncementMode = .both
        let combinedBars = appModel.usageQuotaBars
        assertQuotaBarsEqualExceptDetail(localBars, combinedBars)
        XCTAssertEqual(
            combinedBars.map(\.detail),
            resetDates.map { resetAt in
                resetAt.map { SubscriptionResetAnnouncementMode.both.resetAnnouncement(for: $0) }
            }
        )

        appModel.settings.subscriptionResetAnnouncementMode = .timeRemaining
        let remainingBars = appModel.usageQuotaBars
        assertQuotaBarsEqualExceptDetail(combinedBars, remainingBars)
        XCTAssertEqual(
            remainingBars.map(\.detail),
            resetDates.map { resetAt in
                resetAt.map { SubscriptionResetAnnouncementMode.timeRemaining.resetAnnouncement(for: $0) }
            }
        )
    }
    ...
    func test_renameProviderAccounts_updateDisplayLabelAndPopoverOwner() {
        ...
        appModel.renameUsageOwner(.provider(.chatGPT), customLabel: "Work GPT")
        appModel.renameUsageOwner(.provider(.gemini), customLabel: "Personal Gemini")
    ...
    private func assertQuotaBarsEqualExceptDetail(
        _ lhs: [MenuBarQuotaBar],
        _ rhs: [MenuBarQuotaBar],
        file: StaticString = #filePath,
        line: UInt = #line
    ) {
        XCTAssertEqual(lhs.count, rhs.count, file: file, line: line)
        for (left, right) in zip(lhs, rhs) {
            XCTAssertEqual(left.label, right.label, file: file, line: line)
            XCTAssertEqual(left.heading, right.heading, file: file, line: line)
            XCTAssertEqual(left.owner, right.owner, file: file, line: line)
            XCTAssertEqual(left.percentage, right.percentage, file: file, line: line)
            XCTAssertEqual(left.status, right.status, file: file, line: line)
            XCTAssertEqual(left.colorScheme, right.colorScheme, file: file, line: line)
            XCTAssertEqual(left.renameTarget, right.renameTarget, file: file, line: line)
        }
    }

    // MARK: - Per-account usage fetch

    func test_refreshAdditionalClaudeAccounts_concurrentCallsFetchEachAccountOnce() async {
        let usageService = SuspendedAdditionalAccountUsageService(usage: makeUsageData(percentage: 22))
        let appModel = AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
            usageService: usageService,
        ...
        appModel.settings.claudeAccounts = [
            ...
            ClaudeAccount(
                id: Self.org3UUIDString,
                label: "Third",
                organizationId: UUID(uuidString: Self.org3UUIDString)!,
                keychainAccount: Self.org3UUIDString
            ),
        ]

        let firstRefresh = Task { await appModel.refreshAdditionalClaudeAccounts(forceRefresh: true) }
        await usageService.waitUntilFirstFetchEntered()
        XCTAssertTrue(appModel.isRefreshingAdditionalClaudeAccounts)
        XCTAssertTrue(appModel.isRefreshingConfiguredUsage)

        await appModel.refreshAdditionalClaudeAccounts(forceRefresh: true)
        await usageService.releaseFirstFetch()
        await firstRefresh.value

        let callCounts = await usageService.callCounts
        XCTAssertEqual(callCounts[Self.org2UUIDString], 1)
        XCTAssertEqual(callCounts[Self.org3UUIDString], 1)
        XCTAssertFalse(appModel.isRefreshingAdditionalClaudeAccounts)
    }
...
private actor SuspendedAdditionalAccountUsageService: UsageServiceProtocol {
    let usage: UsageData
    private(set) var callCounts: [String: Int] = [:]
    private var firstFetchEntered = false
    ...
    func fetchUsage(forceRefresh: Bool) async throws -> UsageData { usage }

    func fetchUsage(account: String, organizationId: UUID, forceRefresh: Bool) async throws -> UsageData {
        callCounts[account, default: 0] += 1
        if !firstFetchEntered {
            firstFetchEntered = true
    ...
    func waitUntilFirstFetchEntered() async {
        if firstFetchEntered { return }
...
private actor SuspendedConnectionUsageService: UsageServiceProtocol {
    let organizationsByKey: [String: [Organization]]
    let usage: UsageData
    private(set) var fetchOrganizationsCallCount = 0
    ...
    init(organizationsByKey: [String: [Organization]], usage: UsageData) {
        self.organizationsByKey = organizationsByKey
        self.usage = usage
    }
    ...
    func fetchOrganizations(sessionKey: SessionKey) async throws -> [Organization] {
        ...
        if fetchOrganizationsCallCount <= 2 {
            if fetchOrganizationsCallCount == 2 {
        ...
        return organizationsByKey[sessionKey.value] ?? []
    }
    ...
    func waitUntilBothValidationsEntered() async {
        if fetchOrganizationsCallCount >= 2 { return }
        await withCheckedContinuation { bothEnteredContinuation = $0 }
    }
...
private func makeUsageData(
    ...
    UsageData(
        sessionUsage: UsageLimit(utilization: percentage, resetAt: sessionResetAt),
        weeklyUsage: UsageLimit(utilization: TestConstants.weeklyPercentage, resetAt: weeklyResetAt),
        sonnetUsage: nil,
        fableUsage: UsageLimit(utilization: 31, resetAt: fableResetAt),
        lastUpdated: Date(timeIntervalSince1970: 0)
    )
}

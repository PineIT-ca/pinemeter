...
final class BuildInfoTests: XCTestCase {
    func test_supportDetails_containsOnlyVersionBuildAndMacOS() {
        let bundle = Bundle(for: Self.self)
        let version = bundle.infoDictionary?["CFBundleShortVersionString"] as? String ?? "Unknown"
        let build = bundle.infoDictionary?["CFBundleVersion"] as? String ?? "Unknown"
        XCTAssertEqual(
            BuildInfo.supportDetails(bundle: bundle, macOSVersion: .init(majorVersion: 15, minorVersion: 6, patchVersion: 1)),
            "Version: \(version)\nBuild: \(build)\nmacOS: 15.6.1"
        )
    }

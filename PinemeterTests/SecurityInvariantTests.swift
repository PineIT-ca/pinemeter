...
final class SecurityInvariantTests: XCTestCase {
    ...
    func test_brokerStatusPayload_containsOnlyWhitelistedFieldsAndNoCredentialMaterial() throws {
        let status = BrokerStatus(
            ...
            cooldowns: [
                BrokerStatus.CooldownEntry(key: "t3:claude_secondary/claude-fable-5", availableAt: .now),
            ],
            t3: [
                BrokerStatus.RouteHealth(instanceId: "claude_secondary", reachable: true, why: "http 200"),
            ],
    ...
    func test_brokerPickDecisionPayload_containsOnlyWhitelistedFieldsAndNoCredentialMaterial() throws {
        let decision = BrokerDecision(
            ...
            route: .t3,
            agentModel: nil,
            invocation: .t3Dispatch(model: "claude-fable-5", instanceId: "claude_secondary"),
            reason: "t3:claude_secondary/claude-fable-5 account \"Claude (second account)\" weekly 42% < 85% ok",
            source: .policy,
            ...
            candidatesTried: [
                ...
                BrokerCandidateTried(
                    candidate: "t3:claude_secondary/claude-fable-5",
                    available: true,
                    why: "t3:claude_secondary/claude-fable-5 account \"Claude (second account)\" weekly 42% < 85% ok"
                ),
            ]
        )
    ...
    func test_webViewNetworkServiceDoesNotLogResponseBodies() throws {
        let source = try sourceContents(relativePath: "Pinemeter/Services/WebViewNetworkService.swift")

        for prohibitedPattern in ["responseBody", "\\nResponse:", "String(data: data"] {
            XCTAssertFalse(
                source.contains(prohibitedPattern),
                "WebViewNetworkService must not construct or log response bodies: \(prohibitedPattern)"
            )
        }
        XCTAssertTrue(source.contains("bytes=\\(data.count"))
    }
    ...
    func test_userFacingChatGPTErrorDescriptionsDoNotDiscloseCredentialShapedFragments() {
        let errors: [LocalizedError] = [
            ...
            ChatGPTUsageError.secureStorageUnavailable
        ]
    ...
    func test_providerCredentialResetSourcesStayScopedToSafeRepositoryBoundaries() throws {
        ...
        XCTAssertTrue(
            chatGPTResetBody.contains("try await chatGPTSessionRepository.clear(account: ChatGPTAccount.primaryKeychainAccount)"),
    ...
    func testPhase8SerializedStoresRespectSourceAndCorrelationBoundary() async throws {
        ...
        try await telemetryStore.flush()
        ...
        for rawField in ["raw_request", "raw_error", "source_path", "diff", "branch_name"] {
            let rawObject = """
        ...
        let forbiddenEffectiveDiffValues = [
            #"{"decision_id":"d","status":"started","source_files":"synthetic-secret-value"}"#,
            #"{"decision_id":"d","status":"started","source_lines":"/synthetic/project/File.swift"}"#,
            #"{"decision_id":"d","status":"started","generated_lines":"synthetic-file-content"}"#,
            #"{"decision_id":"d","status":"started","docs_lines":"synthetic-prompt-material"}"#,
            #"{"decision_id":"d","status":"started","trigger":"/synthetic/project/File.swift"}"#,
            #"{"decision_id":"d","status":"started","trigger":"synthetic-secret-value"}"#,
        ]
        for rawObject in forbiddenEffectiveDiffValues {
            XCTAssertThrowsError(
                try JSONDecoder().decode(BrokerLifecycleReport.self, from: Data(rawObject.utf8))
            )
        }

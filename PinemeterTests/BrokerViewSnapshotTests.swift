...
final class BrokerViewSnapshotTests: XCTestCase {
    ...
    func test_instructionsSettingsTab_showsInstalledAuditFindings() {
        let appModel = makeAppModelForBrokerTabSnapshot()
        let report = InstructionAuditReport(sources: [
            ...
            InstructionAuditSourceReport(
                path: "~/.codex/AGENTS.model-policy.md",
                status: .pass,
                findings: []
            ),
            InstructionAuditSourceReport(
        ...
        snapshot(
            InstructionsSettingsTab(appModel: appModel, initialReport: report),

...
final class BrokerViewSnapshotTests: XCTestCase {
    ...
    func test_resetLabels_approvedSecondSampleC_lightAndDark() {
        for appearance in [NSAppearance.Name.aqua, .darkAqua] {
            snapshot(
                HStack(alignment: .top, spacing: 8) {
                    QuotaResetLabel(detail: "Fri at 8:30pm\nin 4d 6h 15m").frame(width: 72)
                    QuotaResetLabel(detail: "Today at 6:30pm\nin 6h 15m").frame(width: 72)
                    QuotaResetLabel(detail: "now").frame(width: 72)
                },
                size: CGSize(width: 248, height: 130), appearance: appearance, named: appearance.rawValue
            )
        }
    }

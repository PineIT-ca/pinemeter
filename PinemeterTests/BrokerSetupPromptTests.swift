...
final class BrokerSetupPromptTests: XCTestCase {
    ...
    func test_bothOriginsCarryTheSameContract() {
        let clauses = [
            ...
            "`native` with `agent`",
            "`t3` with `t3-dispatch`",
            "`codex` with `agent` when caller is `codex`",
            "`codex-exec` for every other caller",
            "harness-native subagent tool",
            "`invocation.model` and optional `invocation.effort`",
            ...
            "explicit request for a Fable review",
            "`auto/claude-fable-5-1`",
            "must not substitute another model",
            "Never persist or reuse an override",
    ...
    func test_pasteboardOriginIsWhatTheInstructionsTabCopies() {
        XCTAssertEqual(BrokerWindowView.setupPrompt(port: port), prompt(.pasteboard))
    }
}

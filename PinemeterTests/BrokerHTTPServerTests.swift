...
final class BrokerHTTPServerTests: XCTestCase {
    ...
    func test_initializeListToolsAndPick_completesOverLoopback() async throws {
        ...
        XCTAssertEqual(decision["model"] as? String, "native/claude-opus-5")
    ...
    func test_initialize_carriesTheRoutingContractAsServerInstructions() async throws {
        ...
        XCTAssertTrue(instructions.contains("model-only request"))
        XCTAssertTrue(instructions.contains("choose the healthy account"))
        XCTAssertTrue(
    ...
    func test_pickWithExplicitOverride_returnsAuditableOneCallDecision() async throws {
        ...
        XCTAssertEqual(normalDecision["model"] as? String, "native/claude-opus-5")
    }
    ...
    func test_downPickUpSequence_changesThenRestoresPickAvailability() async throws {
        ...
        XCTAssertEqual(before["model"] as? String, "native/claude-opus-5")

        let down = try await send(jsonRPC: [
            "jsonrpc": "2.0",
            "id": 2,
            "method": "tools/call",
            "params": ["name": "down", "arguments": ["target": "native/claude-opus-5", "minutes": 30]],
        ])
        XCTAssertEqual(down.status, 200)

        let cooled = try await pickPlanning()
        XCTAssertNotEqual(cooled["model"] as? String, "native/claude-opus-5",
        ...
        let up = try await send(jsonRPC: [
            ...
            "id": 3,
            "method": "tools/call",
            "params": ["name": "up", "arguments": ["target": "native/claude-opus-5"]],
        ])
        XCTAssertEqual(up.status, 200)

        let after = try await pickPlanning()
        XCTAssertEqual(after["model"] as? String, "native/claude-opus-5",

...
final class BrokerHTTPServerTests: XCTestCase {
    ...
    func test_initializeListToolsAndPick_completesOverLoopback() async throws {
        ...
        let pickTool = try XCTUnwrap(tools.first { $0["name"] as? String == "pick" })
        let pickDescription = try XCTUnwrap(pickTool["description"] as? String)
        XCTAssertTrue(pickDescription.contains("explicit request for a Fable review"))
        XCTAssertTrue(pickDescription.contains("`auto/claude-fable-5-1`"))
        XCTAssertTrue(pickDescription.contains("must not substitute another model"))
        ...
        XCTAssertEqual(decision["model"] as? String, "native/claude-opus-5-5")
        XCTAssertEqual(decision["agentModel"] as? String, "opus")
        let invocation = try XCTUnwrap(decision["invocation"] as? [String: Any])
        XCTAssertEqual(invocation["kind"] as? String, "agent")
        XCTAssertEqual(invocation["model"] as? String, "opus")
    }
    ...
    func test_initialize_carriesTheRoutingContractAsServerInstructions() async throws {
        ...
        XCTAssertTrue(instructions.contains("`status` tool lists"))
        XCTAssertTrue(instructions.contains("`explore` for read-only codebase scanning"))
        XCTAssertTrue(instructions.contains("`verification` for test runs and plan checks"))
        ...
        XCTAssertTrue(instructions.contains("`codex` with `agent` when caller is `codex`"))
        XCTAssertTrue(instructions.contains("`codex-exec` for every other caller"))
        XCTAssertTrue(instructions.contains("harness-native subagent tool"))
        XCTAssertTrue(instructions.contains("self-contained prompt"))
        ...
        XCTAssertTrue(instructions.contains("`override_candidate`"))
        XCTAssertTrue(instructions.contains("Never infer an override"))
        XCTAssertTrue(instructions.contains("explicit request for a Fable review"))
        XCTAssertTrue(instructions.contains("`auto/claude-fable-5-1`"))
        XCTAssertTrue(instructions.contains("must not substitute another model"))
        XCTAssertTrue(
    ...
    func test_auditGradesSuppliedSourcesAndReturnsTheContract() async throws {
        let complete = """
            ...
            An explicit request for a Fable review passes `auto/claude-fable-5-1` as `override_candidate` \
            and must not substitute another model.
            """
    ...
    func testPickStartedCompletedLifecycleOverLoopback() async throws {
        ...
        XCTAssertEqual(records.first?.terminal?.sourceFiles, 3)
        XCTAssertEqual(records.first?.terminal?.sourceLines, 42)
        XCTAssertEqual(records.first?.terminal?.generatedLines, 100)
        XCTAssertEqual(records.first?.terminal?.docsLines, 8)
        XCTAssertEqual(records.first?.terminal?.trigger, .dependency)
    ...
    func testPickAuditPersistenceFailureReturnsSafeToolError() async throws {
        let sensitiveDetail = "synthetic-sensitive-writer-detail"
        let failingStore = BrokerAuditStore(
            storeDirectory: tempDirectory,
            writer: { _, _ in
                throw NSError(
                    domain: sensitiveDetail,
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: sensitiveDetail]
                )
            }
        )
        try await replaceServer(
            broker: BrokerService(
                auditStore: failingStore,
                idGenerator: { "decision-not-issued" }
            )
        )
        ...
        let content = try XCTUnwrap(result["content"] as? [[String: Any]])
        let text = try XCTUnwrap(content.first?["text"] as? String)
        XCTAssertEqual(text, "Audit persistence failed; no decision issued")
        XCTAssertLessThanOrEqual(text.unicodeScalars.count, 128)
        XCTAssertFalse(text.contains(sensitiveDetail))
        XCTAssertFalse(text.contains(tempDirectory.path))
        let records = await failingStore.recordsSnapshot
    ...
    func testReportRejectsInvalidWireValuesBeforeMutation() async throws {
        ...
        let invalid: [[String: Any]] = [
            ...
            ["decision_id": "d", "status": "started", "source_files": 0],
            ["decision_id": "d", "status": "started", "source_lines": 0],
            ["decision_id": "d", "status": "started", "generated_lines": 0],
            ["decision_id": "d", "status": "started", "docs_lines": 0],
            ["decision_id": "d", "status": "started", "trigger": "uncertain"],
            ["decision_id": "d", "status": "completed", "source_files": -1],
            ["decision_id": "d", "status": "completed", "source_files": 2_147_483_648],
            ["decision_id": "d", "status": "completed", "source_lines": -1],
            ["decision_id": "d", "status": "completed", "source_lines": 2_147_483_648],
            ["decision_id": "d", "status": "completed", "generated_lines": -1],
            ["decision_id": "d", "status": "completed", "generated_lines": 2_147_483_648],
            ["decision_id": "d", "status": "completed", "docs_lines": -1],
            ["decision_id": "d", "status": "completed", "docs_lines": 2_147_483_648],
            ["decision_id": "d", "status": "completed", "source_files": "synthetic-secret-value"],
            ["decision_id": "d", "status": "completed", "trigger": "other"],
    ...
    func testReportAcceptsBoundedScalarsAndSanitizesFailureReason() async throws {
        ...
        for status in ["completed", "failed", "exhausted"] {
            var arguments: [String: Any] = [
                ...
                "source_files": maxTokens,
                "source_lines": maxTokens,
                "generated_lines": maxTokens,
                "docs_lines": maxTokens,
                "trigger": "uncertain",
            ]
            if status != "completed" {
        ...
        for report in reports.dropFirst() {
            XCTAssertEqual(report.sourceFiles, maxTokens)
            XCTAssertEqual(report.sourceLines, maxTokens)
            XCTAssertEqual(report.generatedLines, maxTokens)
            XCTAssertEqual(report.docsLines, maxTokens)
            XCTAssertEqual(report.trigger, .uncertain)
        }
    ...
    func test_pickWithoutCallerArgument_echoesTheDefaultCaller() async throws {
        ...
        // `t3/gpt-6-sol` lane fails CLOSED and execution falls to codex.
        //
    ...
    func test_pickWithExplicitOverride_returnsAuditableOneCallDecision() async throws {
        await configureDetectedCodexInstance()
        await broker.updateT3Liveness(["codex": T3Liveness(reachable: true, why: "http 200")])
        ...
        let pick = try await send(jsonRPC: [
            ...
            "params": [
                ...
                "arguments": [
                    "role": "planning",
                    "caller": "codex",
                    "override_candidate": "t3:codex/gpt-6-sol",
                ],
            ],
        ])

        let decision = try decodeDecision(from: pick)
        XCTAssertEqual(decision["source"] as? String, "human-override")
        XCTAssertEqual(decision["model"] as? String, "t3:codex/gpt-6-sol")
        XCTAssertEqual(decision["route"] as? String, "t3")
        let invocation = try XCTUnwrap(decision["invocation"] as? [String: Any])
        XCTAssertEqual(invocation["kind"] as? String, "t3-dispatch")
        XCTAssertEqual(invocation["instanceId"] as? String, "codex")
        XCTAssertEqual((decision["backups"] as? [Any])?.count, 0)

        let normal = try await send(jsonRPC: [
            "jsonrpc": "2.0",
            "id": 2,
            "method": "tools/call",
            "params": [
        ...
        let normalDecision = try decodeDecision(from: normal)
        XCTAssertEqual(normalDecision["source"] as? String, "policy")
        XCTAssertEqual(normalDecision["model"] as? String, "native/claude-opus-5-5")
    }

    func test_pickWithUnavailableOverride_returnsToolError() async throws {
        await configureDetectedCodexInstance()
        let pick = try await send(jsonRPC: [
            "jsonrpc": "2.0",
            "id": 1,
        ...
        XCTAssertEqual(result["isError"] as? Bool, true)
        let content = try XCTUnwrap(result["content"] as? [[String: Any]])
        let error = try XCTUnwrap(content.first?["text"] as? String)
        XCTAssertTrue(error.contains("unavailable after automatic refresh"), error)
        XCTAssertTrue(error.contains("Open T3 Code, then retry"), error)
    }
    ...
    func test_downPickUpSequence_changesThenRestoresPickAvailability() async throws {
        ...
        XCTAssertEqual(before["model"] as? String, "native/claude-opus-5-5")

        let down = try await send(jsonRPC: [
            "jsonrpc": "2.0",
            "id": 2,
            "method": "tools/call",
            "params": ["name": "down", "arguments": ["target": "native/claude-opus-5-5", "minutes": 30]],
        ])
        XCTAssertEqual(down.status, 200)

        let cooled = try await pickPlanning()
        XCTAssertNotEqual(cooled["model"] as? String, "native/claude-opus-5-5",
        ...
        let up = try await send(jsonRPC: [
            ...
            "id": 3,
            "method": "tools/call",
            "params": ["name": "up", "arguments": ["target": "native/claude-opus-5-5"]],
        ])
        XCTAssertEqual(up.status, 200)

        let after = try await pickPlanning()
        XCTAssertEqual(after["model"] as? String, "native/claude-opus-5-5",
    ...
    func test_caseVariantDuplicateContentLength_returns400() async throws {
        let server = LoopbackHTTPServer(port: 0) { _ in
            LoopbackRequestHandler(handle: { _ in .ok() })
        }
        let duplicatePort = try await server.start()
        defer { Task { await server.stop() } }

        let connection = try await openRawConnection(port: duplicatePort)
        defer { connection.cancel() }
        try await sendRaw(
            "POST /mcp HTTP/1.1\r\nContent-Length: 0\r\ncontent-length: 0\r\n\r\n",
            on: connection
        ...
        let response = try await receiveRaw(on: connection, timeout: 2) ?? Data()
    ...
    func test_excessSlowConnectionIsRefused_whileAdmittedConnectionsFinish() async throws {
        let admitted = expectation(description: "32 connections admitted")
        admitted.expectedFulfillmentCount = 32
        admitted.assertForOverFulfill = false
        let server = LoopbackHTTPServer(
            port: 0,
            readTimeout: 5,
            authorize: { _, _ in
        ...
        let boundedPort = try await server.start()
        ...
        for _ in 0..<32 {
            let connection = try await openRawConnection(port: boundedPort)
            connections.append(connection)
            try await sendRaw("POST /mcp HTTP/1.1\r\nContent-Length: 1\r\n\r\n", on: connection)
        }
        await fulfillment(of: [admitted], timeout: 2)

        let excess = try await openRawConnection(port: boundedPort)
        defer { excess.cancel() }
        try? await sendRaw("GET /mcp HTTP/1.1\r\n\r\n", on: excess)
        switch await receiveRawOutcome(on: excess, timeout: 1) {
        case .closed:
            break
        case .data(let data):
            XCTFail("excess connection received data: \(String(decoding: data, as: UTF8.self).prefix(80))")
        case .timedOut:
            XCTFail("excess connection remained open")
        }

        for connection in connections {
            try await sendRaw("x", on: connection)
            let response = try await receiveRaw(on: connection, timeout: 2) ?? Data()
    ...
    func test_unauthorizedRequestDoesNotRequireItsLargeBodyToBeConsumed() async throws {
        let server = LoopbackHTTPServer(
            port: 0,
            readTimeout: 0.5,
            authorize: { _, _ in false }
        ) { _ in
            LoopbackRequestHandler(handle: { _ in .ok() })
        }
        let rejectingPort = try await server.start()
        defer { Task { await server.stop() } }

        let connection = try await openRawConnection(port: rejectingPort)
        defer { connection.cancel() }
        try await sendRaw(
        ...
        let response = try await receiveRaw(on: connection, timeout: 1) ?? Data()
        XCTAssertTrue(String(decoding: response, as: UTF8.self).hasPrefix("HTTP/1.1 401"))
    }
    ...
    private func configureDetectedCodexInstance() async {
        var policy = BrokerPolicy.default
        guard let index = policy.t3Instances.firstIndex(where: { $0.id == "codex" }) else { return }
        policy.t3Instances[index].detectedModels = ["gpt-6-sol"]
        await broker.updatePolicy(policy)
    }
    ...
    private func receiveRawOutcome(on connection: NWConnection, timeout: TimeInterval) async -> RawReceiveOutcome {

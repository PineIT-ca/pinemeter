...
final class CacheRepositoryTests: XCTestCase {
    ...
    func test_setWritesPrivateCacheWithoutReplacingPublicAggregateExport() async throws {
    ...
    func test_invalidateRemovesOnlyPrivateCacheArtifacts() async throws {
        let repository = makeRepository()
    ...
    func test_publicExportUsesPrivateFileModesAfterEveryWrite() async throws {
        let publicDirectory = newPublicExportURL.deletingLastPathComponent()
        ...
        await repository.writeAggregateSnapshot(makeAggregateSnapshot())

        XCTAssertEqual(try permissions(at: publicDirectory), 0o700)
        XCTAssertEqual(try permissions(at: newPublicExportURL), 0o600)

        try FileManager.default.setAttributes([.posixPermissions: 0o644], ofItemAtPath: newPublicExportURL.path)
        await repository.writeAggregateSnapshot(makeAggregateSnapshot())
    ...
    private func makeAggregateSnapshot() -> AggregateQuotaSnapshot {

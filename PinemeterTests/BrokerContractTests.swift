...
final class BrokerContractTests: XCTestCase {
    ...
    func test_seededPolicy_carriesTheSurvivingRoutesAndT3Wiring() {
        ...
        XCTAssertEqual(policy.roles["research"]?.map(\.effort), [nil, .high, .high, .high])
        XCTAssertEqual(policy.roles["review"]?.map(\.effort), [.high, .low, nil, .high])
        XCTAssertEqual(policy.roles["execution"]?.map(\.effort), [.high, .high, nil])
        XCTAssertEqual(policy.roles["heavy"]?.map(\.effort), [.medium, .xhigh, .xhigh, .xhigh])
        XCTAssertEqual(policy.roles["standard"]?.map(\.effort), [.high, .medium, .medium, nil])
        XCTAssertEqual(policy.roles["verification"]?.map(\.effort), [.high, .high, nil])
    }

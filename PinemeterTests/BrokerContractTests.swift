...
final class BrokerContractTests: XCTestCase {
    ...
    func test_seededPolicy_carriesTheSurvivingRoutesAndT3Wiring() {
        ...
        XCTAssertEqual(planning?.first?.id, "auto/claude-opus-5")
        ...
        XCTAssertEqual(policy.roles["planning"]?.map(\.effort), [.xhigh, nil, .xhigh])
        XCTAssertEqual(policy.roles["explore"]?.map(\.effort), [nil, .medium, .low])
        XCTAssertEqual(policy.roles["research"]?.map(\.effort), [nil, .high, .high])
        XCTAssertEqual(policy.roles["review"]?.map(\.effort), [.high, nil, .high])
        XCTAssertEqual(policy.roles["execution"]?.map(\.effort), [.high, .high, nil])
        XCTAssertEqual(policy.roles["heavy"]?.map(\.effort), [.xhigh, .xhigh, .xhigh])

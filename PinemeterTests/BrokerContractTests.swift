...
final class BrokerContractTests: XCTestCase {
    ...
    func test_seededPolicy_carriesTheSurvivingRoutesAndT3Wiring() {
        ...
        XCTAssertEqual(planning?.first?.id, "auto/claude-fable-5-1")
        ...
        XCTAssertEqual(policy.roles["planning"]?.map(\.effort), [nil, .xhigh, .xhigh])
        XCTAssertEqual(policy.roles["execution"]?.map(\.effort), [.high, .high, nil])
        XCTAssertEqual(policy.roles["heavy"]?.map(\.effort), [.xhigh, .high, .high])
        XCTAssertEqual(policy.roles["standard"]?.map(\.effort), [.high, .medium, nil])
    }
    ...
    func test_decide_acceptsTheFrozenWorldInputsAndEchoesTheCaller() throws {
        ...
        XCTAssertEqual(decision.model, "native/claude-fable-5-1")

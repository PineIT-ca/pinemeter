...
final class BrokerContractTests: XCTestCase {
    ...
    func test_seededPolicy_carriesTheSurvivingRoutesAndT3Wiring() {
        ...
        XCTAssertEqual(
            policy.roles["planning"]?.map(\.id).dropFirst().first, "t3:*/claude-fable-5",
            "the second Claude lane ships as `any instance`, not as a machine-specific id"
        )
        XCTAssertTrue(
    ...
    func test_usageLane_fallsBackToTheInstanceBindingWhenNoLaneRowNamesTheCandidate() throws {
        ...
        XCTAssertNil(policy.usageLane(for: unbound))
        ...
        let unqualified = try XCTUnwrap(BrokerCandidate(id: "t3/claude-fable-5"))
        XCTAssertEqual(

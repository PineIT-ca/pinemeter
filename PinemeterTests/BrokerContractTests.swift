...
final class BrokerContractTests: XCTestCase {
    ...
    func test_portableClaudeQuota_doesNotBindAnUnknownT3Account() {
        var policy = BrokerPolicy.default
        policy.t3Instances = [T3InstanceConfig(id: "secondary", name: "Secondary")]
        policy.models["claude-future"] = BrokerModelSpec(
            quota: .claudeAccount(accountId: nil, labelContains: nil, isPrimary: true)
        )
        let candidate = BrokerCandidate(route: .t3, instance: "secondary", model: "claude-future")

        XCTAssertNil(policy.usageLane(for: candidate))
        policy.t3Instances[0].boundAccountId = "local-account"
        XCTAssertEqual(
            policy.usageLane(for: candidate),
            .claudeAccount(accountId: "local-account", labelContains: nil, isPrimary: nil)
        )
    }
    ...
    func test_fullyPopulatedDecision_encodesExactlyTheContractKeys() throws {
        ...
        XCTAssertEqual(
            ...
            [
                ...
                "decision_id", "backups", "degraded_reason", "retryable",
            ],
        ...
        XCTAssertEqual(json["degraded_reason"] as? String, Self.fullyPopulatedDecision.reason)
        XCTAssertEqual(json["retryable"] as? Bool, false)
        XCTAssertNil(json["suggested_action"])
        XCTAssertEqual(json["effort"] as? String, "high")
        let backups = try XCTUnwrap(json["backups"] as? [[String: Any]])
        XCTAssertEqual(backups.count, 1)
        XCTAssertEqual(backups[0]["candidate"] as? String, "native/claude-fable-5")
    }
    ...
    func test_decisionWithoutAnEffort_omitsTheKeyEntirely() throws {
        ...
        XCTAssertEqual(
            ...
            [
                "role", "caller", "model", "route", "agentModel", "invocation",
                "reason", "source", "oracle", "degraded", "candidatesTried", "backups",
            ],
        ...
        let emptyBackups = try XCTUnwrap(json["backups"] as? [Any], "backups stays present even when empty")
        XCTAssertTrue(emptyBackups.isEmpty, "backups stays present even when empty")
        XCTAssertNil(json["degraded_reason"])
        XCTAssertNil(json["retryable"])
        XCTAssertNil(json["suggested_action"])
    }

    func test_retryableDegradedDecision_encodesRecoveryAction() throws {
        let decision = BrokerDecision(
            role: "execution", caller: "codex", model: "t3/gpt-5.6-sol", route: .t3,
            agentModel: nil, invocation: .t3Dispatch(model: "gpt-5.6-sol", instanceId: "codex"),
            reason: "t3 lane oracle has no fresh data; t3 reachable, failing open", source: .policy,
            oracle: .absent, degraded: true, retryable: true, candidatesTried: []
        )
        let json = try encodeToObject(decision)
    ...
    // MARK: - Test: backups wire shape and decode safety

    func test_backupOption_encodesExactlyTheContractKeysAndOmitsAnAbsentEffort() throws {
        let withEffort = try encodeToObject(
            BrokerBackupOption(
                candidate: "t3/gpt-5.6-sol",
                route: .t3,
                model: "gpt-5.6-sol",
                effort: .medium,
                invocation: .t3Dispatch(model: "gpt-5.6-sol", instanceId: "codex", effort: .medium),
                why: "next in chain"
            )
        )
        XCTAssertEqual(
            Set(withEffort.keys),
            ["candidate", "route", "model", "effort", "invocation", "why"]
        )
        XCTAssertEqual(withEffort["candidate"] as? String, "t3/gpt-5.6-sol")
        XCTAssertEqual(withEffort["route"] as? String, "t3")
        XCTAssertEqual(withEffort["why"] as? String, "next in chain")

        let withoutEffort = try encodeToObject(
            BrokerBackupOption(
                candidate: "native/claude-sonnet-5",
                route: .native,
                model: "claude-sonnet-5",
                invocation: .agent(model: "sonnet"),
                why: "headroom unverified: oracle missing, failing open to native claude"
            )
        )
        XCTAssertEqual(
            Set(withoutEffort.keys),
            ["candidate", "route", "model", "invocation", "why"],
            "an absent effort must be omitted, not encoded as null"
        )
    }

    func test_decisionBackups_decodeSafeWhenTheKeyIsAbsent() throws {
        // Legacy decision JSON, from a build that predates backups entirely.
        let data = try BrokerDecision.makeEncoder().encode(Self.effortlessDecision)
        var object = try XCTUnwrap(JSONSerialization.jsonObject(with: data) as? [String: Any])
        object.removeValue(forKey: "backups")
        ...
        XCTAssertEqual(decoded.backups, [])
    }

    func test_decisionBackups_capAtTwoOnConstructionAndOnDecode() throws {
        func option(_ n: Int) -> BrokerBackupOption {
            BrokerBackupOption(
                candidate: "native/model-\(n)",
                route: .native,
                model: "model-\(n)",
                invocation: .agent(model: "model-\(n)"),
                why: "next in chain"
            )
        }
        let overfilled = Self.effortlessDecision
        let constructed = BrokerDecision(
            role: overfilled.role,
            caller: overfilled.caller,
            model: overfilled.model,
            route: overfilled.route,
            agentModel: overfilled.agentModel,
            invocation: overfilled.invocation,
            reason: overfilled.reason,
            source: overfilled.source,
            oracle: overfilled.oracle,
            degraded: overfilled.degraded,
            candidatesTried: overfilled.candidatesTried,
            backups: [option(1), option(2), option(3)]
        )
        XCTAssertEqual(constructed.backups.map(\.candidate), ["native/model-1", "native/model-2"])

        let data = try BrokerDecision.makeEncoder().encode(constructed)
        var object = try XCTUnwrap(JSONSerialization.jsonObject(with: data) as? [String: Any])
        var rawBackups = try XCTUnwrap(object["backups"] as? [[String: Any]])
        rawBackups.append([
            "candidate": "native/model-4", "route": "native", "model": "model-4",
            "invocation": ["kind": "agent", "model": "model-4"], "why": "next in chain",
        ])
        object["backups"] = rawBackups
        let overstuffedData = try JSONSerialization.data(withJSONObject: object)

        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        let decoded = try decoder.decode(BrokerDecision.self, from: overstuffedData)
        XCTAssertEqual(decoded.backups.count, 2, "decode must enforce the same cap as construction")
    }
    ...
    func test_decisionSource_encodesStableSpellings() throws {
        XCTAssertEqual(BrokerDecisionSource.policy.rawValue, "policy")
        XCTAssertEqual(BrokerDecisionSource.humanOverride.rawValue, "human-override")
    ...
    func test_effort_hasExactlyTheFourLevelsAndNoNoneCase() {
        XCTAssertEqual(BrokerEffort.builtInLevels, [.low, .medium, .high, .xhigh])
        XCTAssertEqual(BrokerEffort.builtInLevels.map(\.rawValue), ["low", "medium", "high", "xhigh"])
        XCTAssertNil(
            BrokerEffort(rawValue: "none"),
            "absence is expressed by a nil effort, never by a 'none' level"
        )
        XCTAssertEqual(BrokerEffort(rawValue: "ultra")?.displayLabel, "Ultra")
        XCTAssertNil(BrokerEffort(rawValue: "Ultra High"))
    }
    ...
    func test_unknownEffortString_failsDecodeLoudly() {
        ...
        let json = #"[{ "id": "native/claude-fable-5", "effort": "Ultra High" }]"#
        XCTAssertThrowsError(
            try JSONDecoder().decode([BrokerCandidate].self, from: Data(json.utf8))
        )
    }

    func test_futureEffortString_decodesAndRoundTrips() throws {
        let json = #"[{ "id": "codex/gpt-future", "effort": "ultra" }]"#
        let decoded = try JSONDecoder().decode([BrokerCandidate].self, from: Data(json.utf8))

        XCTAssertEqual(decoded.first?.effort, BrokerEffort(rawValue: "ultra"))
        XCTAssertEqual(
            try JSONDecoder().decode([BrokerCandidate].self, from: JSONEncoder().encode(decoded)),
            decoded
        )
    }
    ...
    func test_policyWithNoThresholdsBlockAtAll_usesEverySeededDefault() throws {
        ...
        XCTAssertEqual(policy.thresholds.fableWeeklyPct, 80)
    ...
    func test_policyWithNoCallersOrLanes_decodesToTheSeededDefaults() throws {
        ...
        XCTAssertEqual(policy.agentModelAliases["claude-fable-5"], "fable")
        XCTAssertEqual(policy.agentModelAliases["claude-fable-5-1"], "fable")
    }
    ...
    func test_resolveRole_prefersAnExplicitChainThenAnAvailableAliasTarget() {
        var policy = BrokerPolicy(roles: [
            "architecture": [BrokerCandidate(route: .native, model: "explicit")],
            "planning": [BrokerCandidate(route: .native, model: "aliased")],
        ])

        XCTAssertEqual(policy.resolveRole("architecture"), "architecture")
        policy.roles.removeValue(forKey: "architecture")
        XCTAssertEqual(policy.resolveRole("architecture"), "planning")
        policy.roles.removeValue(forKey: "planning")
        XCTAssertEqual(policy.resolveRole("architecture"), "architecture")
    }
    ...
    func test_seededPolicy_carriesTheSurvivingRoutesAndT3Wiring() {
        ...
        XCTAssertEqual(planning?.first?.id, "auto/claude-opus-5-5")
        XCTAssertEqual(policy.callers[BrokerPolicy.defaultCaller]?.routes, [.native, .codex, .t3])
        XCTAssertEqual(policy.callers["codex"]?.routes, [.codex, .t3])
        XCTAssertEqual(policy.t3.defaultInstance, "claudeAgent")
        XCTAssertEqual(policy.t3.instanceByModel["gpt-5.6-sol"], "codex")
        XCTAssertEqual(policy.t3.instanceByModel["gpt-5.6-terra"], "codex")
        XCTAssertEqual(policy.t3.instanceByModel["gpt-5.6-luna"], "codex")
        XCTAssertEqual(policy.t3.instanceByModel["gpt-6-astra"], "codex")
        XCTAssertEqual(policy.t3.instanceByModel["gpt-6-sol"], "codex")
        XCTAssertEqual(policy.t3.instanceByModel["gpt-6-luna"], "codex")
        XCTAssertEqual(policy.t3Instances.map(\.id), ["claudeAgent", "codex"])
        XCTAssertTrue(policy.roles.values.flatMap { $0 }.allSatisfy { $0.route == .auto })
        XCTAssertTrue(
        ...
        XCTAssertEqual(policy.roles["planning"]?.map(\.effort), [.xhigh, .low, nil, .xhigh])
        XCTAssertEqual(policy.roles["explore"]?.map(\.effort), [nil, .medium, .low])
        XCTAssertEqual(policy.roles["research"]?.map(\.effort), [nil, .high, .medium, .high])
        XCTAssertEqual(policy.roles["review"]?.map(\.effort), [.high, .low, nil, .high])
        XCTAssertEqual(policy.roles["execution"]?.map(\.effort), [.medium, .high, nil])
        XCTAssertEqual(policy.roles["heavy"]?.map(\.effort), [.medium, .xhigh, .xhigh, .high])
        XCTAssertEqual(policy.roles["standard"]?.map(\.effort), [.high, .low, nil])
        XCTAssertEqual(policy.roles["verification"]?.map(\.effort), [.high, .medium, nil])
    }
    ...
    func test_usageLane_fallsBackToTheInstanceBindingWhenNoLaneRowNamesTheCandidate() throws {
        var policy = BrokerPolicy(
            roles: [:],
            t3Instances: [
                T3InstanceConfig(id: "alpha", name: "Alpha", boundAccountId: "acct-a"),
                T3InstanceConfig(id: "beta", name: "Beta"),
            ],
            usageLanes: [
                "t3:alpha/claude-opus-5": .claudeAccount(
                    accountId: "acct-explicit", labelContains: nil, isPrimary: nil
                ),
        ...
        let bound = try XCTUnwrap(BrokerCandidate(id: "t3:alpha/claude-fable-5"))
        XCTAssertEqual(
            policy.usageLane(for: bound),
            .claudeAccount(accountId: "acct-a", labelContains: nil, isPrimary: nil),
            "a bound instance declares whose quota its lanes spend"
        )

        let explicit = try XCTUnwrap(BrokerCandidate(id: "t3:alpha/claude-opus-5"))
        XCTAssertEqual(
            policy.usageLane(for: explicit),
            .claudeAccount(accountId: "acct-explicit", labelContains: nil, isPrimary: nil),
            "an explicit lane row still wins over the binding"
        )

        let unbound = try XCTUnwrap(BrokerCandidate(id: "t3:beta/claude-fable-5"))
        XCTAssertNil(
            policy.usageLane(for: unbound),
            "an unbound Claude instance must not inherit another account's generic lane"
        )

        let sharedChatGPT = try XCTUnwrap(BrokerCandidate(id: "t3:beta/gpt-5.6-sol"))
        XCTAssertEqual(
            policy.usageLane(for: sharedChatGPT),
            .chatGPT(labelContains: "codex weekly")
        )

        let native = try XCTUnwrap(BrokerCandidate(id: "native/claude-fable-5"))
        XCTAssertNil(policy.usageLane(for: native), "the fallback is t3-only")

        policy.t3.defaultInstance = "alpha"
        let unqualified = try XCTUnwrap(BrokerCandidate(id: "t3/claude-sonnet-5"))
        XCTAssertEqual(
            policy.usageLane(for: unqualified),
            .claudeAccount(accountId: "acct-a", labelContains: nil, isPrimary: nil),
            "resolution runs first, so an unqualified candidate inherits its resolved binding"
        )
    }

    func test_instancesServing_keepsUnscannedRowsAndDropsRowsThatListOtherModels() {
        let policy = BrokerPolicy(
            roles: [:],
            t3Instances: [
                T3InstanceConfig(id: "alpha", name: "Alpha", detectedModels: ["claude-fable-5"]),
                T3InstanceConfig(id: "beta", name: "Beta", detectedModels: ["gpt-5.6-sol"]),
                T3InstanceConfig(id: "gamma", name: "Gamma"),
            ]
        )

        XCTAssertEqual(policy.instances(serving: "claude-fable-5").map(\.id), ["alpha", "gamma"])
    }
    ...
    func test_decide_acceptsTheFrozenWorldInputsAndEchoesTheCaller() throws {
        ...
        XCTAssertEqual(decision.agentModel, "opus")
    }

...
final class BrokerAppModelTests: XCTestCase {
    ...
    func test_bootstrap_forwardsControllerUIStateToAppModel() async throws {
        let fakeBroker = FakeBrokerService()
        let appModel = AppModel(
        ...
        await appModel.bootstrap()
        await fakeBroker.updateServerState(.failed(message: "forwarded"))
        for _ in 0..<100 {
            if appModel.brokerUIState?.serverState == .failed(message: "forwarded") { break }
            await Task.yield()
        }

        XCTAssertEqual(appModel.brokerUIState?.serverState, .failed(message: "forwarded"))
    }
    ...
    func test_controllerOwnsBrokerLifecycleTransitions() async throws {
        let fakeBroker = FakeBrokerService()
        let factory = RecordingBrokerLoopbackServerFactory()
        let controller = BrokerLifecycleController(
            brokerService: fakeBroker,
            keychainRepository: KeychainRepositoryFake(),
            serverFactory: { _, port, policy in factory.make(port: port, policy: policy) }
        )
        var settings = BrokerSettings.default
        settings.isEnabled = true

        let started = await controller.apply(settings)
        settings.port = 50000
        let restarted = await controller.apply(settings)
        settings.isEnabled = false
        let stopped = await controller.apply(settings)

        let firstStopCount = await factory.servers[0].stopCallCount
        let secondStopCount = await factory.servers[1].stopCallCount
        let lastServerState = await fakeBroker.serverStates.last

        XCTAssertTrue(started)
        XCTAssertTrue(restarted)
        XCTAssertTrue(stopped)
        XCTAssertEqual(factory.ports, [43117, 50000])
        XCTAssertEqual(firstStopCount, 1)
        XCTAssertEqual(secondStopCount, 1)
        XCTAssertEqual(lastServerState, .stopped)
    }
    ...
    // MARK: - Routing update notifications

    private func routingSettings(lastChecked: Bool = true) -> AppSettings {
        var settings = AppSettings.default
        settings.broker.isEnabled = true
        settings.hasNotificationsEnabled = true
        settings.broker.applyProfile(id: BrokerAgentProfile.balancedID)
        settings.broker.seenRemotePresetIDs = lastChecked
            ? Set(BrokerAgentProfile.builtIns.map(\.id))
            : []
        settings.broker.presetManifest.sources = [BrokerPresetManifestSource(
            urlString: "https://example.com/presets.json",
            lastCheckedAt: lastChecked ? Date(timeIntervalSince1970: 1_760_000_000) : nil
        )]
        return settings
    }

    private func updatedBalanced(model: String) -> BrokerAgentProfile {
        var rules = BrokerAgentProfile.balanced.rules
    ...
    private func makeRoutingAppModel(
        settings: AppSettings,
        manifests: [BrokerPresetManifest],
        notifications: NotificationServiceSpy,
        broker: FakeBrokerService = FakeBrokerService(),
        historyStore: BrokerManifestHistoryStore? = nil
    ) -> AppModel {
        let model = AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
            ...
            notificationService: notifications,
            presetManifestService: SequentialPresetManifestService(manifests: manifests),
            presetManifestHistoryStore: historyStore,
            brokerService: broker,
            brokerServerFactory: { _, _, _ in FakeBrokerLoopbackServer() },
            t3InstanceDiscovery: T3InstanceDiscoveryFake()
        )
    ...
    func test_manifestHistoryChangedFetchPublishesDurableStateWithoutMutatingPolicy() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("BrokerAppModelTests-history-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let historyStore = BrokerManifestHistoryStore(storeDirectory: directory)
        let settings = routingSettings()
        let source = try XCTUnwrap(settings.broker.presetManifest.sources.first)
        let policyBefore = settings.broker.policy
        let manifest = BrokerPresetManifest(
            schemaVersion: 1,
            presets: [updatedBalanced(model: "history-model")],
            manifestRevision: 14,
            publishedAt: Date(timeIntervalSince1970: 1_760_000_000),
            summary: "Safer mixed-provider fallback."
        )
        let model = makeRoutingAppModel(
            settings: settings,
            manifests: [manifest],
            notifications: NotificationServiceSpy(),
        ...
        await model.refreshPresetManifest(force: true)

        guard case .loaded(let revisions) = model.manifestHistoryState(for: source) else {
            return XCTFail("Expected loaded manifest history")
        }
        XCTAssertEqual(revisions.map(\.manifestRevision), [14])
        XCTAssertEqual(model.settings.broker.policy, policyBefore, "fetching history must never apply rules")

        let restarted = BrokerManifestHistoryStore(storeDirectory: directory)
        guard case .loaded(let persisted) = await restarted.load(
            sourceID: source.id,
            urlString: source.urlString
        ) else {
            return XCTFail("Expected persisted history after restart")
        }
        XCTAssertEqual(persisted.map(\.manifestRevision), [14])
    }

    func test_manifestHistorySourceURLChangeDuringFetchCannotPublishOldURLRows() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("BrokerAppModelTests-url-race-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let service = SuspendedPresetManifestService()
        let historyStore = BrokerManifestHistoryStore(storeDirectory: directory)
        var settings = routingSettings()
        let source = try XCTUnwrap(settings.broker.presetManifest.sources.first)
        settings.broker.presetManifest.sources[0].urlString = "https://example.com/old.json"
        let model = AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
            usageService: UsageServiceStub(fetchUsageResult: .failure(TestError(message: "not used"))),
        ...
        let refresh = Task { await model.refreshPresetManifest(force: true) }
        await service.waitUntilFetchStarts()
        model.settings.broker.presetManifest.sources[0].urlString = "https://example.com/new.json"
        model.presetManifestURLChanged(sourceID: source.id)
        await service.resume(with: BrokerPresetManifest(
            schemaVersion: 1,
            presets: [updatedBalanced(model: "stale-response")],
            manifestRevision: 1
        ))
        await refresh.value

        let oldURLHistory = await historyStore.load(
            sourceID: source.id,
            urlString: "https://example.com/old.json"
        )
        XCTAssertTrue(oldURLHistory.revisions.isEmpty)
        XCTAssertTrue(model.settings.broker.remotePresets.isEmpty)
    }

    func test_manifestHistoryRemovalDuringStoreWriteCannotRepublishRemovedSource() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("BrokerAppModelTests-removal-race-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let settings = routingSettings()
        let source = try XCTUnwrap(settings.broker.presetManifest.sources.first)
        let initialStore = BrokerManifestHistoryStore(storeDirectory: directory)
        _ = try await initialStore.seedIfNeeded(
            sourceID: source.id,
            urlString: source.urlString,
            presets: [],
            agentSetup: nil,
        ...
        let writer = BlockingHistoryWriter()
        let historyStore = BrokerManifestHistoryStore(
        ...
        let model = makeRoutingAppModel(
            ...
            manifests: [BrokerPresetManifest(
                schemaVersion: 1,
                presets: [updatedBalanced(model: "removed-source")],
                manifestRevision: 1
            )],
            notifications: NotificationServiceSpy(),
            historyStore: historyStore
        ...
        model.removePresetManifestSource(id: source.id)
        writer.resume()
        await refresh.value

        XCTAssertTrue(model.settings.broker.presetManifest.sources.isEmpty)
        XCTAssertFalse(model.presetManifestHistory.keys.contains { $0.sourceID == source.id })
    }

    func test_routingUpdate_activeBuiltInNotifiesOnceThenRearmsAfterApplyAndChange() async {
        let first = updatedBalanced(model: "first-model")
        let second = updatedBalanced(model: "second-model")
        let notifications = NotificationServiceSpy()
        let fakeBroker = FakeBrokerService()
        let model = makeRoutingAppModel(
            settings: routingSettings(),
            manifests: [
                BrokerPresetManifest(schemaVersion: 1, presets: [first]),
                BrokerPresetManifest(schemaVersion: 1, presets: [first]),
                BrokerPresetManifest(schemaVersion: 1, presets: [second]),
            ],
        ...
        XCTAssertEqual(notifications.sentRoutingUpdates.count, 1)
        XCTAssertEqual(notifications.sentRoutingUpdates.first?.profileName, "Balanced")
        model.applyRoutingUpdate(profileID: BrokerAgentProfile.balancedID)
        XCTAssertFalse(model.settings.broker.activeProfileHasUpdatedRules)
        // Apply fires with Settings closed, so it must push the policy to the
        // running broker itself rather than rely on the Broker window.
        await model.routingUpdateApplyTask?.value
        let pushed = await fakeBroker.updatedPolicies.last
        XCTAssertEqual(pushed?.roles["planning"]?.first?.model, "first-model")

        await model.refreshPresetManifest(force: true)

        XCTAssertEqual(notifications.sentRoutingUpdates.count, 2)
        XCTAssertEqual(model.settings.broker.policy.roles["planning"]?.first?.model, "first-model")
    }

    func test_routingUpdate_updatedProfileWithNewPresetNotifiesOnceAcrossRepeatRefreshes() async {
        let updated = updatedBalanced(model: "published-model")
        let preset = BrokerAgentProfile(
            id: UUID(), name: "New", detail: "", symbolName: "leaf", rules: .default
        )
        let manifest = BrokerPresetManifest(schemaVersion: 1, presets: [updated, preset])
        let notifications = NotificationServiceSpy()
        let model = makeRoutingAppModel(
            settings: routingSettings(),
        ...
        // The first refresh carries both signals in one notification. The
        // repeats carry the same update with no new presets, which is not
        // news, so they must not notify again.
        XCTAssertEqual(notifications.sentRoutingUpdates.count, 1)
        XCTAssertEqual(notifications.sentRoutingUpdates.first?.profileName, "Balanced")
        XCTAssertEqual(notifications.sentRoutingUpdates.first?.newPresetCount, 1)
        XCTAssertEqual(
            model.settings.broker.seenRemotePresetIDs,
            Set(model.settings.broker.remotePresets.map(\.id)),
            "the seen-set tracks exactly what the manifests still publish"
        )
    }

    func test_routingUpdate_userProfilePinNeverNotifies() async {
        var settings = routingSettings()
        let profileID = settings.broker.createProfile(named: "Mine")
        let index = settings.broker.profiles.firstIndex { $0.id == profileID }!
        settings.broker.profiles[index].rules.roles["planning"] = [
            BrokerCandidate(route: .auto, model: "changed-elsewhere"),
        ]
        XCTAssertTrue(settings.broker.activeProfileHasUpdatedRules)
        let notifications = NotificationServiceSpy()
        let model = makeRoutingAppModel(
            settings: settings,
            manifests: [BrokerPresetManifest(schemaVersion: 1, presets: [])],
            notifications: notifications
        )

        await model.refreshPresetManifest(force: true)

        XCTAssertTrue(notifications.sentRoutingUpdates.isEmpty)
    }

    func test_routingUpdate_newPresetNotifiesThenBecomesSeenAndFirstRunSeedsSilently() async {
        let preset = BrokerAgentProfile(
            id: UUID(), name: "New", detail: "", symbolName: "leaf", rules: .default
        )
        let manifest = BrokerPresetManifest(schemaVersion: 1, presets: [preset])
        let notifications = NotificationServiceSpy()
        let model = makeRoutingAppModel(
            settings: routingSettings(),
        ...
        XCTAssertEqual(notifications.sentRoutingUpdates.map(\.newPresetCount), [1])
        XCTAssertTrue(model.settings.broker.seenRemotePresetIDs.contains(preset.id))

        let firstRunNotifications = NotificationServiceSpy()
        let firstRun = makeRoutingAppModel(
            settings: routingSettings(lastChecked: false),
            manifests: [manifest],
            notifications: firstRunNotifications
        )
        await firstRun.refreshPresetManifest(force: true)
        XCTAssertTrue(firstRunNotifications.sentRoutingUpdates.isEmpty)
        XCTAssertTrue(firstRun.settings.broker.seenRemotePresetIDs.contains(preset.id))
    }

    func test_routingUpdate_respectsEveryGate() async {
        let manifest = BrokerPresetManifest(
            schemaVersion: 1,
            presets: [updatedBalanced(model: "new-model")]
        )
        ...
        func sends(
            ...
            await model.refreshPresetManifest(force: true)
            return notifications.sentRoutingUpdates.count
        }

        let brokerDisabled = await sends { $0.broker.isEnabled = false }
        let notificationsDisabled = await sends { $0.hasNotificationsEnabled = false }
        let routingNotificationsDisabled = await sends {
            $0.broker.routingUpdateNotificationsEnabled = false
        }
        let permissionDenied = await sends(configure: { _ in }, hasPermission: false)
        let allowed = await sends { _ in }

        XCTAssertEqual(brokerDisabled, 0)
        XCTAssertEqual(notificationsDisabled, 0)
        XCTAssertEqual(routingNotificationsDisabled, 0)
        XCTAssertEqual(permissionDenied, 0)
        XCTAssertEqual(allowed, 1)
    }

    func test_routingUpdate_failedSendDoesNotStoreFingerprint() async {
        let notifications = NotificationServiceSpy()
        notifications.routingUpdateError = TestError(message: "send failed")
        let model = makeRoutingAppModel(
            settings: routingSettings(),
            manifests: [BrokerPresetManifest(
                schemaVersion: 1,
        ...
        XCTAssertNil(model.settings.broker.lastRoutingUpdateNotifiedFingerprint)
    }

    func test_routingUpdateApplyPreservesLocalEditsUntilExplicitlyConfirmed() {
        var settings = routingSettings()
        settings.broker.updateRemotePresets([updatedBalanced(model: "published-model")])
        settings.broker.policy.roles["planning"] = [BrokerCandidate(route: .auto, model: "local-model")]
        let model = makeRoutingAppModel(settings: settings, manifests: [], notifications: NotificationServiceSpy())
        model.applyRoutingUpdate(profileID: BrokerAgentProfile.balancedID)
        XCTAssertEqual(model.settings.broker.policy.roles["planning"]?.first?.model, "local-model")
        XCTAssertTrue(model.settings.broker.activeProfileHasUpdatedRules)
        XCTAssertTrue(model.applyRoutingUpdate(profileID: BrokerAgentProfile.balancedID, discardingEdits: true))
        XCTAssertEqual(model.settings.broker.policy.roles["planning"]?.first?.model, "published-model")
        XCTAssertFalse(model.settings.broker.activeProfileHasUpdatedRules)
        XCTAssertTrue(model.undoRoutingUpdate())
        XCTAssertEqual(model.settings.broker.policy.roles["planning"]?.first?.model, "local-model")
        XCTAssertNotNil(model.routingUpdateApplyTask)
        XCTAssertFalse(model.undoRoutingUpdate())
    }

    func test_revisionRestoreRequiresConsentRevalidatesSelectionAndPushesPolicy() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("BrokerAppModelTests-restore-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: directory) }
        let broker = FakeBrokerService()
        var initial = routingSettings()
        initial.broker.policy.thresholds.weeklyPct = 71
        let source = try XCTUnwrap(initial.broker.presetManifest.sources.first)
        let model = makeRoutingAppModel(
            settings: initial,
            manifests: [BrokerPresetManifest(schemaVersion: 1, presets: [updatedBalanced(model: "claude-fable-5")])],
            notifications: NotificationServiceSpy(), broker: broker,
            historyStore: BrokerManifestHistoryStore(storeDirectory: directory)
        )
        await model.refreshPresetManifest(force: true)
        guard case .loaded(let revisions) = model.manifestHistoryState(for: source) else {
        ...
        let selection = BrokerRevisionSelection(
            namespace: BrokerManifestHistoryNamespace(sourceID: source.id, urlString: source.urlString),
            snapshot: revision, profileID: BrokerAgentProfile.balancedID
        )
        let before = model.settings.broker
        XCTAssertFalse(model.restoreProfileRules(selection: selection))
        XCTAssertEqual(model.settings.broker, before)
        XCTAssertTrue(model.restoreProfileRules(selection: selection, discardingEdits: true))
        await model.routingUpdateApplyTask?.value
        let restoredPolicies = await broker.updatedPolicies
        XCTAssertEqual(restoredPolicies.last?.ruleSet, revision.profile(id: selection.profileID)?.rules)
        XCTAssertTrue(model.undoRoutingUpdate())
        await model.routingUpdateApplyTask?.value
        let undonePolicies = await broker.updatedPolicies
        XCTAssertEqual(undonePolicies.last?.ruleSet, before.policy.ruleSet)
        XCTAssertEqual(model.settings.broker.activeProfileRules, before.activeProfileRules)

        model.settings.broker.activeProfileID = BrokerAgentProfile.maxQualityID
        let switched = model.settings.broker
        XCTAssertFalse(model.restoreProfileRules(selection: selection, discardingEdits: true))
        XCTAssertEqual(model.settings.broker, switched)
        model.settings.broker = before
        // Reusing a digest does not authorize an altered snapshot payload.
        var presets = revision.presets
        presets.removeAll { $0.id == selection.profileID }
        let missingProfile = BrokerManifestHistorySnapshot(
            id: revision.id, recordedAt: revision.recordedAt,
            contentDigest: revision.contentDigest, presets: presets,
            agentSetup: revision.agentSetup, manifestRevision: revision.manifestRevision,
            publishedAt: revision.publishedAt, summary: revision.summary
        )
        let invalidSelection = BrokerRevisionSelection(
            namespace: selection.namespace, snapshot: missingProfile, profileID: selection.profileID
        )
        let unchanged = model.settings.broker
        XCTAssertFalse(model.restoreProfileRules(selection: invalidSelection, discardingEdits: true))
        XCTAssertEqual(model.settings.broker, unchanged)
        model.settings.broker = before
        model.settings.broker.presetManifest.sources[0].urlString = "https://example.com/replaced.json"
        let changedSource = model.settings.broker
        XCTAssertFalse(model.restoreProfileRules(selection: selection, discardingEdits: true))
        XCTAssertEqual(model.settings.broker, changedSource)
    }

    func test_routingUpdateApplyActionOnlyAppliesTheStillActiveProfile() {
        var settings = routingSettings()
        let published = updatedBalanced(model: "published-model")
        settings.broker.updateRemotePresets([published])
        let model = makeRoutingAppModel(
            settings: settings,
            manifests: [],
            notifications: NotificationServiceSpy()
        ...
        notificationService.handleNotificationResponse(
            ...
            userInfo: ["profile_id": BrokerAgentProfile.balancedID.uuidString]
        )
        XCTAssertEqual(
            model.settings.broker.policy.roles["planning"]?.first?.model,
            "published-model"
        )

        model.settings.broker.applyProfile(id: BrokerAgentProfile.maxQualityID)
        let currentRules = model.settings.broker.policy.ruleSet
        notificationService.handleNotificationResponse(
            categoryIdentifier: "broker.routingUpdate",
            actionIdentifier: "broker.routingUpdate.apply",
            userInfo: ["profile_id": BrokerAgentProfile.balancedID.uuidString]
        )
        XCTAssertEqual(model.settings.broker.policy.ruleSet, currentRules)
    }

    func test_manifestSetupRevisionTriggersImmediateRecheckReminder() async {
        let broker = FakeBrokerService()
        await broker.setInstructionCheck(InstructionCheck(
            runID: "old",
            caller: "claude-code",
            checkedAt: Date(),
            gradedBy: BrokerMCPServer.appVersion,
        ...
        let notice = BrokerAgentSetupNotice(
            revision: 1,
        ...
        XCTAssertEqual(notifications.sentRecheckReasons, [.setupContractChanged])
        XCTAssertEqual(notifications.sentRecheckNotices, [notice])
        let revisions = await broker.setupRevisions
        XCTAssertEqual(revisions.last ?? nil, 1)
    }
    ...
    func test_ensureBrokerAPIKey_reportsKeychainSaveFailureWithoutPublishingAKey() async throws {
        let settingsRepository = SettingsRepositoryFake()
        var settings = AppSettings.default
        settings.broker.apiKeyMode = .all
        try await settingsRepository.save(settings)
        ...
        let appModel = AppModel(
            settingsRepository: settingsRepository,
            keychainRepository: BrokerKeyWriteFailingRepository(),
            usageService: UsageServiceStub(fetchUsageResult: .failure(TestError(message: "no key"))),
            notificationService: NotificationServiceSpy(),
            brokerService: FakeBrokerService(),
            brokerServerFactory: { _, _, _ in FakeBrokerLoopbackServer() },
        ...
        await appModel.bootstrap()
        let succeeded = await appModel.ensureBrokerAPIKey()

        XCTAssertFalse(succeeded)
        XCTAssertNil(appModel.brokerAPIKey)
        XCTAssertEqual(
            appModel.brokerAPIKeyErrorMessage,
    ...
    func test_APIKeyModeChange_onSaveFailureRestoresModeAndKeepsServer() async throws {
        ...
        model.settings.broker.apiKeyMode = .all
        await model.applyBrokerAPIKeyModeChange(from: .none)
        XCTAssertEqual(model.settings.broker.apiKeyMode, .none)
        XCTAssertNil(model.brokerAPIKey)
        XCTAssertNotNil(model.brokerAPIKeyErrorMessage)
        XCTAssertEqual(factory.policies.count, 1)
        // SwiftUI observes the rollback too. It must not clear its explanation.
        await model.applyBrokerAPIKeyModeChange(from: .all)
        XCTAssertNotNil(model.brokerAPIKeyErrorMessage)
        XCTAssertEqual(factory.policies.count, 1)
    }

    func test_regenerateBrokerAPIKey_onKeychainFailureKeepsTheOldKeyAndServer() async throws {
        let settingsRepository = SettingsRepositoryFake()
        var settings = AppSettings.default
        settings.broker.isEnabled = true
        settings.broker.apiKeyMode = .all
        ...
        let appModel = AppModel(
            settingsRepository: settingsRepository,
            keychainRepository: BrokerKeyWriteFailingRepository(storedKey: "pm_old"),
            usageService: UsageServiceStub(fetchUsageResult: .failure(TestError(message: "no key"))),
            notificationService: NotificationServiceSpy(),
            brokerService: FakeBrokerService(),
            brokerServerFactory: { _, port, policy in factory.make(port: port, policy: policy) },
        ...
        await appModel.bootstrap()
        let succeeded = await appModel.regenerateBrokerAPIKey()

        XCTAssertFalse(succeeded)
        XCTAssertEqual(appModel.brokerAPIKey, "pm_old")
        XCTAssertEqual(factory.policies.count, 1, "a failed rotation must not restart the server")
        XCTAssertEqual(
            appModel.brokerAPIKeyErrorMessage,
            "Could not save the broker API key in Keychain. Check Keychain access and try again."
        )
    ...
    // MARK: - Test 11: active-cooldown detection behind the degraded-pick alert

    func test_hasActiveBrokerCooldowns_withOnlyExpiredEntries_isFalse() async throws {
        let fakeBroker = FakeBrokerService()
        await fakeBroker.setCooldowns([
            BrokerStatus.CooldownEntry(key: "t3:claudeAgent", availableAt: Date().addingTimeInterval(-60))
        ])
        let appModel = makeCooldownProbeAppModel(broker: fakeBroker)

        let hasActive = await appModel.hasActiveBrokerCooldowns()

        XCTAssertFalse(
            hasActive,
            "an entry whose availableAt has already passed gates nothing, so it must not offer the action"
        )
    }

    func test_hasActiveBrokerCooldowns_withAFutureEntry_isTrue() async throws {
        let fakeBroker = FakeBrokerService()
        await fakeBroker.setCooldowns([
            BrokerStatus.CooldownEntry(key: "t3:claudeAgent", availableAt: Date().addingTimeInterval(-60)),
            BrokerStatus.CooldownEntry(key: "codex", availableAt: Date().addingTimeInterval(600))
        ])
        let appModel = makeCooldownProbeAppModel(broker: fakeBroker)
    ...
    func test_hasActiveBrokerCooldowns_withNoEntries_isFalse() async throws {
        let appModel = makeCooldownProbeAppModel(broker: FakeBrokerService())
    ...
    // MARK: - Test 12: the modal's cause title reads the same oracle the broker does

    func test_brokerProviderFault_beforeAnyOraclePush_isNil() async throws {
        // `codex/gpt-5.6-sol` on purpose: it is in `BrokerPolicy.bundledDefault`
        // and resolves to a real ChatGPT lane, so the absent oracle is the ONLY
        // thing that can produce nil here. An unmapped candidate would return
        // nil at the lane lookup and the test would pass without ever reaching
        // the behaviour it claims to cover.
        let appModel = makeCooldownProbeAppModel(broker: FakeBrokerService())
        let candidate = try XCTUnwrap(BrokerCandidate(id: "codex/gpt-5.6-sol"))
        XCTAssertNotNil(
            appModel.settings.broker.policy.usageLane(for: candidate),
            "fixture precondition: this candidate must map to a lane, or the assertion below is vacuous"
        )

        XCTAssertNil(
            appModel.brokerProviderFault(for: Self.degradedCodexDecision()),
            "during the launch window nothing has polled yet; announcing a disconnect then would send "
                + "the operator to reconnect an account that is fine"
        )
    }

    func test_brokerProviderFault_readsTheSnapshotItPushedToTheBroker() async throws {
        let settingsRepository = SettingsRepositoryFake()
        var settings = AppSettings.default
        settings.broker.isEnabled = true
        settings.broker.policy.usageLanes["codex/gpt-5.6-sol"] = .chatGPT(labelContains: nil)
        try await settingsRepository.save(settings)
        ...
        let appModel = AppModel(
            ...
            notificationService: NotificationServiceSpy(),
            brokerService: fakeBroker,
            brokerServerFactory: { _, _, _ in FakeBrokerLoopbackServer(startResult: .success(43117)) },
            t3LivenessChecker: BrokerAppModelFakeT3LivenessChecker(),
            t3InstanceDiscovery: T3InstanceDiscoveryFake()
        )
        ...
        let pushed = await fakeBroker.pushedOracleSnapshots.compactMap { $0 }.last
        XCTAssertNotNil(pushed, "bootstrap must push a snapshot for this test to mean anything")
        XCTAssertEqual(
            appModel.retainedOracleSnapshotForTesting, pushed,
            "the modal and the broker must judge a provider from the same reading, or the title can "
                + "contradict the decision reason shown beneath it"
        )
    }

    private static func degradedCodexDecision() -> BrokerDecision {
        BrokerDecision(
            role: "heavy",
            caller: "codex",
            ...
            route: .codex,
            agentModel: nil,
            invocation: .codexExec(model: "gpt-5.6-sol"),
            reason: "failing open",
            source: .policy,
            oracle: .absent,
            degraded: true,
            degradedReason: "failing open",
            retryable: true,
            candidatesTried: []
        )
    }

    private func makeCooldownProbeAppModel(broker: FakeBrokerService) -> AppModel {
        AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
            usageService: UsageServiceStub(fetchUsageResult: .failure(TestError(message: "no key"))),
...
private actor BrokerKeyWriteFailingRepository: KeychainRepositoryProtocol {
    ...
    func repairClaudeSessionKey(
        _ sessionKey: String,
        account: String
    ) async throws -> ClaudeCredentialRepairResult {
        throw KeychainError.saveFailed(OSStatus: OSStatus(errSecInteractionNotAllowed))
    }

    func retrieve(account: String) async throws -> String {
        guard let storedKey else { throw KeychainError.notFound }
        return storedKey
    }
...
private actor SequentialPresetManifestService: PresetManifestServiceProtocol {
    private let manifests: [BrokerPresetManifest]
    private var index = 0

    init(manifests: [BrokerPresetManifest]) {
        self.manifests = manifests
    }

    func fetch(from url: URL, etag: String?) async throws -> PresetManifestFetchOutcome {
        guard !manifests.isEmpty else {
            throw TestError(message: "no manifest configured")
        }
        let manifest = manifests[min(index, manifests.count - 1)]
        index += 1
        return .updated(manifest: manifest, etag: nil)
    }
}

private actor SuspendedPresetManifestService: PresetManifestServiceProtocol {
    private var fetchStarted = false
    private var startWaiters: [CheckedContinuation<Void, Never>] = []
    private var response: CheckedContinuation<PresetManifestFetchOutcome, Never>?
    ...
    func resume(with manifest: BrokerPresetManifest) {
        response?.resume(returning: .updated(manifest: manifest, etag: nil))
        response = nil
    }
}
...
private final class BlockingHistoryWriter: @unchecked Sendable {
    private let started = DispatchSemaphore(value: 0)
    private let resumeWrite = DispatchSemaphore(value: 0)
    ...
    func waitUntilWriteStarts() async {
        await Task.detached { self.started.wait() }.value
    }
...
private actor FakeBrokerService: BrokerLifecycleProtocol {
    ...
    private(set) var setupRevisions: [Int?] = []
    ...
    private var cooldowns: [BrokerStatus.CooldownEntry] = []
    ...
    func setCooldowns(_ entries: [BrokerStatus.CooldownEntry]) {
        cooldowns = entries
    }
    ...
    func updateSetupRevision(_ revision: Int?) async {
        setupRevisions.append(revision)
    }

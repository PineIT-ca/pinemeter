...
final class UsageServiceTests: XCTestCase {
    func test_retryDelay_isJitteredAndSkipsFinalAttempt() {
        for attempt in 0..<(Constants.Network.maxRetries - 1) {
            let base = Constants.Network.backoffBase
            let unjittered = pow(base, Double(attempt))
            let delay = UsageService.retryDelay(attempt: attempt, base: base)

            XCTAssertNotNil(delay)
            XCTAssertGreaterThanOrEqual(delay ?? 0, unjittered * 0.8)
            XCTAssertLessThanOrEqual(delay ?? 0, unjittered * 1.2)
        }
        XCTAssertNil(
            UsageService.retryDelay(
                attempt: Constants.Network.maxRetries - 1,
                base: Constants.Network.backoffBase
            )
        )
    }
    ...
    func test_usageResponse_mapsModelScopedFableLimit() throws {
        ...
        response.memberDashboardAvailable = false
        response.limits = [
    ...
    func test_usageResponse_reportsUnavailableDashboard() throws {
        let json = """
        {
            "five_hour": null,
            "seven_day": null,
        ...
        XCTAssertThrowsError(try response.toDomain()) { error in
            guard case MappingError.usageUnavailable = error else {

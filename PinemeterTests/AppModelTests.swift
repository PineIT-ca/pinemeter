...
final class AppModelTests: XCTestCase {
    func test_refreshConfiguredUsageProviders_rescansAndPromptsWhenClaudeSessionIsLost() async {
        let importService = SessionKeyImportServiceStub(
            result: .failure(SessionKeyImportError.noSessionKeyFound)
        )
        var prompts: [[String]] = []
        let appModel = AppModel(
            usageService: UsageServiceStub(fetchUsageResult: .failure(AppError.sessionKeyInvalid)),

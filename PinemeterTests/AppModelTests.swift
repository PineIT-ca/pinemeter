...
final class AppModelTests: XCTestCase {
    func test_refreshConfiguredUsageProviders_recoversRejectedClaudeAccountsThenRestoresFreshness() async {
        let primaryId = UUID()
        let secondaryId = UUID()
        let expectedUsage = makeUsageData(percentage: TestConstants.sessionPercentage)
        let usageService = RecoveryUsageServiceStub(result: .failure(AppError.sessionKeyInvalid))
        ...
        let appModel = AppModel(
            ...
            browserLoginPrompt: { prompts.append($0) }
        )
        ...
        let requestedSources = await importService.requestedSources
        XCTAssertEqual(requestedSources, [.chrome, .chrome])
        XCTAssertEqual(prompts, [["Claude"]])
        ...
        XCTAssertEqual(
            appModel.claudeAccountErrors[secondaryId.uuidString],
            AppError.sessionKeyInvalid.localizedDescription
        )

        await usageService.setResult(.success(expectedUsage))
        await appModel.refreshConfiguredUsageProviders(forceRefresh: true)

        XCTAssertEqual(appModel.claudeCredentialState.health, .valid)
        XCTAssertNil(appModel.claudeCredentialState.failureCategory)
        XCTAssertEqual(appModel.usageData, expectedUsage)
        XCTAssertEqual(appModel.claudeAccountUsage[secondaryId.uuidString], expectedUsage)
        XCTAssertNil(appModel.claudeAccountErrors[secondaryId.uuidString])
        XCTAssertEqual(prompts, [["Claude"]])
    }

    func test_refreshConfiguredUsageProviders_doesNotRejectCredentialsForTransportOrDecodingFailures() async {
        let errors: [Error] = [
            AppError.networkError(.networkUnavailable),
            AppError.networkError(.decodingFailed(underlyingError: TestError(message: "malformed JSON"))),
        ]

        for error in errors {
            var prompts: [[String]] = []
            let appModel = AppModel(
                usageService: UsageServiceStub(fetchUsageResult: .failure(error)),
                notificationService: NotificationServiceSpy(),
                sessionKeyImportService: SessionKeyImportServiceStub(
                    result: .failure(SessionKeyImportError.noSessionKeyFound)
                ),
                runningBrowserSources: { [.chrome] },
                browserLoginPrompt: { prompts.append($0) }
    ...
    func test_availableUpdate_usesNumericAndPrereleasePrecedence() {
        XCTAssertTrue(AvailableUpdate(version: "1.1.0").isNewer(than: "1.1.0-beta.13"))
        XCTAssertTrue(AvailableUpdate(version: "1.10.0").isNewer(than: "1.9.0"))
        XCTAssertTrue(AvailableUpdate(version: "2.0.0").isNewer(than: "1.1.0-beta.1"))
        XCTAssertFalse(AvailableUpdate(version: "1.1.0-beta.2").isNewer(than: "1.1.0-beta.13"))
    ...
    func test_betaUpdates_propagateFromBootstrapAndSettingsChanges() async throws {
        let settingsRepository = SettingsRepositoryFake()
        var settings = AppSettings.default
        ...
        await appModel.bootstrap()
        appModel.settings.includeBetaUpdates = false

        XCTAssertEqual(updater.betaUpdatesEnabledValues, [true, false])
        XCTAssertEqual(updater.startedWithBetaUpdatesEnabled, [true])
    }

    func test_stableBootstrap_startsUpdaterOnceWithBetaDisabled() async {
        let updater = AppUpdaterSpy()
        let appModel = AppModel(
        ...
        XCTAssertEqual(updater.startedWithBetaUpdatesEnabled, [false])
    }
    ...
    func test_refreshingUsage_surfacesUnavailableClaudeAccounts() async {
        let primaryId = UUID()
        let secondaryId = UUID()
        let appModel = AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
            usageService: UsageServiceStub(fetchUsageResult: .failure(MappingError.usageUnavailable)),
            notificationService: NotificationServiceSpy()
        )
        appModel.isSetupComplete = true
        ...
        XCTAssertNil(appModel.usageData)
        XCTAssertEqual(appModel.errorMessage, MappingError.usageUnavailable.localizedDescription)
        XCTAssertEqual(
            appModel.claudeAccountErrors[secondaryId.uuidString],
            MappingError.usageUnavailable.localizedDescription
        )
        XCTAssertTrue(appModel.hasUsagePopoverContent)
        XCTAssertEqual(appModel.claudeUsageSections.map(\.title), ["Primary", "Secondary"])
        XCTAssertEqual(appModel.claudeCredentialState.health, .valid)
    }
    ...
    func test_usagePopoverContentIncludesBrokerWithoutUsageAccounts() {
        let appModel = AppModel(
            settingsRepository: SettingsRepositoryFake(),
            keychainRepository: KeychainRepositoryFake(),
            usageService: UsageServiceStub(fetchUsageResult: .failure(TestError(message: "unused"))),
            notificationService: NotificationServiceSpy()
        )
        ...
        appModel.settings.isChatGPTUsageShown = true
        appModel.chatGPTAccountErrors["secondary"] = "Session expired"
        XCTAssertTrue(appModel.hasUsagePopoverContent)
        appModel.settings.isChatGPTUsageShown = false
        XCTAssertFalse(appModel.hasUsagePopoverContent)
        appModel.hasGeminiAPIKey = true
        appModel.geminiAccountErrors["secondary"] = "Quota unavailable"
        XCTAssertTrue(appModel.hasUsagePopoverContent)
    }
...
private final class AppUpdaterSpy: AppUpdaterProtocol {
    ...
    private(set) var betaUpdatesEnabledValues: [Bool] = []
    private(set) var startedWithBetaUpdatesEnabled: [Bool] = []
    private var betaUpdatesEnabled = false

    func start() {
        startedWithBetaUpdatesEnabled.append(betaUpdatesEnabled)
    }
    ...
    func setBetaUpdatesEnabled(_ enabled: Bool) {
        ...
        betaUpdatesEnabledValues.append(enabled)
    }
}
...
private actor RecoveryUsageServiceStub: UsageServiceProtocol {
    private var result: Result<UsageData, Error>

    init(result: Result<UsageData, Error>) {
        self.result = result
    }

    func setResult(_ result: Result<UsageData, Error>) {
        self.result = result
    }

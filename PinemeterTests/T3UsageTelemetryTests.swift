...
final class T3UsageTelemetryTests: XCTestCase {
    ...
    func testExplicitTelemetryFlushReportsPersistenceFailure() async throws {
        let directory = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let now = Date(timeIntervalSince1970: 1_786_700_000)
        ...
        XCTAssertEqual(result.availability, .fresh(readAt: now))
        XCTAssertEqual(lastFreshSnapshot, result.freshSnapshot)
        XCTAssertEqual(records.count, 1)
        do {
            try await service.flushTelemetry()
            XCTFail("flush should surface the writer failure")
        } catch let error as CocoaError {
            XCTAssertEqual(error.code, .fileWriteOutOfSpace)
        }
        let restartedRecords = await UsageTelemetryStore(storeDirectory: directory).records()
        XCTAssertTrue(restartedRecords.isEmpty)
    }

    func testPersistenceFailureAvailabilityRemainsDecodable() throws {
        let encoded = Data(#""persistence_failure""#.utf8)
        XCTAssertEqual(
            try JSONDecoder().decode(T3UsageUnavailableReason.self, from: encoded),
            .persistenceFailure
        )
    }
    ...
    func testTelemetryStoreBatchesRapidAppendsUntilExplicitFlush() async throws {
        let directory = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let writer = TelemetryWriterSpy()
        let store = UsageTelemetryStore(storeDirectory: directory, writer: writer.write)
        ...
        let inMemoryRecords = await store.records()
        XCTAssertEqual(inMemoryRecords.count, 5)

        try await store.flush()

        XCTAssertEqual(writer.writeCount, 1)
        let restartedRecords = await UsageTelemetryStore(storeDirectory: directory).records()
        XCTAssertEqual(restartedRecords.count, 5)
    }

    func testScheduledFlushFailureUsesObservableLogger() async throws {
        let directory = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let logger = TelemetryFailureLoggerSpy()
        let logged = expectation(description: "scheduled flush failure logged")
        let store = UsageTelemetryStore(
            storeDirectory: directory,
            writer: { _, _ in throw CocoaError(.fileWriteOutOfSpace) },
            scheduledFlushInterval: .zero,
        ...
        try await store.append(Self.record(at: Date(), contractVersion: 3))
        await fulfillment(of: [logged], timeout: 1)

        XCTAssertEqual(logger.messages.count, 1)
        XCTAssertTrue(logger.messages[0].contains("Scheduled telemetry flush failed"))
    }

    func testProductionTerminationCallerFlushesTelemetryWithoutBlocking() async throws {
        let sourceRoot = URL(fileURLWithPath: #filePath).deletingLastPathComponent().deletingLastPathComponent()
        let appDelegate = try String(
            contentsOf: sourceRoot.appendingPathComponent("Pinemeter/App/AppDelegate.swift"),
            encoding: .utf8
        )

        XCTAssertTrue(appDelegate.contains("func applicationShouldTerminate("))
        XCTAssertTrue(appDelegate.contains("await appModel.flushUsageTelemetry()"))
        XCTAssertTrue(appDelegate.contains("return .terminateLater"))
        XCTAssertTrue(appDelegate.contains("reply(toApplicationShouldTerminate: true)"))
    }
    ...
    func testTelemetryStoreRetentionRestartAndSecretExclusion() async throws {
        ...
        try await concurrentStore.flush()
...
private final class TelemetryWriterSpy: @unchecked Sendable {
...
private final class TelemetryFailureLoggerSpy: @unchecked Sendable {
    private let lock = NSLock()
    private var loggedMessages: [String] = []

    var messages: [String] { lock.withLock { loggedMessages } }

    func log(_ message: String) {
        lock.withLock { loggedMessages.append(message) }
    }
}

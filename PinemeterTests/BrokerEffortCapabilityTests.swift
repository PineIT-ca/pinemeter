...
final class BrokerEffortCapabilityTests: XCTestCase {
    ...
    func test_support_supportedModel_offersEveryLevel() {
        ...
        XCTAssertEqual(support.selectableLevels, BrokerEffort.builtInLevels)
    }

    func test_support_supportedModel_isEquatableOnItsLabel() {
        XCTAssertEqual(
            BrokerEffort.support(forModel: "claude-fable-5-1"),
            .supported(levels: BrokerEffort.builtInLevels, nilLabel: "Adaptive (high)")
        )
    ...
    /// Opus 5.5 lowered the unset default to medium while keeping an id the
    /// `claude-opus-5` prefix branch also matches, so this pins the branch
    /// order: a regression here silently mislabels the picker's nil row.
    func test_nilLabel_opus55_readsDefaultMediumNotHigh() {
        XCTAssertEqual(BrokerEffort.support(forModel: "claude-opus-5-5").nilLabel, "Default (medium)")
        XCTAssertEqual(
            BrokerEffort.support(forModel: "claude-opus-5-5").selectableLevels,
            BrokerEffort.builtInLevels
        )
    }

    /// The bundled catalog wins over the compiled heuristics, so the shipped
    /// spec must agree with them rather than quietly contradict them.
    func test_bundledCatalog_opus55_declaresTheSameMediumDefault() {
        XCTAssertEqual(
            BrokerEffort.support(forModel: "claude-opus-5-5", in: BrokerPolicy.bundledDefault.models),
            .supported(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
        )
    }
    ...
    func test_nilLabel_gpt6SolAndLuna_readDefaultMedium() {
        XCTAssertEqual(BrokerEffort.support(forModel: "gpt-6-sol").nilLabel, "Default (medium)")
        XCTAssertEqual(BrokerEffort.support(forModel: "gpt-6-luna").nilLabel, "Default (medium)")
    }

    /// The bundled catalog wins over the compiled heuristics, so the shipped
    /// spec must agree with them rather than quietly contradict them.
    func test_bundledCatalog_gpt6SolAndLuna_declareTheSameMediumDefault() {
        XCTAssertEqual(
            BrokerEffort.support(forModel: "gpt-6-sol", in: BrokerPolicy.bundledDefault.models),
            .supported(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
        )
        XCTAssertEqual(
            BrokerEffort.support(forModel: "gpt-6-luna", in: BrokerPolicy.bundledDefault.models),
            .supported(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
        )
        // gpt-6-astra's catalog entry moved off "Provider default" to match:
        // the local Codex catalog reports medium as its default too.
        XCTAssertEqual(
            BrokerEffort.support(forModel: "gpt-6-astra", in: BrokerPolicy.bundledDefault.models),
            .supported(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
        )
    }
    ...
    func test_effortAccessibilityValue_noEffort_returnsModelNilLabel() {
        ...
        XCTAssertEqual(
            BrokerPolicyEditorView.effortAccessibilityValue(for: candidate, in: BrokerPolicy.default.models),
            "Default (high)"
        )
    }
    ...
    func test_effortAccessibilityValue_staleEffortOnAnUnsupportedModel_readsUnsupported() {
        ...
        XCTAssertEqual(BrokerPolicyEditorView.effortAccessibilityValue(for: bareAlias, in: [:]), "Unsupported")
    }
    ...
    func test_clampingEffortToModelSupport_haikuCandidate_dropsEffort() {
        let candidate = BrokerCandidate(route: .native, model: "claude-haiku-4-5-20251001", effort: .high)
        XCTAssertNil(candidate.clampingEffortToModelSupport(in: BrokerPolicy.default.models).effort)
    }

    func test_clampingEffortToModelSupport_opusCandidate_keepsEffort() {
        let candidate = BrokerCandidate(route: .native, model: "claude-opus-5", effort: .xhigh)
        XCTAssertEqual(candidate.clampingEffortToModelSupport(in: BrokerPolicy.default.models).effort, .xhigh)
    }
    ...
    func test_clampingEffortToModelSupport_unrecognisedModel_keepsEffort() {
        ...
        XCTAssertEqual(candidate.clampingEffortToModelSupport(in: [:]).effort, .low)
    }

    func test_manifestDeclaredLevelsDriveSupportAndClamp() {
        let ultra = BrokerEffort(rawValue: "ultra")!
        let models = [
            "gpt-future": BrokerModelSpec(
                effort: BrokerEffortCapability(
                    levels: [.low, ultra], nilLabel: "Default (ultra)"
                )
            )
        ]

        let support = BrokerEffort.support(forModel: "gpt-future", in: models)

        XCTAssertEqual(support.selectableLevels, [.low, ultra])
        XCTAssertEqual(support.nilLabel, "Default (ultra)")
        XCTAssertEqual(
            BrokerCandidate(route: .codex, model: "gpt-future", effort: ultra)
                .clampingEffortToModelSupport(in: models).effort,
            ultra
        )
        XCTAssertNil(
            BrokerCandidate(route: .codex, model: "gpt-future", effort: .xhigh)
                .clampingEffortToModelSupport(in: models).effort
        )
    }

    @MainActor
    func test_effortBinding_manifestDeclaredUltra_storesTheLevel() {
        let appModel = makeEditorTestAppModel()
        let ultra = BrokerEffort(rawValue: "ultra")!
        appModel.settings.broker.policy.models["gpt-future"] = BrokerModelSpec(
            effort: BrokerEffortCapability(levels: [.low, ultra])
        )
        appModel.settings.broker.policy.roles["standard"] = [
            BrokerCandidate(route: .auto, model: "gpt-future")
        ]
        let view = BrokerPolicyEditorView(appModel: appModel, initialSelectedRole: "standard")

        view.candidateEffortBinding(role: "standard", index: 0).wrappedValue = ultra

        XCTAssertEqual(appModel.settings.broker.policy.roles["standard"]?.first?.effort, ultra)
    }

    func test_adoptingPreset_clampsUndeclaredEffortAndAccessibilityUsesDefault() throws {
        let data = Data(#"""
        {"schema_version":1,"presets":[{
          "id":"99999999-0000-0000-0000-000000000099","name":"Future",
          "rules":{"roles":{"heavy":[{"id":"auto/gpt-future","effort":"xhigh"}]},
            "models":{"gpt-future":{"effort":{"levels":["low","medium"]}}}}
        }]}
        """#.utf8)
        ...
        settings.applyProfile(id: preset.id)

        XCTAssertNil(settings.policy.roles["heavy"]?.first?.effort)
        XCTAssertEqual(
            BrokerPolicyEditorView.effortAccessibilityValue(
                for: BrokerCandidate(route: .auto, model: "gpt-future", effort: .xhigh),
                in: settings.policy.models
            ),
            "Provider default"
        )
    }

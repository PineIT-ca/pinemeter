...
final class BrokerEffortCapabilityTests: XCTestCase {
    ...
    func test_support_supportedModel_isEquatableOnItsLabel() {
        XCTAssertEqual(BrokerEffort.support(forModel: "claude-fable-5"), .supported(nilLabel: "Adaptive (high)"))
    ...
    func test_nilLabel_fable_readsAdaptiveHigh() {
        XCTAssertEqual(BrokerEffort.support(forModel: "claude-fable-5").nilLabel, "Adaptive (high)")
    }
    ...
    @MainActor
    func test_updateCandidate_modelOnlyEditOntoHaiku_clearsStoredEffortAndKeepsOtherFields() {
        let appModel = makeEditorTestAppModel()
        appModel.settings.broker.policy.roles["planning"] = [
            BrokerCandidate(route: .t3, instance: "claude_autimo", model: "claude-opus-5", effort: .high)
        ]
        ...
        XCTAssertEqual(updated?.route, .t3)
        XCTAssertEqual(updated?.instance, "claude_autimo")
        XCTAssertNil(updated?.effort)
    }

    @MainActor
    func test_updateCandidate_routeOnlyEditOnHaikuRow_leavesEffortNilAndDoesNotResurrectIt() {
        let appModel = makeEditorTestAppModel()
    ...
    // MARK: - The row's own bindings route through the shared path
    //
    ...
    @MainActor
    func test_routeBinding_setOnAHaikuRow_leavesEffortNil() {
        let appModel = makeEditorTestAppModel()
        // The stored pair is already invalid (an effort on a model with no
        // effort parameter), as an older build or a hand-edited file can leave
        // it. A route edit must not carry it forward.
        appModel.settings.broker.policy.roles["execution"] = [
            BrokerCandidate(route: .native, model: "claude-haiku-4-5-20251001", effort: .high)
        ]
        let view = BrokerPolicyEditorView(appModel: appModel, initialSelectedRole: "execution")

        view.candidateRouteBinding(role: "execution", index: 0).wrappedValue = .t3
    ...
    @MainActor
    func test_instanceBinding_setOnAHaikuRow_leavesEffortNil() {
        let appModel = makeEditorTestAppModel()
        appModel.settings.broker.policy.roles["execution"] = [
            BrokerCandidate(route: .t3, model: "claude-haiku-4-5-20251001", effort: .medium)
        ]
        let view = BrokerPolicyEditorView(appModel: appModel, initialSelectedRole: "execution")

        view.candidateInstanceBinding(role: "execution", index: 0).wrappedValue = "claude_autimo"

...
final class BrokerEngineTests: XCTestCase {
    ...
    func test_claudeAccountLaneWithMissingSharedMetric_isAvailableButDegraded() throws {
        let candidate = "t3:claude_autimo/claude-fable-5"
        let policy = BrokerFixture.policy(
        ...
        for account in [
            BrokerFixture.account(id: "acct-autimo", isPrimary: false, session: nil),
            BrokerFixture.account(id: "acct-autimo", isPrimary: false, weekly: nil),
        ] {
            let decision = try decide(
                policy: policy,
    ...
    func test_staleSnapshotsDoNotGateNonNativeLanes() throws {
        let claudeCandidate = "t3:claude_autimo/claude-fable-5"
        ...
        let cappedAccount = BrokerFixture.account(
            id: "acct-autimo", label: "Autimo", isPrimary: false, weekly: 99, fable: 99
        )

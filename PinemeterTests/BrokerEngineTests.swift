...
final class BrokerEngineTests: XCTestCase {
    ...
    func testFreshWeeklyWindowDoesNotPaceGateAHealthyNativeLane() throws {
        // Regression: 6.6 hours into a 7-day weekly window, 5% weekly usage
        // scored 1.27 on the raw pacing ratio and gated every Claude lane, so
        // every role fell through to a forced-degraded pick of the candidate it
        // had just rejected. The window is barely open; 5% is not a burn rate.
        let candidate = "native/claude-opus-5"
        let weeklyWindow: TimeInterval = 7 * 24 * 60 * 60
        let decision = try decide(
            policy: BrokerFixture.policy(roles: ["planning": [candidate]]),
            oracle: BrokerFixture.oracle(accounts: [
                BrokerFixture.account(
                    session: 34,
                    weekly: 5,
                    // Minutes, not hours: the session window is deliberately
                    // near its end, so 34% session usage paces comfortably and
                    // only the weekly pool is under test here.
                    sessionResetAt: BrokerFixture.now.addingTimeInterval(4 * 60),
                    weeklyResetAt: BrokerFixture.now.addingTimeInterval(weeklyWindow - 6.6 * 60 * 60)
                )
            ])
        )
    ...
    func testPacedLaneIsTakenAsANormalPickWhenNothingHasSparePace() throws {
        // Regression: pacing used to make a candidate unavailable. With one
        // account ahead of pace, every Claude lane went unavailable, the walk
        // fell through to the forced fallback, and the fallback handed back the
        // same candidate flagged degraded. Spending ahead of pace is a reason
        // to prefer another lane, not a reason to refuse the work.
        let candidate = "native/claude-opus-5"
        let decision = try decide(
            policy: BrokerFixture.policy(roles: ["planning": [candidate]]),
            oracle: BrokerFixture.oracle(accounts: [
        ...
        XCTAssertEqual(decision.source, .policy)
        XCTAssertFalse(decision.degraded)
        XCTAssertTrue(decision.reason.contains("pacing"), decision.reason)
        XCTAssertFalse(decision.reason.contains("degraded"), decision.reason)
        XCTAssertTrue(try tried(decision, candidate).available)
    }

    func testPacedLaneOutranksALaneWithNoConfiguredQuotaSource() throws {
        // Both candidates are weak yeses, so their order has to be decided
        // rather than fall out of the walk. A paced lane's headroom was
        // measured and sits under every ceiling; an unconfigured lane's
        // headroom has never been seen at all, and on this machine the CLI
        // behind it may not even be installed. Measured-and-fast wins.
        let decision = try decide(
            role: "execution",
        ...
        XCTAssertEqual(decision.source, .policy)
        XCTAssertFalse(decision.degraded, "measured headroom under every ceiling is not a blind pick")
        XCTAssertTrue(
            try tried(decision, "codex/gpt-5.6-sol").why.contains("held back for a source-verified candidate")
        )
    }

    func testHealthyLaneStillOutranksAPacedOneAndThePacedOneBecomesABackup() throws {
        // The demotion has to keep working in the ordinary case: a lane with
        // spare pace wins outright, and the paced lane is still offered as a
        // backup — above a hard-capped lane, and labelled for what it is.
        // Three different models on purpose: equivalent routes for the SAME
        // model are re-ranked by headroom, which would reorder the chain and
        // hide the demotion this case exists to check.
        let paced = "native/claude-opus-5"
        let healthy = "t3:spare/claude-sonnet-5"
        let capped = "t3:capped/claude-fable-5"
        let policy = BrokerFixture.policy(
            roles: ["planning": [paced, healthy, capped]],
            t3Instances: [
                T3InstanceConfig(id: "spare", name: "Spare", boundAccountId: "spare-acct"),
                T3InstanceConfig(id: "capped", name: "Capped", boundAccountId: "capped-acct"),
            ],
            usageLanes: [
                healthy: .claudeAccount(accountId: nil, labelContains: nil, isPrimary: nil),
                capped: .claudeAccount(accountId: nil, labelContains: nil, isPrimary: nil),
            ]
        )
        let weeklyReset = BrokerFixture.now.addingTimeInterval(5.25 * 24 * 60 * 60)
        let decision = try decide(
            ...
            t3: BrokerFixture.reachable("spare", "capped")
        )

        XCTAssertEqual(decision.model, healthy)
        XCTAssertFalse(decision.degraded)
        let pacedRow = try tried(decision, paced)
        XCTAssertTrue(pacedRow.available)
        XCTAssertTrue(
            pacedRow.why.contains("held back for a lane with spare pace"),
            pacedRow.why
        )
        XCTAssertEqual(decision.backups.map(\.candidate), [paced, capped])
        XCTAssertTrue(decision.backups[0].why.contains("ahead of pace"), decision.backups[0].why)
    ...
    func testHardCapsAndForcedDegradedSafetySurvivePacing() throws {
        ...
        let hardCapCases: [(model: String, account: OracleSnapshot.AccountRow, cap: String)] = [
            ...
            (
                "claude-fable-5",
                BrokerFixture.account(session: 1, weekly: 1, fable: 80, fableResetAt: weeklyReset),
                "fable pool 80% >= 80%"
            ),
        ]
        ...
        let missingReset = try decide(
            policy: BrokerFixture.policy(roles: ["planning": ["native/claude-fable-5"]]),
            oracle: BrokerFixture.oracle(accounts: [BrokerFixture.account(fable: 79)])
        )
        XCTAssertEqual(missingReset.source, .policy)

        // Hard-capped, not merely ahead of pace: pacing demotes a candidate
        // rather than removing it, so only a ceiling can empty the walk and
        // reach the forced fallback.
        let capped = BrokerFixture.account(
            session: 99,
            weekly: 99,
        ...
        let forced = try decide(
            ...
            oracle: BrokerFixture.oracle(accounts: [capped]),
            t3: [:]
        )
    ...
    func test_explicitOverride_resolvesTheArchitectureAliasAndEchoesRequestedRole() throws {
        let decision = try BrokerEngine.decideOverride(
            role: "architecture",
            caller: nil,
            overrideCandidate: "auto/claude-opus-5-5",
        ...
        XCTAssertEqual(decision.role, "architecture")
        XCTAssertEqual(decision.model, "native/claude-opus-5-5")
        // The override is accepted only because the alias resolves to the
        // planning chain, which configures this candidate. An override carries
        // exactly the effort the caller wrote, and this one wrote none.
        XCTAssertNil(decision.effort)
    }

    func test_explicitAutoOverrideSkipsRoutesTheCallerCannotInvoke() throws {
        let policy = BrokerFixture.policy(
            roles: ["review": ["auto/claude-fable-5-1"]],
            callers: ["codex": BrokerCallerPolicy(routes: [.codex, .t3])],
            t3Instances: [
                T3InstanceConfig(
                    id: "claudeAgent",
                    name: "Claude",
                    detectedModels: ["claude-fable-5-1"]
                ),
            ],
            agentModelAliases: ["claude-fable-5-1": "fable"]
        )

        let decision = try BrokerEngine.decideOverride(
            role: "review",
            caller: "codex",
            overrideCandidate: "auto/claude-fable-5-1",
            policy: policy,
            oracle: nil,
            cooldowns: [:],
            now: BrokerFixture.now,
        ...
        XCTAssertEqual(decision.model, "t3:claudeAgent/claude-fable-5-1")
        XCTAssertEqual(
            decision.invocation,
            .t3Dispatch(model: "claude-fable-5-1", instanceId: "claudeAgent")
        )
        XCTAssertEqual(
            decision.candidatesTried.map(\.candidate),
            ["native/claude-fable-5-1", "t3:claudeAgent/claude-fable-5-1"]
        )
        XCTAssertFalse(decision.candidatesTried[0].available)
        XCTAssertTrue(decision.candidatesTried[0].why.contains("not invocable by caller"))
    }

    func test_explicitAutoOverrideWithOnlyCallerFilteredRoutesFailsStructurally() throws {
        let policy = BrokerFixture.policy(
            roles: ["review": ["auto/claude-fable-5-1"]],
            callers: ["codex": BrokerCallerPolicy(routes: [.codex, .t3])],
            agentModelAliases: ["claude-fable-5-1": "fable"]
        ...
        XCTAssertThrowsError(try BrokerEngine.decideOverride(
            ...
            XCTAssertTrue(error.localizedDescription.contains("override rejected"))
            XCTAssertTrue(error.localizedDescription.contains("not invocable by caller"))
            XCTAssertFalse(error.localizedDescription.contains("Rescan Browsers"))
        }
    }

    func test_explicitCodexOverrideUsesNativeAgentForCodexCaller() throws {
        let policy = BrokerFixture.policy(
            roles: ["execution": ["codex/gpt-5.6-sol"]],
            callers: ["codex": BrokerCallerPolicy(routes: [.codex])]
        )

        let decision = try BrokerEngine.decideOverride(
            role: "execution",
            caller: "codex",
            overrideCandidate: "codex/gpt-5.6-sol",
            policy: policy,
            oracle: nil,
            cooldowns: [:],
            now: BrokerFixture.now,
    ...
    func test_architectureAlias_usesBundledPlanningChainAndEchoesRequestedRole() throws {
        let decision = try decide(role: "architecture", policy: .bundledDefault)
    ...
    func test_explicitArchitectureChain_winsOverThePlanningAlias() throws {
        let policy = BrokerFixture.policy(roles: [
            "architecture": ["native/claude-sonnet-5"],
            "planning": ["native/claude-opus-5"],
        ])

        let decision = try decide(role: "architecture", policy: policy)

        XCTAssertEqual(decision.model, "native/claude-sonnet-5")
    }

    func test_architectureAlias_withoutPlanningChainThrowsUnknownRole() {
        let policy = BrokerFixture.policy(roles: [
            "execution": ["native/claude-sonnet-5"],
        ])

        XCTAssertThrowsError(try decide(role: "architecture", policy: policy)) { error in
            guard case BrokerError.unknownRole(let role, let known) = error else {
                return XCTFail("expected unknownRole, got \(error)")
            }
            XCTAssertEqual(role, "architecture")
            XCTAssertEqual(known, ["execution"])
        }
    }
    ...
    func test_nativeGateOrder_surfacesWeeklyThenSessionThenSonnetThenFable() throws {
        ...
        XCTAssertTrue(fable.why.contains("fable pool 95% >= 80%"), fable.why)
        ...
        XCTAssertTrue(legacyFable.why.contains("fable pool 95% >= 80%"), legacyFable.why)
    }
    ...
    func test_nativeOracleSeparatesStalenessFromNonFreshRowState() throws {
        let policy = BrokerFixture.policy(roles: ["planning": ["native/claude-fable-5"]])
        let cases: [(
            name: String,
            ...
            ("fresh", .fresh, 535, false, false, "native weekly 20% < 85% ok"),
            ("fresh-aged error", .error, 535, false, true, "oracle row state error"),
            ("stale by age", .fresh, 1_201, true, true, "oracle stale (age 1201s > 1200s)"),
            ("invalid future timestamp", .fresh, -1, true, true, "oracle timestamp missing or invalid"),
        ]
        ...
        for testCase in cases {
            let decision = try decide(
                policy: policy,
                oracle: BrokerFixture.oracle(accounts: [
                    BrokerFixture.account(age: testCase.age, state: testCase.state)
                ])
            )

            XCTAssertEqual(decision.oracle.stale, testCase.stale, testCase.name)
            XCTAssertEqual(decision.oracle.ageSeconds, testCase.age >= 0 ? testCase.age : nil, testCase.name)
            XCTAssertEqual(decision.degraded, testCase.degraded, testCase.name)
            XCTAssertTrue(decision.reason.contains(testCase.reason), "\(testCase.name): \(decision.reason)")
        }
    }
    ...
    func test_reachableT3MappedToAClaudeAccountLane_gatesOnThatAccountsFreshRow() throws {
        ...
        XCTAssertTrue(cappedFable.why.contains("fable pool 95% >= 80%"), cappedFable.why)
    }
    ...
    /// An oracle that caps every routed quota pool, so nothing has headroom.
    private var cappedOracle: OracleSnapshot {
        BrokerFixture.oracle(
            accounts: [BrokerFixture.account(session: 99, weekly: 99)],
            chatGPTRows: [.init(label: "Codex weekly", usedPercent: 99)],
    ...
    func test_roleWithForcedDegradedDisabled_surfacesTheNoHeadroomErrorInstead() {
        ...
        XCTAssertThrowsError(try decide(policy: policy, oracle: cappedOracle)) { error in
            guard case BrokerError.noHeadroom(let role, let tried, let oracle) = error else {
                return XCTFail("expected noHeadroom, got \(error)")
            }
            XCTAssertEqual(role, "planning")
            // The dead end is diagnosable from the error alone: every
            // rejection reason and the snapshot they were computed from.
            XCTAssertEqual(
                tried.map(\.candidate), ["native/claude-fable-5", "native/claude-opus-5"]
            )
            XCTAssertTrue(tried.allSatisfy { !$0.available && !$0.why.isEmpty })
            XCTAssertEqual(oracle.session, 99)

            // The wire text is unchanged from the `configError` this case was
            // split out of — agents match on it.
            XCTAssertEqual(
                (error as? BrokerError)?.errorDescription,
                "Broker policy error: role \'planning\' has no candidate with headroom "
                    + "and forced-degraded is disabled for it"
            )
        }
    }

    /// The incident this case exists for: Max Quality dead-ended every capped
    /// `review` pick. It now returns a decision the harness can put in front of
    /// the user instead.
    func test_maxQualityReview_withEveryPoolCapped_returnsADegradedPickRatherThanThrowing() throws {
        var policy = BrokerPolicy.bundledDefault
        policy.apply(BrokerAgentProfile.maxQuality.rules)

        let decision = try decide(role: "review", policy: policy, oracle: cappedOracle)

        XCTAssertTrue(decision.degraded)
        XCTAssertEqual(decision.source, .forcedDegraded)
        XCTAssertFalse(
            (decision.degradedReason ?? "").isEmpty,
            "a degraded pick must say why, so the confirmation prompt can quote it"
        )
        XCTAssertTrue(decision.candidatesTried.allSatisfy { !$0.available })
    }

    func test_maxQualityPlanning_withEveryPoolCapped_stillThrowsTheNoHeadroomError() {
        var policy = BrokerPolicy.bundledDefault
        policy.apply(BrokerAgentProfile.maxQuality.rules)

        XCTAssertThrowsError(try decide(role: "planning", policy: policy, oracle: cappedOracle)) { error in
            guard case BrokerError.noHeadroom(let role, _, _) = error else {
                return XCTFail("expected noHeadroom, got \(error)")
            }
            XCTAssertEqual(role, "planning")
            XCTAssertEqual(
    ...
    func test_shippedMaxQualityDesignAndResearchUseCodexWhenClaudeIsCapped() throws {
        ...
        var policy = BrokerPolicy.bundledDefault
        policy.apply(preset.rules)
        let oracle = BrokerFixture.oracle(
            accounts: [BrokerFixture.account(session: 97, weekly: 95, sonnet: 97, fable: 97)],
            chatGPTRows: [.init(label: "Codex weekly", usedPercent: 10)],
            chatGPTConfigured: true
        )

        for role in ["design", "research"] {
            let decision = try decide(
                role: role,
                caller: BrokerPolicy.defaultCaller,
                policy: policy,
                oracle: oracle
            )

            XCTAssertEqual(decision.model, "codex/gpt-6-sol", role)
            XCTAssertEqual(decision.source, .policy, role)
            XCTAssertFalse(decision.degraded, role)
        }
    }

    func test_architectureAlias_usesPlanningForcedDegradedSetting() {
        var policy = BrokerPolicy.bundledDefault
        policy.apply(BrokerAgentProfile.maxQuality.rules)

        XCTAssertThrowsError(
            try decide(role: "architecture", policy: policy, oracle: cappedOracle)
        ) { error in
            guard case BrokerError.noHeadroom = error else {
                return XCTFail("expected noHeadroom, got \(error)")
            }
    ...
    func test_agentModelAliasesApplyToNativePicksOnly() throws {
        ...
        let nativeChildDecision = try decide(
            role: "execution",
            caller: "codex",
            policy: BrokerFixture.policy(
                roles: ["execution": ["codex/gpt-5.6-sol"]],
        ...
        XCTAssertEqual(nativeChildDecision.invocation, .agent(model: "gpt-5.6-sol"))
    }
    ...
    func test_bundledSeed_carriesTheDistilledRoleChains() {
        ...
        XCTAssertEqual(seed.roles["planning"]?.map(\.id), planningChain)
        XCTAssertNil(seed.roles["architecture"])
        XCTAssertEqual(seed.roles["design"]?.map(\.id), [
            "auto/claude-fable-5-1",
            "auto/gpt-6-astra",
            "auto/claude-opus-5-5",
            "auto/claude-sonnet-5",
        ])
        XCTAssertEqual(seed.roles["research"]?.map(\.id), [
            "auto/claude-fable-5-1",
            "auto/claude-opus-5-5",
            "auto/gpt-6-sol",
            "auto/claude-sonnet-5",
        ])
        // Review leads with Opus 5.5 because it is the highest-volume judgment
        // role and Anthropic's named review model. High effort limits quota burn,
        // while Fable remains ahead of Sonnet when Opus is capped.
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            seed.roles["verification"]?.map(\.id),
            [
                "auto/claude-sonnet-5",
                "auto/gpt-6-sol",
                "auto/claude-haiku-4-5-20251001",
    ...
    func test_bundledSeed_carriesTheResearchBackedEffortMatrix() {
        ...
        XCTAssertEqual(seed.roles["planning"]?.map(\.effort), [.xhigh, .low, nil, .xhigh])
        XCTAssertEqual(seed.roles["design"]?.map(\.effort), [nil, .low, .xhigh, .xhigh])
        XCTAssertEqual(seed.roles["explore"]?.map(\.effort), [nil, .medium, .low])
        XCTAssertEqual(seed.roles["research"]?.map(\.effort), [nil, .high, .medium, .high])
        XCTAssertEqual(
            seed.roles["review"]?.map(\.effort),
            [.high, .low, nil, .high],
            "review: Opus and Sonnet use high, while Fable stays adaptive"
        )
        XCTAssertEqual(seed.roles["execution"]?.map(\.effort), [.medium, .high, nil])
        XCTAssertEqual(seed.roles["heavy"]?.map(\.effort), [.medium, .xhigh, .xhigh, .high])
        XCTAssertEqual(seed.roles["standard"]?.map(\.effort), [.high, .low, nil])
        XCTAssertEqual(seed.roles["verification"]?.map(\.effort), [.high, .medium, nil])
    ...
    func test_bundledHeavyRole_isInvocableByCodex() throws {
        ...
        XCTAssertEqual(decision.model, "codex/gpt-6-astra")
    }
    ...
    func test_bundledSeed_carriesTheCallerRouteSetsT3WiringAndUnboundLanes() throws {
        ...
        XCTAssertEqual(seed.t3.instanceByModel["gpt-5.6-terra"], "codex")
        XCTAssertEqual(seed.t3.instanceByModel["gpt-5.6-luna"], "codex")
        XCTAssertEqual(seed.t3.instanceByModel["gpt-6-astra"], "codex")
        XCTAssertEqual(seed.t3.instanceByModel["gpt-6-sol"], "codex")
        XCTAssertEqual(seed.t3.instanceByModel["gpt-6-luna"], "codex")
        ...
        for model in ["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-6-astra", "gpt-6-sol", "gpt-6-luna"] {
            XCTAssertEqual(
                seed.models[model]?.quota,
                .chatGPT(labelContains: "codex weekly"),
                model
            )
            for lane in ["t3/\(model)", "codex/\(model)"] {
                XCTAssertNil(seed.usageLanes[lane], lane)
            }
        }
    ...
    func test_seedEndToEnd_designFallsFromACappedFablePoolToAstra() throws {
        let decision = try decide(
            role: "design",
            caller: "claude-code",
            policy: .bundledDefault,
        ...
        XCTAssertEqual(
            decision.invocation,
            .codexExec(model: "gpt-6-astra", effort: .low)
        )

        let skipped = try tried(decision, "native/claude-fable-5-1")
        XCTAssertFalse(skipped.available)
        XCTAssertTrue(skipped.why.contains("fable pool 95% >= 80%"), skipped.why)
    }

    // MARK: - Any-instance expansion

    func test_bundledTerraModelChoice_expandsToCodex() {
        var policy = BrokerPolicy.bundledDefault
        policy.usageLanes = [:]
        let expanded = BrokerEngine.expandingModelChoices(
            [BrokerCandidate(route: .auto, model: "gpt-5.6-terra", effort: .medium)],
            policy: policy,
            oracle: nil,
            now: BrokerFixture.now
        )
    ...
    func test_manifestOnlyModelSuppliesCodexRouteQuotaAndFutureEffort() throws {
        let presetID = UUID(uuidString: "77000000-0000-4000-A000-000000000001")!
        let json = """
        {
          "schema_version": 1,
          "presets": [{
            ...
            "rules": {
              "roles": {"standard": [
                {"id":"auto/gpt-7-nova","effort":"ultra"},
                "auto/claude-sonnet-5"
              ]},
              "callers": {"claude-code":{"routes":["native","codex","t3"]}},
              "instance_by_model": {"gpt-7-nova":"codex"},
              "agent_model_aliases": {"claude-sonnet-5":"sonnet"},
              "models": {"gpt-7-nova": {
                "quota":{"provider":"chatgpt","label_contains":"codex weekly"},
                "effort":{"levels":["low","ultra"],"default_label":"Default (ultra)"}
              }}
            }
          }]
        }
        """
        let manifest = try BrokerPresetManifest.decode(from: Data(json.utf8))
        var settings = BrokerSettings.default
        settings.updateRemotePresets(manifest.presets)
        settings.applyProfile(id: presetID)
        let oracle = BrokerFixture.oracle(
            chatGPTRows: [.init(label: "Codex Weekly", usedPercent: 20)],
            chatGPTConfigured: true
        )

        let decision = try decide(role: "standard", caller: "claude-code", policy: settings.policy, oracle: oracle)

        XCTAssertEqual(decision.model, "codex/gpt-7-nova")
        XCTAssertEqual(decision.invocation, .codexExec(model: "gpt-7-nova", effort: BrokerEffort(rawValue: "ultra")))
        XCTAssertFalse(decision.degraded)
        XCTAssertEqual(
            BrokerEffort.support(forModel: "gpt-7-nova", in: settings.policy.models).selectableLevels,
            [.low, BrokerEffort(rawValue: "ultra")!]
        )
        XCTAssertEqual(BrokerEffort.support(forModel: "gpt-7-nova").nilLabel, "Provider default")

        settings.policy.roles["standard"]?[0] = BrokerCandidate(
            route: .auto, model: "gpt-7-nova", effort: .xhigh
        )
        let clamped = try decide(
            role: "standard", caller: "claude-code", policy: settings.policy, oracle: oracle
        )
        XCTAssertNil(clamped.effort)
        ...
        let capped = try decide(
            ...
            oracle: BrokerFixture.oracle(
                chatGPTRows: [.init(label: "Codex Weekly", usedPercent: 95)],
                chatGPTConfigured: true
            )
        )
        XCTAssertEqual(capped.model, "native/claude-sonnet-5")
        XCTAssertTrue(try tried(capped, "codex/gpt-7-nova").why.contains("chatgpt"))

        settings.policy.models = [:]
        let withoutCatalog = BrokerEngine.expandingModelChoices(
            settings.policy.roles["standard", default: []],
            policy: settings.policy,
            oracle: oracle,
            now: BrokerFixture.now
        )
        XCTAssertFalse(withoutCatalog.contains { $0.id == "codex/gpt-7-nova" })
    }
    ...
    func test_distinctModelsPreserveConfiguredOrderWhenLowerRankHasMoreHeadroom() {
        let ids = ["native/claude-opus-5", "t3:secondary/claude-sonnet-5"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            t3Instances: [
                T3InstanceConfig(
        ...
        let oracle = BrokerFixture.oracle(accounts: [
            BrokerFixture.account(weekly: 80),
            BrokerFixture.account(
                id: "acct-secondary", label: "Secondary", isPrimary: false, weekly: 10
            ),
        ])
    ...
    func test_weeklyCeilingUsesExactUnroundedComparison() {
        let available = [84.999_999, 85, 85.000_001].map { weekly in
            BrokerEngine.evaluateNativePools(
                model: "claude-opus-5",
                thresholds: .default,
        ...
        XCTAssertEqual(available, [true, false, false])
    }

    // MARK: - Expiring quota

    /// Quota is use-it-or-lose-it. An account holding less headroom for 14 more
    /// hours is holding stock that is about to be thrown away; the account with
    /// more headroom for six more days is not. Spend the perishable one first.
    func test_equivalentRoutes_prefersTheAccountWhoseQuotaExpiresSoonest() {
        let ids = ["native/claude-opus-5", "t3:secondary/claude-opus-5"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            t3Instances: [
                T3InstanceConfig(
        ...
        let oracle = BrokerFixture.oracle(accounts: [
            ...
            BrokerFixture.account(
                id: "acct-secondary", label: "Secondary", isPrimary: false, weekly: 40,
                weeklyResetAt: BrokerFixture.now.addingTimeInterval(14 * 60 * 60)
            ),
        ])
        ...
        XCTAssertEqual(
            ranked.map(\.id),
            ["t3:secondary/claude-opus-5", "native/claude-opus-5"],
            "60 points expiring in 14h outrank 90 points that live another six days"
        )
    }

    /// Self-limiting: the expiring account only wins while it still holds
    /// enough to be worth spending. Once it is nearly done the durable account
    /// takes the lead again, so ranking never pins traffic to an empty lane.
    func test_equivalentRoutes_stopPreferringAnExpiringAccountOnceItIsSpent() {
        let ids = ["native/claude-opus-5", "t3:secondary/claude-opus-5"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            t3Instances: [
        ...
        let oracle = BrokerFixture.oracle(accounts: [
            ...
            BrokerFixture.account(
                id: "acct-secondary", label: "Secondary", isPrimary: false, weekly: 95,
                weeklyResetAt: BrokerFixture.now.addingTimeInterval(14 * 60 * 60)
            ),
        ])
        ...
        XCTAssertEqual(
            ranked.map(\.id),
            ["native/claude-opus-5", "t3:secondary/claude-opus-5"]
        )
    }

    /// The 5-hour session window is not durable quota: it recycles several
    /// times a day, so an imminent session reset must not make a lane look
    /// urgent when its weekly window is as far off as everyone else's.
    func test_equivalentRoutes_ignoreTheSessionWindowWhenRankingOnExpiry() {
        let ids = ["native/claude-opus-5", "t3:secondary/claude-opus-5"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            t3Instances: [
        ...
        let weeklyReset = BrokerFixture.now.addingTimeInterval(6 * 24 * 60 * 60)
        let oracle = BrokerFixture.oracle(accounts: [
            BrokerFixture.account(weekly: 5, weeklyResetAt: weeklyReset),
            BrokerFixture.account(
                id: "acct-secondary", label: "Secondary", isPrimary: false, weekly: 40,
                sessionResetAt: BrokerFixture.now.addingTimeInterval(60 * 60),
                weeklyResetAt: weeklyReset
            ),
        ])
        ...
        XCTAssertEqual(
            ranked.map(\.id),
            ["native/claude-opus-5", "t3:secondary/claude-opus-5"],
            "equal weekly expiry falls back to headroom; the session reset is not inventory"
        )
    }

    /// A missing reset is least urgent, never most: absent data can gate a
    /// lane out of the ranking but must never promote one.
    func test_equivalentRoutes_treatAnUnknownResetAsTheLeastUrgentQuota() {
        let ids = ["native/claude-opus-5", "t3:secondary/claude-opus-5"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            t3Instances: [
        ...
        let oracle = BrokerFixture.oracle(accounts: [
            BrokerFixture.account(session: 0, weekly: 0),
            BrokerFixture.account(
                id: "acct-secondary", label: "Secondary", isPrimary: false, weekly: 40,
                weeklyResetAt: BrokerFixture.now.addingTimeInterval(14 * 60 * 60)
            ),
        ...
        XCTAssertEqual(
            ranked.map(\.id),
            ["t3:secondary/claude-opus-5", "native/claude-opus-5"],
            "an untimed full account does not outrank one whose points expire today"
        )
    }

    /// A ChatGPT row whose window length the server omitted is NOT durable
    /// quota. The 5-hour row is precisely the one that arrives without a
    /// length, and a window two hours from recycling must never divide the
    /// score and win the ranking.
    func test_equivalentRoutes_ignoreAChatGPTWindowOfUnstatedLength() {
        let ids = ["codex/gpt-5.6-sol", "t3:secondary/gpt-5.6-sol"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            t3Instances: [T3InstanceConfig(id: "secondary", name: "Secondary")],
            usageLanes: [
                "codex/gpt-5.6-sol": .chatGPT(labelContains: "alpha"),
                "t3:secondary/gpt-5.6-sol": .chatGPT(labelContains: "beta"),
            ]
        )
        let oracle = BrokerFixture.oracle(chatGPTRows: [
            OracleSnapshot.ChatGPTRow(
                label: "alpha weekly", usedPercent: 5,
                resetAt: BrokerFixture.now.addingTimeInterval(6 * 24 * 60 * 60),
                windowSeconds: 7 * 24 * 60 * 60
            ),
            OracleSnapshot.ChatGPTRow(
                label: "beta 5h", usedPercent: 40,
                resetAt: BrokerFixture.now.addingTimeInterval(2 * 60 * 60),
                windowRole: .chatGPT5h
            ),
            OracleSnapshot.ChatGPTRow(
                label: "beta weekly", usedPercent: 40,
                resetAt: BrokerFixture.now.addingTimeInterval(6 * 24 * 60 * 60),
                windowSeconds: 7 * 24 * 60 * 60
            ),
        ])
        ...
        XCTAssertEqual(
            ranked.map(\.id),
            ["codex/gpt-5.6-sol", "t3:secondary/gpt-5.6-sol"],
            "a 5-hour window of unstated length is not quota that expires unspent"
        )
    }

    /// The ChatGPT branch does rank on a durable window when the row states one.
    func test_equivalentRoutes_prefersTheChatGPTAccountWhoseWeeklyExpiresSoonest() {
        let ids = ["codex/gpt-5.6-sol", "t3:secondary/gpt-5.6-sol"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            t3Instances: [T3InstanceConfig(id: "secondary", name: "Secondary")],
        ...
        let oracle = BrokerFixture.oracle(chatGPTRows: [
            ...
            OracleSnapshot.ChatGPTRow(
                label: "beta weekly", usedPercent: 40,
                resetAt: BrokerFixture.now.addingTimeInterval(14 * 60 * 60),
                windowSeconds: 7 * 24 * 60 * 60
            ),
        ])
        ...
        XCTAssertEqual(
            ranked.map(\.id),
            ["t3:secondary/gpt-5.6-sol", "codex/gpt-5.6-sol"]
        )
    }

    /// A reset already in the past is a reading about to be replaced, not the
    /// most urgent quota on the machine: it falls back to the full horizon.
    func test_equivalentRoutes_treatAPastResetAsTheLeastUrgentQuota() {
        let ids = ["native/claude-opus-5", "t3:secondary/claude-opus-5"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            t3Instances: [
        ...
        let oracle = BrokerFixture.oracle(accounts: [
            BrokerFixture.account(
                weekly: 5,
                weeklyResetAt: BrokerFixture.now.addingTimeInterval(-60 * 60)
            ),
            BrokerFixture.account(
                id: "acct-secondary", label: "Secondary", isPrimary: false, weekly: 40,
                weeklyResetAt: BrokerFixture.now.addingTimeInterval(14 * 60 * 60)
        ...
        XCTAssertEqual(
            ranked.map(\.id),
            ["t3:secondary/claude-opus-5", "native/claude-opus-5"],
            "a stale reset must not promote the lane it belongs to"
        )
    }

    /// Turning the preference off restores the historical ordering exactly.
    func test_equivalentRoutes_rankOnHeadroomAloneWhenExpiryPreferenceIsOff() {
        var thresholds = BrokerThresholds.default
        thresholds.preferExpiringQuota = false
        let ids = ["native/claude-opus-5", "t3:secondary/claude-opus-5"]
        let policy = BrokerFixture.policy(
            roles: ["planning": ids],
            thresholds: thresholds,
    ...
    /// End to end through `decide`: the expiring account is the one dispatched.
    func test_decide_dispatchesTheExpiringAccountBeforeTheDurableOne() throws {
        let policy = BrokerFixture.policy(
            roles: ["planning": ["native/claude-opus-5", "t3:secondary/claude-opus-5"]],
            callers: ["claude-code": BrokerCallerPolicy(routes: [.native, .t3])],
            t3Instances: [
                T3InstanceConfig(
                    id: "secondary", name: "Secondary", boundAccountId: "acct-secondary",
                    detectedModels: ["claude-opus-5"]
                )
            ]
        )
        let oracle = BrokerFixture.oracle(accounts: [
        ...
        XCTAssertEqual(decision.model, "t3:secondary/claude-opus-5")
    }

    /// Settings saved before this preference existed decode with it on: the
    /// broker adopts the better default rather than pinning old behaviour.
    func test_thresholds_decodeExpiryPreferenceOnWhenTheKeyIsAbsent() throws {
        let json = Data(#"{"session_pct": 90, "weekly_pct": 85}"#.utf8)
        let decoded = try JSONDecoder().decode(BrokerThresholds.self, from: json)
        XCTAssertTrue(decoded.preferExpiringQuota)
    }

    func test_thresholds_roundTripTheExpiryPreference() throws {
        var thresholds = BrokerThresholds.default
        thresholds.preferExpiringQuota = false
        let data = try JSONEncoder().encode(thresholds)
        let decoded = try JSONDecoder().decode(BrokerThresholds.self, from: data)
        XCTAssertFalse(decoded.preferExpiringQuota)
    }
    ...
    func test_winnersEffort_reachesBothTheDecisionAndTheInvocation() throws {
        ...
        let codexNativeChild = try decide(
            caller: "codex",
            policy: effortPolicy([
                BrokerCandidate(route: .codex, model: "gpt-5.6-sol", effort: .low)
            ]),
        ...
        XCTAssertEqual(codexNativeChild.invocation, .agent(model: "gpt-5.6-sol", effort: .low))
    }
    ...
    func test_staleChatGPTOracle_isNotTreatedAsUnconfigured() throws {
        ...
        //
        // They are no longer worded alike. Both still keep the rank-1
        // candidate and both are still degraded, but only `.stale` resolves on
        // its own, so only `.stale` reads as missing data; `.error` names the
        // failing poll, because the caller has to act on it rather than wait.
        // Neither may ever read as an account this machine does not have.
        for state in [BrokerQuotaState.stale, .error] {
            ...
            let why = try tried(decision, "codex/gpt-5.6-sol").why
            XCTAssertFalse(
                why.contains("no configured quota source"),
                "\(state) proves the account exists and must never read as an absent one"
            )
            switch state {
            case .stale:
                XCTAssertTrue(
                    why.contains("no fresh data"),
                    "stale data resolves on the next poll, so it reads as missing data"
                )
            case .error:
                XCTAssertTrue(
                    why.contains("ChatGPT usage poll is failing"),
                    "a failed poll needs a human, so it must name the fault instead of the symptom"
                )
                XCTAssertFalse(
                    why.contains("no fresh data"),
                    "reporting a failed poll as missing data is the conflation this test now guards"
                )
            default:
                XCTFail("unexpected state \(state) in this fixture")
            }
        }
    }

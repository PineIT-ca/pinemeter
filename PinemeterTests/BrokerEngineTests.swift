...
final class BrokerEngineTests: XCTestCase {
    ...
    func testEveryQuotaBackedHardCapPacesAfterCapWithKnownWindow() throws {
        ...
        // The server-reported window wins over the role's implied duration:
        // the Codex-Spark meter is a 5h window whose role would otherwise be
        // paced as weekly, hiding a burn that exhausts it within the hour.
        let sparkPolicy = BrokerFixture.policy(
            roles: ["execution": ["codex/gpt-5.6-sol"]],
            usageLanes: ["codex/gpt-5.6-sol": .chatGPT(labelContains: nil)]
        )
        let sparkReset = BrokerFixture.now.addingTimeInterval(4.5 * 60 * 60)
        let sparkFiveHour = try decide(
            role: "execution",
            policy: sparkPolicy,
            oracle: BrokerFixture.oracle(chatGPTRows: [
                .init(
                    ...
                    usedPercent: 50,
                    resetAt: sparkReset,
                    windowRole: .chatGPTCodexSpark,
                    windowSeconds: 5 * 60 * 60
                )
            ])
        )
        XCTAssertTrue(try tried(sparkFiveHour, "codex/gpt-5.6-sol").why.contains("pacing"))

        let sparkRoleOnly = try decide(
            role: "execution",
            policy: sparkPolicy,
            oracle: BrokerFixture.oracle(chatGPTRows: [
                .init(
        ...
        XCTAssertTrue(
            try tried(sparkRoleOnly, "codex/gpt-5.6-sol").available,
            "Rows cached before window_seconds existed still fall back to the role"
        )
    ...
    func test_explicitOverrideSelectsOneConfiguredExecutableCandidate() throws {
        let policy = BrokerFixture.policy(
            roles: [
        ...
        let oracle = BrokerFixture.oracle(chatGPTRows: [
            .init(label: "Codex weekly", usedPercent: 14, resetAt: nil, windowRole: .chatGPTWeekly),
        ])
    ...
    func test_forcedDegradedCodexCaller_staysInHarnessWhenReachableT3SharesThePacedLane() throws {
        ...
        XCTAssertEqual(decision.source, .forcedDegraded)
        XCTAssertEqual(decision.model, "codex/gpt-5.6-sol")
        XCTAssertEqual(decision.invocation, .codexExec(model: "gpt-5.6-sol"))
        XCTAssertTrue(decision.candidatesTried.allSatisfy { !$0.available })

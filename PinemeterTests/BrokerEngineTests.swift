...
final class BrokerEngineTests: XCTestCase {
    ...
    func test_explicitOverride_resolvesTheArchitectureAliasAndEchoesRequestedRole() throws {
        let decision = try BrokerEngine.decideOverride(
            role: "architecture",
            caller: nil,
            overrideCandidate: "auto/claude-opus-5",
    ...
    func test_bundledSeed_carriesTheDistilledRoleChains() {
        ...
        for role in ["design", "research"] {
            XCTAssertEqual(
                seed.roles[role]?.map(\.id),
                ["auto/claude-fable-5-1", "auto/claude-opus-5", "auto/claude-sonnet-5"],
                role
            )
        }
        // Review leads with Opus because it is the highest-volume judgment role
        // and Anthropic's named review model. High effort limits quota burn,
    ...
    func test_bundledSeed_carriesTheResearchBackedEffortMatrix() {
        ...
        XCTAssertEqual(seed.roles["planning"]?.map(\.effort), [.xhigh, nil, .xhigh])
        XCTAssertEqual(seed.roles["design"]?.map(\.effort), [nil, .xhigh, .xhigh])
        XCTAssertEqual(seed.roles["explore"]?.map(\.effort), [nil, .medium, .low])
        XCTAssertEqual(seed.roles["research"]?.map(\.effort), [nil, .high, .high])
        XCTAssertEqual(
        ...
        XCTAssertEqual(seed.roles["heavy"]?.map(\.effort), [.xhigh, .xhigh, .xhigh])
    ...
    func test_seedEndToEnd_designFallsFromACappedFablePoolToTheNextModel() throws {
        let decision = try decide(
            role: "design",
            caller: "claude-code",
            policy: .bundledDefault,

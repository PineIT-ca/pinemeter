...
final class BrokerEngineTests: XCTestCase {
    ...
    func test_bundledSeed_carriesTheCallerRouteSetsT3WiringAndUnboundLanes() throws {
        ...
        XCTAssertEqual(seed.t3Instances.map(\.id), ["claudeAgent", "claude_secondary", "codex"])
        XCTAssertTrue(
        ...
        XCTAssertEqual(
            seed.usageLanes["t3:claude_secondary/claude-fable-5"],
            .claudeAccount(accountId: nil, labelContains: nil, isPrimary: nil),
            "the secondary lane resolves through the instance's bound account, seeded unbound"
        )
    ...
    func test_seedEndToEnd_planningFallsFromACappedFablePoolToTheSecondaryT3Lane() throws {
        let decision = try decide(
            role: "planning",
            caller: "claude-code",
        ...
        XCTAssertEqual(decision.source, .policy)
        XCTAssertTrue(decision.degraded, "the secondary lane ships unbound, so the pick is quota-blind")
        XCTAssertEqual(
            decision.invocation,
            .t3Dispatch(model: "claude-fable-5", instanceId: "claude_secondary")
        )

...
final class BrokerEngineTests: XCTestCase {
    ...
    func test_shippedMaxQualityDesignAndResearchUseCodexWhenClaudeIsCapped() throws {
        ...
        for role in ["design", "research"] {
            ...
            XCTAssertEqual(decision.model, "codex/gpt-5.6-sol", role)
    ...
    func test_bundledSeed_carriesTheResearchBackedEffortMatrix() {
        ...
        XCTAssertEqual(seed.roles["research"]?.map(\.effort), [nil, .high, .high, .high])
        XCTAssertEqual(
        ...
        XCTAssertEqual(seed.roles["heavy"]?.map(\.effort), [.medium, .xhigh, .xhigh, .xhigh])
        XCTAssertEqual(seed.roles["standard"]?.map(\.effort), [.high, .medium, .medium, nil])
        XCTAssertEqual(seed.roles["verification"]?.map(\.effort), [.high, .high, nil])

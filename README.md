...
| **ChatGPT / Codex** | Codex weekly limit plus per-model meters (e.g. Spark) and any additional plan limit rows | Existing ChatGPT browser session |
...
- **Scriptable multi-provider data**: the current Claude, ChatGPT/Codex, and Gemini quota state is exported for shell prompts, status lines, and local dashboards
...
- GitHub is contacted for signed release checks and downloads. Pinemeter also contacts the configured HTTPS preset-manifest URL while the local broker is enabled, or after an explicit manifest refresh
...
Pinemeter atomically updates `~/.pinemeter/usage.json` from its in-memory refresh results. Reading this file makes no provider request. It always has `schema_version: 1`, an ISO-8601 `generated_at`, ordered `claude_accounts`, `chatgpt`, and `gemini`:
...
```json
{
  ...
  "claude_accounts": [{
    ...
    "is_primary": true,
    "state": "fresh",
    "usage": { "session_usage": { "utilization": 42, "reset_at": "2026-08-04T19:00:00Z" }, "weekly_usage": { "utilization": 10, "reset_at": "2026-08-10T15:00:00Z" }, "last_updated": "2026-08-04T14:59:00Z" }
  }],
  "chatgpt": { "label": "ChatGPT", "state": "fresh", "last_updated": "2026-08-04T14:59:00Z", "rows": [{ "label": "Codex 5h", "used_percent": 33, "reset_at": "2026-08-04T19:00:00Z" }] },
  "gemini": { "label": "Gemini", "state": "unavailable", "quota": null },
  "session_usage": { "utilization": 42, "reset_at": "2026-08-04T19:00:00Z" },
  "weekly_usage": { "utilization": 10, "reset_at": "2026-08-10T15:00:00Z" },
  "last_updated": "2026-08-04T14:59:00Z"
}
```

`claude_accounts` keeps the primary account first, then the connected account order. `chatgpt.rows` keeps the provider row order. `usage`, `last_updated`, `reset_at`, and `gemini.quota` are nullable where applicable.

Every provider/account has one `state`:

- `fresh`: data is within Pinemeter's refresh staleness threshold.
- `stale`: data exists but is older than that threshold.
- `error`: Pinemeter has a current refresh error, even if it retains last-known data.
- `unavailable`: no normalized quota data is available.

Existing primary-Claude consumers remain compatible: when primary Claude usage exists, the top-level `session_usage`, `weekly_usage`, optional `sonnet_usage` and `fable_usage`, and `last_updated` fields are unchanged and a `UsageData` decoder can ignore the new fields. The snapshot contains normalized labels, percentages, reset times, and timestamps only. It never includes credentials, credential diagnostics, raw provider responses, or error text.

Read a quota with one local operation:

```bash
jq '.chatgpt.rows[] | select(.label == "Codex 5h") | .used_percent' \

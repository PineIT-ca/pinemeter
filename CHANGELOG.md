...
- Export a versioned `~/.pinemeter/usage.json` snapshot covering all connected Claude accounts, ChatGPT/Codex quota rows, and Gemini state while preserving the legacy primary-Claude fields.
...
- Label ChatGPT/Codex quota rows as Codex quotas (Codex and Spark columns), classifying each window by its own duration instead of assuming primary is the 5-hour window.

### Security

- Stop writing the Claude session key to disk. The menu bar's WebView injected it as a cookie with a 30-day expiry into WebKit's persistent store, outside the Keychain, where nothing purged it on disconnect. The WebView now uses a non-persistent store, and any cookie left by an earlier build is purged on launch.

### Fixed

- Never fail a whole usage response over one malformed reset timestamp; utilization is kept and the reset time falls back to the pacing window.
- Keep the Fable meter alive through claude.ai's usage API migration to the `limits` array: the flat `five_hour`/`seven_day` keys may now be null, the Fable limit is matched by stable model id as well as its display name, and reset timestamps parse with or without fractional seconds.
- Restore the Codex meter after ChatGPT's usage API nested additional rate limits under `rate_limit` with new `limit_name`/`metered_feature` labels; the old flat shape still decodes.
- Collapse duplicate quota roles into a single menu bar meter if the server ever maps two windows to the same bucket.

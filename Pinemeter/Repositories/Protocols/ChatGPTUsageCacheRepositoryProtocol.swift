...
protocol ChatGPTUsageCacheRepositoryProtocol: Actor {
    /// Persist the last-good usage snapshot, replacing any prior one.
    func save(_ data: ChatGPTUsageData) async

    /// Load the last-good usage snapshot, ignoring staleness -- callers
    /// compare `lastUpdated` against the broker's own freshness thresholds.
    /// Returns `nil` if nothing is persisted or the file is unreadable/corrupt.
    func load() async -> ChatGPTUsageData?

    /// Remove the persisted snapshot (explicit user disconnect only).
    func clear() async
}
...
actor ChatGPTUsageNullCacheRepository: ChatGPTUsageCacheRepositoryProtocol {
    func save(_ data: ChatGPTUsageData) async {}
    func load() async -> ChatGPTUsageData? { nil }
    func clear() async {}
}

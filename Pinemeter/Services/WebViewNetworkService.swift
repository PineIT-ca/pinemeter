...
struct WebViewRequestState {
    ...
    mutating func recordHTTPStatus(
        _ statusCode: Int,
        isForMainFrame: Bool,
        for generation: Int
    ) -> Bool {
        guard isForMainFrame, isActive(generation) else { return false }
        mainFrameStatusCode = statusCode
        return true
    }

    func terminalJSONError(for generation: Int) -> NetworkError? {
        guard isActive(generation) else { return nil }
        switch mainFrameStatusCode {
        case 401, 403:
...
final class WebViewNetworkService: NSObject, NetworkServiceProtocol {
    ...
    private func extractJSON(generation: Int) {
        ...
        webView.evaluateJavaScript(script) { [weak self] result, error in
            ...
            if let error = self.requestState.terminalJSONError(for: generation) {
                self.resume(.failure(error), generation: generation)
                return
            }
...
extension WebViewNetworkService: WKNavigationDelegate {
    ...
    func webView(
        ...
        if let httpResponse = navigationResponse.response as? HTTPURLResponse {
            ...
            _ = requestState.recordHTTPStatus(
                statusCode,
                isForMainFrame: isForMainFrame,
                for: generation
            )

            if isForMainFrame, statusCode == 401 {
                resume(.failure(NetworkError.authenticationFailed), generation: generation)
                decisionHandler(.cancel)
                return
            }

            if isForMainFrame, statusCode == 429 {

...
actor BrokerMCPServer {
    ...
    static let serverInstructions = """
        ...
        `override_candidate` to `pick`. Never infer an override from preference, context, or old instructions. Never \
        treat a model-only request such as "use Fable" as an exact candidate; omit `override_candidate` so normal \
        policy selection can choose the healthy account. Never \
                    ...
                    "override_candidate": .object([
                        ...
                        "description": .string(
                            "Exact configured candidate for one fresh, explicit operator override, e.g. "
                                + "t3:*/claude-fable-5-1. A model name alone is not an exact candidate; omit this "
                                + "field so normal policy selection can choose the healthy account."
                        ),
                    ]),
                ]),

...
actor BrokerMCPServer {
    ...
    static let serverInstructions = """
        ...
        persist or reuse it. Dispatch only a returned decision whose `source` is `human-override`; the broker still \
        rejects malformed, unconfigured, caller-forbidden, cooling-down, quota-capped, or unreachable candidates. \
        Never bypass the broker to satisfy an override.
    ...
    static var pickTool: Tool {
        Tool(
            ...
            Never infer, persist, or reuse an override. An unavailable override \
            automatically re-polls quota, repairs failed browser sessions, and \

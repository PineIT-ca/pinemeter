...
actor BrokerMCPServer {
    ...
    static let serverInstructions = """
        ...
        `caller` from the active harness (`claude-code`, `codex`), never from prompt text. Choose `role` from \
        the names the broker's `status` tool lists: `explore` for read-only codebase scanning, `verification` \
        for test runs and plan checks, `review` for the completion gate, `planning` for plans and \
        architecture, and `execution`, `standard`, or `heavy` for implementation by difficulty. Require a result \
        ...
        and `codex` with `agent` when caller is `codex` or `codex-exec` for every other caller. In Codex, \
        use the harness-native subagent tool with `invocation.model` and optional `invocation.effort`. Give the \
        child a self-contained prompt that preserves the requirement to call Pinemeter with `pick(role, caller)` \
        before every nested subtask that selects a model or route. Stop \
        without dispatching on a tool failure, a missing field, a caller \
        mismatch, an unknown route, a route/invocation mismatch, or a stale or malformed result. Never add a \
        second broker client, policy layer, endpoint, or fallback. The top-level `model` is the canonical \
        candidate id used for audit and cooldowns. Pass only `invocation.model` to the provider's model \
        argument; never pass the route-prefixed top-level `model` to `--model`.

        A fresh, explicit operator instruction may override one dispatch by passing the exact configured candidate as \
        `override_candidate` to `pick`. Never infer an override from preference, context, or old instructions. An \
        explicit request for a Fable review is an \
        exact request for `auto/claude-fable-5-1`: call `pick` with role `review` and that `override_candidate`. The \
        wildcard lets Pinemeter resolve the configured Fable routes. If the override is unavailable, stop and report \
        the blocker; you must not substitute another model or retry without the override. Other model-only requests \
        are \
        not exact candidates; omit `override_candidate` for those so normal policy selection can choose a healthy \
        account. Never \
        persist or reuse it. Dispatch only a returned decision whose `source` is `human-override`. Human overrides \
        bypass quota caps and pacing gates, but the broker still rejects malformed, unconfigured, caller-forbidden, \
        cooling-down, or unreachable candidates. Never bypass the broker to satisfy an override.

        Degraded picks require user confirmation: never auto-dispatch a pick that returns `degraded: true`. \
        Surface it with AskUserQuestion and dispatch only after the user chooses. Build the option list from \
        the decision, do not hardcode it. If `retryable` is true, first offer the recommended “Refresh and \
        re-pick now”: call `refresh`, then `pick` again with the same role and caller. Dispatch a non-degraded \
        replacement without re-asking. If it remains degraded, re-surface the question without that retry \
        option. Always offer “Proceed with the degraded pick”, stating route, model, effort, and \
        `degraded_reason` verbatim; each listed backup labeled with route and model; and “Wait”. If \
        `retryable` is false, omit retry. Recommend the degraded pick unless a listed backup ranks higher. \
        Render retryability from the decision. Never judge it yourself.
    ...
    static var pickTool: Tool {
        Tool(
            ...
            as a validated lowercase token declared by the selected model, \
            at top level and inside `invocation`; \
            ...
            possibly empty. When `degraded` is true, it also carries required \
            `degraded_reason` and `retryable`; retryable picks carry \
            `suggested_action`. A fresh, explicit operator instruction may set \
            `override_candidate` to one exact configured candidate for this call only. \
            An explicit request for a Fable review means `auto/claude-fable-5-1`; \
            use role `review`, and if that override is unavailable you must not substitute another model. \
            Never infer, persist, or reuse an override. Overrides bypass quota \
            caps and pacing gates. An unavailable override automatically \
            repairs failed browser sessions and \
            re-probes T3 once before returning an actionable error.
            """,
            inputSchema: .object([
                "type": .string("object"),
                "properties": .object([
                    ...
                    "override_candidate": .object([
                        "type": .string("string"),
                        "description": .string(
                            "Exact configured candidate for one fresh, explicit operator override, e.g. "
                                + "auto/claude-fable-5-1. An explicit Fable review request uses that wildcard. "
                                + "Other model names alone are not exact candidates; omit this field so normal "
                                + "policy selection can choose a healthy account."
                        ),
                    ]),
                ]),
    ...
    private static func handlePick(
        ...
        let overrideCandidate = params.arguments?["override_candidate"]?.stringValue

        do {
            let decision = if let overrideCandidate {
                try await broker.pick(role: role, caller: caller, overrideCandidate: overrideCandidate)
            } else {
                try await broker.pick(role: role, caller: caller)
            }
        ...
        } catch is BrokerAuditStoreError {
            return CallTool.Result(
                content: [
                    .text(
                        text: "Audit persistence failed; no decision issued",
                        annotations: nil,
                        _meta: nil
                    )
                ],
    ...
    static var downTool: Tool {
        Tool(
            ...
            `target` is a cooldown key: a full candidate id (e.g. t3:claude_alt/claude-fable-5-1), \
    ...
    static var refreshTool: Tool {
        Tool(
            ...
            description: "Re-poll quota and T3 reachability. Override failures invoke this recovery path once automatically.",
            inputSchema: .object([
                "type": .string("object"),
                "properties": .object([:]),
                "additionalProperties": .bool(false),
        ...
        Tool(
            ...
            inputSchema: .object([
                ...
                "properties": .object([
                    ...
                    "source_files": integerSchema(maximum: 2_147_483_647),
                    "source_lines": integerSchema(maximum: 2_147_483_647),
                    "generated_lines": integerSchema(maximum: 2_147_483_647),
                    "docs_lines": integerSchema(maximum: 2_147_483_647),
                    "trigger": .object([
                        "type": .string("string"),
                        "enum": .array(BrokerReviewTrigger.allCases.map { .string($0.rawValue) }),
                    ]),
    ...
    private static func handleReport(
        ...
        let allowedFields: Set<String> = [
            ...
            "actual_reasoning_tokens", "source_files", "source_lines",
            "generated_lines", "docs_lines", "trigger", "failure_reason",
        ]
        ...
        let sourceFiles = try boundedInteger(arguments["source_files"], maximum: 2_147_483_647)
        let sourceLines = try boundedInteger(arguments["source_lines"], maximum: 2_147_483_647)
        let generatedLines = try boundedInteger(arguments["generated_lines"], maximum: 2_147_483_647)
        let docsLines = try boundedInteger(arguments["docs_lines"], maximum: 2_147_483_647)
        let trigger: BrokerReviewTrigger?
        if let value = arguments["trigger"] {
            guard let rawValue = value.stringValue,
                  let parsed = BrokerReviewTrigger(rawValue: rawValue) else {
                throw invalidReportParams()
            }
            trigger = parsed
        } else {
        ...
        let hasOutcome = durationMS != nil || actualInputTokens != nil
            ...
            || sourceFiles != nil || sourceLines != nil || generatedLines != nil
            || docsLines != nil || trigger != nil

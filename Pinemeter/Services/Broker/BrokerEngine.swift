...
enum BrokerEngine {
    ...
    /// Ranks every route for the same model by live quota headroom while
    /// preserving the configured model priority.
    ///
    ...
    static func rankingEquivalentCandidatesByHeadroom(
        ...
        return modelOrder.flatMap { model in
            ...
            return group.enumerated().sorted { left, right in
                let leftHeadroom = headroom(for: left.element, policy: policy, oracle: oracle, now: now)
                let rightHeadroom = headroom(for: right.element, policy: policy, oracle: oracle, now: now)
                switch (leftHeadroom, rightHeadroom) {

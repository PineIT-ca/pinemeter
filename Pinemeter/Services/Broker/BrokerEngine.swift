...
enum BrokerEngine {
    ...
    static func decide(
        ...
        // concrete instance. Equivalent routes for the same model are then
        // ranked by live headroom, without changing model priority.
        let expanded = expandingAnyInstance(chain, policy: policy, oracle: oracle, now: now)

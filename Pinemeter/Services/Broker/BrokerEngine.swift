...
enum BrokerError: LocalizedError, Equatable {
    ...
    case overrideUnavailable(candidate: String, reasons: [String], recoveryAttempted: Bool)
    /// Every candidate was over its ceiling and the role forbids a forced
    /// degraded pick. Split out of `configError` because it is an exhaustion,
    /// not a misconfiguration: it can heal on its own, so `BrokerService`
    /// re-polls quota once before surfacing it, and it is the only pick
    /// failure worth persisting as a block. The rejection reasons and the
    /// oracle block that produced them ride along so that trace can be
    /// written without re-running the walk.
    case noHeadroom(role: String, candidatesTried: [BrokerCandidateTried], oracle: BrokerOracleBlock)
    ...
    var errorDescription: String? {
        ...
        case .overrideUnavailable(let candidate, let reasons, let recoveryAttempted):
            let retry = recoveryAttempted ? " after automatic refresh" : ""
            let details = reasons.joined(separator: "; ")
            let normalized = details.lowercased()
            let action: String
            if normalized.contains("cooldown") {
                action = "Wait for the reported cooldown to expire, or omit override_candidate to use a healthy route."
            } else if normalized.contains("pacing risk") || normalized.contains("% >=") {
                action = "Wait for quota headroom, or omit override_candidate to use a healthy route."
            } else if normalized.contains("t3 signal") || normalized.contains("t3 unreachable") {
                action = "Open T3 Code, then retry."
            } else {
                action = "Open Pinemeter, choose Rescan Browsers, then retry."
            }
            return "Override candidate '\(candidate)' unavailable\(retry): \(details). \(action)"
        case .noHeadroom(let role, _, _):
            // Byte-identical to the `configError` text this case was split out
            // of, prefix included: agents read this message off the MCP wire
            // and it is quoted in the routing instructions, so giving the case
            // a name must not reword what the caller is told.
            return "Broker policy error: role '\(role)' has no candidate with headroom "
...
enum BrokerEngine {
    /// Selects one exact configured candidate for one call. Normal ranking and
    /// quota gates are skipped, while caller, cooldown, and reachability gates
    /// still apply.
    static func decideOverride(
        ...
        let resolvedRole = policy.resolveRole(role)
        guard let roleChain = policy.roles[resolvedRole] else {
        ...
        let orderedRoles = [resolvedRole] + policy.roles.keys.sorted().filter { $0 != resolvedRole }
        let configured = orderedRoles.flatMap { policy.roles[$0] ?? [] }
        let candidates: [BrokerCandidate]
        if parsed.route == .auto {
            guard configured.contains(where: { $0.id == requested }) else {
                throw BrokerError.configError("override candidate '\(requested)' is not configured")
            }
            candidates = expandingModelChoices([parsed], policy: policy, oracle: oracle, now: now)
        } else if parsed.isAnyInstance {
            guard configured.contains(where: { $0.id == requested }) else {
                throw BrokerError.configError("override candidate '\(requested)' is not configured")
            }
            candidates = expandingAnyInstance([parsed], policy: policy, oracle: oracle, now: now)
        } else {
            let expanded = orderedRoles
                .flatMap {
                    expandingModelChoices(policy.roles[$0] ?? [], policy: policy, oracle: oracle, now: now)
                }
            guard let candidate = expanded.first(where: { $0.id == requested }) else {
                throw BrokerError.configError("override candidate '\(requested)' is not configured")
            }
        ...
        for candidate in candidates {
            ...
            } else if candidate.route == .t3 {
                let instance = policy.resolvedInstance(for: candidate)
                if let liveness = t3[instance], liveness.reachable {
                    evaluation = .available("quota gates bypassed")
                } else if let liveness = t3[instance] {
                    evaluation = .unavailable("t3 unreachable: \(liveness.why)")
                } else {
                    evaluation = .unavailable(
            ...
            let dispatched = candidate.clampingEffortToModelSupport(in: policy.models)
            ...
            return BrokerDecision(
                ...
                invocation: invocation(for: dispatched, caller: resolvedCaller, policy: policy),
                reason: reason,
        ...
        if tried.allSatisfy(\.callerFiltered) {
            throw BrokerError.configError(
                "override rejected: \(tried.map(\.why).joined(separator: "; "))"
            )
        }
        throw BrokerError.overrideUnavailable(
            candidate: requested,
            reasons: tried.map(\.why),
    ...
    static func decide(
        ...
        let resolvedRole = policy.resolveRole(role)
        guard let chain = policy.roles[resolvedRole] else {
            throw BrokerError.unknownRole(role: role, known: policy.roles.keys.sorted())
        }
        ...
        // A role ranks models, not transports. Expand each model into every
        // execution route this Mac can actually identify, then rank equivalent
        // routes by quota headroom without changing model priority.
        let expanded = expandingModelChoices(chain, policy: policy, oracle: oracle, now: now)
        let candidates = rankingEquivalentCandidatesByHeadroom(
            expanded,
            policy: policy,
            oracle: oracle,
            now: now
        ...
        let quotaBlindExpansions: Set<String> = Set(
            candidates
                .filter { $0.route == .t3 && !chain.contains($0) && policy.usageLane(for: $0) == nil }
                .map(\.id)
        )
        guard !candidates.isEmpty else {
            throw BrokerError.configError(
                "role '\(role)' has no confirmed execution route for its configured models"
            )
        }
        ...
        // A candidate whose quota is real and under every ceiling but is being
        // spent ahead of pace. Held back so a lane pacing normally can win,
        // and taken ahead of `unverified`: known headroom spent fast beats
        // headroom nobody can see.
        var paced: (candidate: BrokerCandidate, evaluation: Evaluation)?
        ...
        for candidate in candidates {
            ...
            if evaluation.available, !evaluation.failOpen, !evaluation.paced,
            ...
            tried.append(
                BrokerCandidateTried(
                    ...
                    // better follows it.
                    why: heldBackNote(for: evaluation)
                )
            )
            guard evaluation.available else { continue }
            ...
            if evaluation.paced {
                // Same rule for equally paced candidates: rank picks the first.
                if paced == nil { paced = (candidate, evaluation) }
                continue
            }
            chosen = (candidate, evaluation)
            break
        }
        // Nothing pacing normally: a held-back candidate is better than
        // forcing, and a lane with measured headroom beats an unmeasured one.
        chosen = chosen ?? paced ?? unverified
        ...
        if let chosen {
            ...
            guard policy.allowsForcedDegraded(role: role) else {
                throw BrokerError.noHeadroom(role: role, candidatesTried: tried, oracle: block)
            }
            ...
            // A forced Codex fallback must stay in the current harness when it
            // can. Re-dispatching the same degraded lane through T3 only starts
            // another approval cycle. Healthy picks keep configured order.
            let forcedCandidates = resolvedCaller == "codex"
                ? candidates.filter { $0.route == .codex } + candidates.filter { $0.route != .codex }
                : candidates
            let configured = forcedCandidates.first {
                isInvocable($0)
                    && !laneSourceUnconfigured(for: $0, policy: policy, oracle: oracle)
            }
            guard let forced = configured ?? forcedCandidates.first(where: isInvocable) else {
                throw BrokerError.configError(
        ...
        let dispatched = winner.clampingEffortToModelSupport(in: policy.models)
    ...
    ///
    /// Four tiers, worse only ever shown when nothing better exists:
    /// 1. Verified available (a real gate passed, not fail-open).
    /// 2. Available but ahead of pace — measured headroom under every ceiling,
    ///    spent faster than the window sustains.
    /// 3. Available but unverified — fail-open or an unconfigured quota source;
    ///    still reachable, just quota-blind.
    /// 4. Gated — quota-capped, cooling down, or (for `t3`) unreachable. Offered
    ...
    static func collectBackups(
        ...
        for candidate in candidates {
            ...
            if evaluation.available, !evaluation.failOpen, !evaluation.paced {
                tier = 0
                why = "next in chain"
            } else if evaluation.paced {
                tier = 1
                why = "ahead of pace, offered because nothing with spare pace was available: "
                    + evaluation.why
        ...
        return sorted.prefix(BrokerDecision.maxBackups).map { entry in
            let dispatched = entry.candidate.clampingEffortToModelSupport(in: policy.models)
            return BrokerBackupOption(
                candidate: dispatched.id,
                route: dispatched.route,
                model: dispatched.model,
                effort: dispatched.effort,
                invocation: invocation(for: dispatched, caller: filter.caller, policy: policy),
                why: entry.why
            )
        }
    }
    ...
    /// The `tried` row wording for a candidate the walk may pass over, so an
    /// audit reader never sees an `available` row skipped with no explanation.
    static func heldBackNote(for evaluation: Evaluation) -> String {
        if evaluation.unconfigured {
            return "\(evaluation.why); held back for a source-verified candidate"
        }
        if evaluation.paced {
            return "\(evaluation.why); held back for a lane with spare pace"
        }
    ...
    static func buildReason(
        ...
        if evaluation.failOpen { return "\(evaluation.why) (degraded)" }
        if evaluation.paced {
            return "\(evaluation.why); taken because no lane had spare pace"
        }
        if winner.route == .native { return evaluation.why }
    ...
    struct Evaluation: Equatable {
        ...
        /// The lane is under every hard ceiling but is burning its window
        /// faster than the window can sustain. That is a reason to prefer a
        /// lane with spare pace, not a reason to refuse work: the quota is
        /// still there to spend. A paced candidate is therefore available and
        /// demoted, never unavailable.
        let paced: Bool
        let why: String
        ...
        /// A weak yes: real quota data, under every ceiling, spending ahead of
        /// pace. Not `failOpen` — the data was trustworthy — so a pick that
        /// lands here is a normal policy pick, not a degraded one.
        static func paced(_ why: String) -> Evaluation {
            Evaluation(
                available: true,
                failOpen: false,
                unconfigured: false,
    ...
    static func evaluate(
        ...
        case .auto:
            return .unavailable("automatic model choice was not expanded")
        ...
        case .native:
            return evaluateNativePools(
                ...
                block: block,
                rowState: oracle?.primaryAccount?.state,
                now: now
            )
    ...
    static func quotaBlindEvaluation(
        ...
        guard laneSourceUnconfigured(for: candidate, policy: policy, oracle: oracle) else {
            // Name the cause when there is one. "No fresh data" is true of a
            // failed poll and of a merely old one, and the caller cannot act
            // on the two the same way: one clears itself, the other needs a
            // human. Reporting the union is what left agents asking the
            // operator what the message meant.
            if let fault = providerFault(for: candidate, policy: policy, oracle: oracle) {
                return .available(
                    failOpen: true,
                    "\(candidate.id) \(fault.reasonClause); \(suffix)"
                )
            }
            return .available(
                failOpen: true,
    ...
    ///
    /// FAIL OPEN when the oracle is absent, stale, non-fresh, or missing session/weekly:
    ...
    static func evaluateNativePools(
        ...
        guard !block.stale else {
            guard let age = block.ageSeconds else {
                return .available(
                    failOpen: true,
                    "oracle timestamp missing or invalid, failing open to native claude"
                )
            }
            return .available(
                failOpen: true,
                "oracle stale (age \(percent(age))s > \(percent(thresholds.stalenessSeconds))s), failing open to native claude"
            )
        }
        guard rowState == .fresh else {
            return .available(
                failOpen: true,
                "oracle row state \(rowState?.rawValue ?? "unavailable"), failing open to native claude"
            )
        }
        ...
        let sonnet = model.hasPrefix("claude-sonnet-5") ? block.sonnet : nil
        if let sonnet, sonnet >= thresholds.sonnetWeeklyPct {
            return .unavailable(
                "native sonnet pool \(percent(sonnet))% >= \(percent(thresholds.sonnetWeeklyPct))%"
            )
        }
        let fable = model.hasPrefix("claude-fable-5") ? block.fable : nil
    ...
    /// Expands policy-only `auto` rows into every confirmed executable route.
    /// Explicit route rows keep their exact legacy behavior.
    static func expandingModelChoices(
        _ chain: [BrokerCandidate],
        policy: BrokerPolicy,
        oracle: OracleSnapshot?,
        now: Date
    ) -> [BrokerCandidate] {
        chain.flatMap { choice -> [BrokerCandidate] in
            guard choice.route == .auto else {
                return expandingAnyInstance([choice], policy: policy, oracle: oracle, now: now)
            }

            let model = choice.model
            var seen = Set<String>()
            var routes: [BrokerCandidate] = []
            ...
            if policy.agentModelAliases[model] != nil {
                append(BrokerCandidate(route: .native, model: model))
            }
            if policy.usageLanes["codex/\(model)"] != nil
                || policy.models[model]?.quota.map({ lane in
                    if case .chatGPT = lane { return true }
                    return false
                }) == true {
                append(BrokerCandidate(route: .codex, model: model))
            }
            // Unknown capability is not enough for automatic routing. Explicit
            // `t3:*` rows retain their fail-open treatment for legacy policies.
            for instance in policy.t3Instances where instance.detectedModels.contains(model) {
                append(BrokerCandidate(route: .t3, instance: instance.id, model: model))
            }
    ...
    static func headroom(
        ...
        guard let oracle else { return nil }
        let lane: BrokerUsageLane? = candidate.route == .native
            ? .claudeAccount(accountId: nil, labelContains: nil, isPrimary: true)
            : policy.usageLane(for: candidate)
    ...
    /// When the durable quota behind a candidate expires: the soonest reset
    /// among the long windows that gate it, or `nil` when none is known.
    ///
    /// Long windows only. The 5-hour session window is excluded on purpose —
    /// it recycles several times a day, so it never holds quota that is lost
    /// unspent, and including it would drown the weekly signal every lane
    /// shares.
    static func expiringWindowResetAt(
        for candidate: BrokerCandidate,
        policy: BrokerPolicy,
        oracle: OracleSnapshot?,
        now: Date
        ...
        case .claudeAccount(let accountId, let labelContains, let isPrimary):
            ...
            var resets: [Date?] = [row.weeklyResetAt]
            if candidate.model.contains("sonnet") { resets.append(row.sonnetResetAt) }
            if candidate.model.contains("fable") { resets.append(row.fableResetAt) }
            return resets.compactMap { $0 }.filter { $0 > now }.min()

        case .chatGPT(let labelContains):
            let needle = labelContains?.lowercased()
            return oracle.chatGPTRows.compactMap { row -> Date? in
                guard let reset = row.resetAt, reset > now else { return nil }
                guard Self.isDurableWindow(row) else { return nil }
                guard let needle, !needle.isEmpty else { return reset }
                return row.matchText.lowercased().contains(needle) ? reset : nil
            }.min()
        }
    }

    /// Whether a ChatGPT row meters durable quota rather than a short window
    /// that recycles several times a day.
    ///
    /// Positive evidence is required. A row whose `windowSeconds` the server
    /// omitted is NOT assumed durable: the 5-hour row is exactly the one that
    /// arrives without a length (``ChatGPTAPIResponses`` falls back to the 5h
    /// role, and rows restored from a cache written before `window_seconds`
    /// existed carry none), so trusting the omission would let a window
    /// minutes from recycling divide the score and win every ranking.
    static func isDurableWindow(_ row: OracleSnapshot.ChatGPTRow) -> Bool {
        if let seconds = row.windowSeconds {
            return seconds >= Constants.Pacing.durableWindowFloor
        }
        switch row.windowRole {
        case .chatGPTWeekly, .chatGPTMonthly:
            return true
        case .chatGPT5h, .chatGPTPro, .chatGPTCodexSpark, nil:
            return false
        }
    }

    /// What equivalent routes are actually ordered on.
    ///
    /// With ``BrokerThresholds/preferExpiringQuota`` off this is plain
    /// headroom, the historical behaviour. With it on the headroom is divided
    /// by the hours left before it expires, which ranks the pools the way a
    /// perishable stock is spent: an account holding 60 points for 14 more
    /// hours (4.3/h) outranks one holding 80 points for six days (0.6/h),
    /// because only the first set of points is about to be lost. The division
    /// is self-limiting in principle: once the expiring account is spent down
    /// far enough, its rate falls below the durable one's and ranking swings
    /// back. In practice the quota ceilings usually remove that lane first, so
    /// the swing-back is a backstop rather than the common exit.
    ///
    /// The numerator is the worst pool gating the lane; the divisor is the
    /// soonest DURABLE reset. They can belong to different pools, so the score
    /// is an ordering signal, not a literal spend rate.
    ///
    /// An unknown or already-passed reset is treated as a full weekly window:
    /// least urgent, never most, so missing data cannot promote a lane.
    static func rankingScore(
        ...
        guard let headroom = headroom(for: candidate, policy: policy, oracle: oracle, now: now)
        else { return nil }
        guard policy.thresholds.preferExpiringQuota else { return headroom }

        let horizonHours = Constants.Pacing.weeklyWindow / 3600
        let hours: Double
        if let reset = expiringWindowResetAt(
            for: candidate, policy: policy, oracle: oracle, now: now
        ) {
            hours = min(
                max(reset.timeIntervalSince(now) / 3600, Constants.Pacing.minimumExpiryHours),
                horizonHours
            )
        } else {
            hours = horizonHours
    ...
    /// Ranks every route for the same model by live quota capacity (see
    /// ``rankingScore``) while preserving the configured model priority.
    ///
    /// "The same model" means every occurrence in the chain, not only adjacent
    /// ones: a role ranks models, and a model's routes are interchangeable ways
    /// of running it. A chain of `[native/M, native/N, t3:*/M]` therefore ranks
    /// as M's routes by headroom, then N — the second M row is another route to
    /// M, not a rank below N. Ranking only contiguous runs instead would strand
    /// a T3 account that still has headroom behind a lower-priority model,
    /// which is exactly what
    /// `test_equivalentRoutesUseTheAccountWithTheMostHeadroomForEveryClaudeModel`
    /// forbids. The order of DISTINCT models is what stays fixed.
    static func rankingEquivalentCandidatesByHeadroom(
        _ candidates: [BrokerCandidate],
        policy: BrokerPolicy,
        oracle: OracleSnapshot?,
        now: Date
    ) -> [BrokerCandidate] {
        var modelOrder: [String] = []
        var candidatesByModel: [String: [BrokerCandidate]] = [:]
        for candidate in candidates {
            if candidatesByModel[candidate.model] == nil {
                modelOrder.append(candidate.model)
            }
            candidatesByModel[candidate.model, default: []].append(candidate)
        }

        return modelOrder.flatMap { model in
            let group = candidatesByModel[model, default: []]
            return group.enumerated().sorted { left, right in
                let leftScore = rankingScore(
                    for: left.element, policy: policy, oracle: oracle, now: now
                )
                let rightScore = rankingScore(
                    for: right.element, policy: policy, oracle: oracle, now: now
                )
    ...
    /// The pacing verdict for one quota pool, or `nil` when the pool is pacing
    /// acceptably or carries no reset time.
    ///
    /// The verdict demotes: pacing means the lane is spending its window faster
    /// than the window can sustain, which is only meaningful next to a lane
    /// that is not. Refusing the lane outright made every Claude candidate
    /// unavailable whenever one account was ahead of pace, and the forced
    /// fallback then handed back the very candidate it had just rejected —
    /// the same route, reported as degraded. The fixed utilization ceilings
    /// above this check are what actually refuse an exhausted lane.
    ///
    /// Shares `UsageLimit.isAtRisk` with the menu bar card so the badge and the
    /// routing decision never disagree, which also means it inherits that
    /// method's clamped divisor: a window that has only just opened does not
    /// demote a lane on a ratio computed from a few minutes of history.
    static func paceVerdict(
        ...
        return .paced("\(label) pacing risk")
    }
    ...
    /// Why a candidate's quota source is not answering, when the cause is a
    /// broken provider rather than an absent or merely old reading.
    ///
    /// Two consumers read this and must never disagree: the reason text the
    /// calling agent renders verbatim, and the title on the degraded-pick
    /// modal. Deriving it twice is how they drift, so it is derived here.
    ///
    /// Only `.error` counts. `.stale` and `.unavailable` are deliberately not
    /// faults: stale means the account answered recently and waiting fixes it,
    /// and unavailable means this machine has never fetched, which is an
    /// unconfigured lane rather than a broken one. Reporting either as a
    /// disconnect would send the operator to reconnect an account that is fine.
    /// This mirrors the rule the rest of the walk already follows -- not
    /// knowing is never evidence.
    static func providerFault(
        for candidate: BrokerCandidate,
        policy: BrokerPolicy,
        oracle: OracleSnapshot?
    ) -> BrokerProviderFault? {
        ...
        switch lane {
        case .chatGPT:
            return oracle.chatGPTState == .error ? .chatGPTDisconnected : nil
        ...
        case .claudeAccount(let accountId, let labelContains, let isPrimary):
            guard let row = claudeAccountRow(
                ...
                labelContains: labelContains,
                isPrimary: isPrimary
            ), row.state == .error else { return nil }
            return .claudeAccountDisconnected(label: row.label)
        }
    }
    ...
    ///
    /// These are never quota events: a Codex session can invoke only its own
    /// native child agents, so handing it a Claude-native pick is a defect, not
    ...
    static func invocation(
        ...
        switch candidate.route {
        case .auto:
            preconditionFailure("automatic model choice must be expanded before invocation")
        case .native:
            return .agent(
        ...
        case .codex:
            return caller == "codex"
                ? .agent(model: candidate.model, effort: candidate.effort)
                : .codexExec(model: candidate.model, effort: candidate.effort)
        case .t3:
            return .t3Dispatch(
                model: candidate.model,
    ...
    static func oracleBlock(
        ...
        let stale = ageSeconds.map { $0 > thresholds.stalenessSeconds } ?? true

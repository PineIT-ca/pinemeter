...
actor BrokerService: BrokerServiceProtocol {
    ...
    private func makePick(
        ...
        let timestamp = now()
        let picked = if let overrideCandidate {
            ...
            try decide(role, caller, policy, oracle, cooldowns, timestamp, t3Liveness)
        }
        let decision = picked
    ...
    private func validCooldownTargets() -> Set<String> {
        let timestamp = now()
        return Set(policy.roles.values
            .flatMap {
                BrokerEngine.expandingAnyInstance($0, policy: policy, oracle: oracle, now: timestamp)
            }

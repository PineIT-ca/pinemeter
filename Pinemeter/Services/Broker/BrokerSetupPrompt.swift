...
enum BrokerSetupPrompt {
    ...
    private static let contract = """
        ...
        A fresh, explicit operator instruction may override one dispatch. Pass its exact configured candidate as `override_candidate` to `pick`. Never infer an override from preference, context, or old instructions. A model-only request such as "use Fable" is not an exact candidate; omit `override_candidate` so normal policy selection can choose the healthy account. Never persist or reuse it. Dispatch only the returned decision with `source: human-override`. The broker still rejects malformed, unconfigured, caller-forbidden, cooling-down, quota-capped, or unreachable candidates. Never bypass the broker to satisfy an override.

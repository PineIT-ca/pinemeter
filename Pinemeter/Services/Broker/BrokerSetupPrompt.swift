...
enum BrokerSetupPrompt {
    ...
    private static let contract = """
        ...
        A fresh, explicit operator instruction may override one dispatch. Pass its exact configured candidate as `override_candidate` to `pick`. Never infer an override from preference, context, or old instructions. An explicit request for a Fable review is an exact request for `auto/claude-fable-5-1`: call `pick` with role `review` and that `override_candidate`. The wildcard lets Pinemeter resolve the configured Fable routes. If the override is unavailable, stop and report the blocker; you must not substitute another model or retry without the override. Other model-only requests are not exact candidates; omit `override_candidate` for those so normal policy selection can choose a healthy account. Never persist or reuse an override. Dispatch only the returned decision with `source: human-override`. Human overrides bypass quota caps and pacing gates. An unavailable override automatically repairs failed browser sessions and re-probes T3 once before returning an actionable error. The broker still rejects malformed, unconfigured, caller-forbidden, cooling-down, or unreachable candidates. Never bypass the broker to satisfy an override.

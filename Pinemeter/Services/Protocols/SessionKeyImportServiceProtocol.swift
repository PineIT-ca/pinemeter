...
struct BrowserScanOutcome: Equatable, Sendable {
    ...
    var discoveryFailures: [BrowserSessionImportFailure] = []
    ...
    var offersFullDiskAccessSettings: Bool {
        discoveryFailures.contains(where: \.offersFullDiskAccessSettings)
...
/// One browser session shown for review before any credential is saved.
struct BrowserSessionImportSession: Identifiable, Sendable, CustomDebugStringConvertible {
    enum Credential: Sendable, CustomDebugStringConvertible {
        case claudeSessionKey(String)
        case chatGPTCookie(String)

        var debugDescription: String { "<redacted browser credential>" }
    }
    ...
    var debugDescription: String {
        "BrowserSessionImportSession(id: \(id), provider: \(provider.rawValue), source: \(source.rawValue), profileLabel: \(profileLabel), credential: <redacted>)"
    }
}

struct BrowserSessionImportFailure: Identifiable, Equatable, Sendable {
    let provider: CredentialProvider
    let source: BrowserImportSource
    let message: String
    let offersFullDiskAccessSettings: Bool

    var id: String { "\(provider.rawValue):\(source.rawValue)" }
}

/// Read-only browser discovery result. Its debug output intentionally excludes
/// credential material so diagnostics can never reveal a session value.
struct BrowserSessionImportReview: Identifiable, Sendable, CustomDebugStringConvertible {
    let id: UUID
    let scannedBrowsers: [BrowserImportSource]
    let sessions: [BrowserSessionImportSession]
    let failures: [BrowserSessionImportFailure]

    init(
        id: UUID = UUID(),
        scannedBrowsers: [BrowserImportSource],
        sessions: [BrowserSessionImportSession],
        failures: [BrowserSessionImportFailure]
    ) {
        self.id = id
        self.scannedBrowsers = scannedBrowsers
        self.sessions = sessions
        self.failures = failures
    }

    var offersFullDiskAccessSettings: Bool {
        failures.contains { $0.offersFullDiskAccessSettings }
    }

    var debugDescription: String {
        let rows = sessions.map { "\($0.provider.rawValue):\($0.source.rawValue):\($0.profileLabel)" }
        return "BrowserSessionImportReview(sessions: \(rows), failures: \(failures.count))"
    }
}
...
enum SessionKeyImportError: LocalizedError {
    ...
    case primaryAccountNeedsReconnect(provider: String, label: String)
    ...
    var errorDescription: String? {
        ...
        case .primaryAccountNeedsReconnect(let provider, let label):
            return "Sign in to \(label) in \(provider) again, then scan again. Pinemeter kept your existing accounts unchanged."
        case .allDiscoveredAccountsExcluded:
...
protocol SessionKeyImportServiceProtocol: Actor {
    ...
    /// Import every distinct ChatGPT session cookie reachable from `source`,
    /// one per browser profile, so all signed-in accounts can be connected.
...
extension SessionKeyImportServiceProtocol {
    ...
    func importAllChatGPTSessionCookies(
        ...
        [try await importChatGPTSessionCookie(from: source)]
    }
}

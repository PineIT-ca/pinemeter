...
struct RoutingUpdateNotice: Equatable, Sendable {
...
protocol NotificationServiceProtocol {
    ...
    /// Announce routing rules or presets delivered by the manifest.
    func sendRoutingUpdateNotification(_ update: RoutingUpdateNotice) async throws

    /// Send the reminder that the recorded instruction check is due to be
    /// re-run, for the reason that made it due.

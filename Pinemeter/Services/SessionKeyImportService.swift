...
actor SessionKeyImportService: SessionKeyImportServiceProtocol {
    ...
    static let claudeCookieHosts = ["claude.ai"]
    static let chatGPTCookieHosts = ["chatgpt.com", "chat.openai.com"]
    ...
    func importSessionKey(from source: BrowserImportSource) async throws -> ImportedSessionKey {
        let query = BrowserCookieQuery(domains: Self.claudeCookieHosts, domainMatch: .exact)
    ...
    func importChatGPTSessionCookie(from source: BrowserImportSource) async throws -> ImportedChatGPTSessionCookie {
        let query = BrowserCookieQuery(domains: Self.chatGPTCookieHosts, domainMatch: .exact)
        var sawAccessDenied = false
        var sawSafariAccessDenied = false
        var deniedKeychainBrowserName: String?
    ...
    /// Every distinct ChatGPT session cookie across the source's browsers and
    /// profiles. `importChatGPTSessionCookie` stops at the first hit, which is
    /// why a second signed-in ChatGPT account was never discovered; connecting
    /// accounts needs all of them, and dedupes by the identity each one
    /// resolves to rather than by cookie value.
    func importAllChatGPTSessionCookies(
        from source: BrowserImportSource
    ) async throws -> [ImportedChatGPTSessionCookie] {
        let query = BrowserCookieQuery(domains: Self.chatGPTCookieHosts, domainMatch: .exact)
        var imported: [ImportedChatGPTSessionCookie] = []
        var seenValues = Set<String>()
        var sawAccessDenied = false
        var sawSafariAccessDenied = false
        var deniedKeychainBrowserName: String?
        ...
        for browser in browsers(for: source) {
            do {
                let sources = try cookieClient.records(matching: query, in: browser)
                Self.logger.info("importAllChatGPTSessionCookies(\(browser.displayName, privacy: .public)): \(sources.count) cookie store(s)")
                for storeRecords in sources {
                    guard let one = importedChatGPTSessionCookie(from: storeRecords) else { continue }
                    if seenValues.insert(one.cookieHeader).inserted {
                        imported.append(one)
                    }
                }
            } catch let error as BrowserCookieError {
        ...
        // The Chromium fallback reads cookie stores this client could not, so
        // it can still contribute an account the loop above missed.
        if let fallback = ChatGPTChromiumCookieFallbackImporter.importSessionCookie(from: browsers(for: source)),
           seenValues.insert(fallback.cookieHeader).inserted {
...
struct ChatGPTChromiumCookieFallbackImporter {
    ...
    static let supportedHosts = SessionKeyImportService.chatGPTCookieHosts.flatMap { [$0, ".\($0)"] }
    static let cookieQuerySQL = """
    SELECT host_key, name, value, encrypted_value
    FROM cookies
    WHERE host_key IN (\(supportedHosts.map { "'\($0)'" }.joined(separator: ", ")))
    """
    ...
    private static func readCookies(from databaseURL: URL, key: Data) -> [BrowserCookie] {
        ...
        guard sqlite3_prepare_v2(database, cookieQuerySQL, -1, &statement, nil) == SQLITE_OK else { return [] }

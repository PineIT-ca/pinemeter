...
final class NotificationService: NSObject, NotificationServiceProtocol, UNUserNotificationCenterDelegate {
    ...
    func sendRoutingUpdateNotification(_ update: RoutingUpdateNotice) async throws {
        ...
        if let profileName = update.profileName {
            content.body = "\(profileName) has updated routing rules. Review and apply them in the Broker window."
            ...
            content.body = "\(update.newPresetCount) new routing preset"
                ...
                + " arrived from the manifest. Review them in the Broker window."
        }
        content.sound = .default
...
extension Notification.Name {
    ...
    /// Opens Settings on the Accounts pane. ``openBrokerSettings`` opens the
    /// Broker window; disconnected providers are repaired in Accounts.
    static let openAccountsSettings = Notification.Name("openAccountsSettings")

...
actor GeminiUsageService: GeminiUsageServiceProtocol {
    ...
    func fetchUsage() async throws -> GeminiUsageData {
        try await fetchUsage(account: Self.defaultAPIKeyAccount)
    }

    func fetchUsage(account: String) async throws -> GeminiUsageData {
        let apiKey: GeminiAPIKey
        do {
            apiKey = try await apiKeyRepository.load(account: account)
        ...
        do {
            ...
            try? await apiKeyRepository.clear(account: account)
            throw GeminiUsageError.invalidAPIKey
        }
    }
...
actor GeminiHTTPClient: GeminiHTTPClientProtocol {
    ...
    func request<T: Decodable>(_ endpoint: String, apiKey: GeminiAPIKey) async throws -> T {
        ...
        request.setValue(apiKey.value, forHTTPHeaderField: "x-goog-api-key")

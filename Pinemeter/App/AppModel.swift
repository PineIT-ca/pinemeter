...
final class AppModel {
    ...
    private func recoverBrowserSessionsIfNeeded() async {
        ...
        let approvedSessions = review.sessions.filter { session in
            failedProviders.contains(session.provider)
        ...
        if !approvedSessions.isEmpty {
            ...
            // Recovery may refresh credential material only. Keep every
            // user-visible label and Keychain account binding unchanged.
    ...
    ///
    /// The push matters: every other apply path lives inside the Broker
    /// settings tab, whose `onChange(of: policy)` does it. This action fires
    /// with Settings closed, so without the push the persisted policy and the
    /// one serving picks would disagree until the tab is next opened.
    func applyRoutingUpdate(profileID: UUID) {
        guard settings.broker.activeProfileID == profileID else { return }

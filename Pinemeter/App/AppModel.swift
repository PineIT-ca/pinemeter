...
final class AppModel {
    ...
    @ObservationIgnored private let t3LivenessChecker: any T3LivenessCheckerProtocol
    ...
    private func stopBrokerServer(generation: Int? = nil) async {
        guard let server = brokerLoopbackServer else { return }
        await server.stop()
    ...
    private func buildAggregateQuotaRows(generatedAt: Date) -> AggregateQuotaRows {
        ...
        return AggregateQuotaRows(claudeAccounts: claudeAccounts, chatGPT: chatGPT, gemini: gemini)
    }
    ...
    private func exportAggregateUsageSnapshot(generatedAt: Date = Date()) async {
        ...
        await brokerService.updateOracleSnapshot(Self.makeOracleSnapshot(generatedAt: generatedAt, rows: rows))
    }

    private static func makeOracleSnapshot(generatedAt: Date, rows: AggregateQuotaRows) -> OracleSnapshot {
        ...
        let chatGPTRows = rows.chatGPT.rows.map {
            OracleSnapshot.ChatGPTRow(label: $0.label, usedPercent: $0.usedPercent)
        }
    ...
    static func aggregateQuotaState(
        ...
        return generatedAt.timeIntervalSince(lastUpdated) > Constants.Refresh.stalenessThreshold ? .stale : .fresh
    }
    ...
    func performScheduledRefreshTick() async {
        ...
        await refreshAdditionalClaudeAccounts()
        if hasChatGPTSessionCookie && settings.isChatGPTUsageShown {
            await refreshChatGPTUsage()
        }
        if hasGeminiAPIKey {
        ...
        let liveness = await t3LivenessChecker.checkLiveness(
            instances: settings.broker.policy.t3Instances
        )
        await brokerService.updateT3Liveness(liveness)
    }

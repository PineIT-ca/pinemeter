...
final class AppModel {
    ...
    var usageQuotaBars: [MenuBarQuotaBar] {
        ...
        for section in chatGPTUsageSections {
            ...
            let renameTarget: QuotaRenameTarget = section.isRenameable
                ...
                : .provider(.chatGPT)
            for row in usageData.displayRows {
                bars.append(MenuBarQuotaBar(
    ...
    /// plan stay current. Usage held under the old id moves with it.
    ...
    func refreshAdditionalChatGPTAccounts() async {
        ...
        for account in additionalAccounts {
            do {
                ...
                chatGPTAccountUsage[account.id] = usage
                ...
                chatGPTAccountErrors[account.id] = error.localizedDescription
            }
        }
    ...
    func removeChatGPTAccount(id: String) async throws {
        ...
        if account.isPrimary {
            let successor = settings.chatGPTAccounts.first { !$0.isPrimary }
            guard let successor else {
                try await clearChatGPTSessionCookie()
                return
            }
            let session = try await chatGPTSessionRepository.load(account: successor.keychainAccount)
    ...
    /// browser profiles, and returns the primary one's cookie so single
    /// -account callers keep their existing behavior.
    @discardableResult
    ...
    /// ChatGPT account among them. Callers that gather cookies from several
    /// browsers must aggregate first and call this once: connecting replaces
    /// `settings.chatGPTAccounts`, so per-browser calls would drop the
    /// previous browser's accounts.
    @discardableResult
    ...
    ///
    /// No-op when the feature is off, the broker is off (unless `force`), the
    /// URL is missing or not `https`, or (unless `force`) the last check is

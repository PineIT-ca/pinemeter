...
protocol BrokerLifecycleProtocol: BrokerServiceProtocol {
    ...
    /// the Instructions pane. `nil` until one has run on this machine.
...
final class AppModel {
    ...
    /// The access policy the running server was built with, so a settings
    /// change that alters the bind interface or the key requirement is
    /// detected the same way a port change is.
    @ObservationIgnored private var brokerActiveAccessPolicy: BrokerAccessPolicy?
    ...
    init(
        ...
        self.brokerServerFactory = brokerServerFactory ?? { broker, port, accessPolicy in
    ...
    func applyBrokerSettingsChange() async {
        ...
        // Reads the Keychain, so it has to happen before any generation-gated
        // work below and be re-checked afterwards like every other await here.
        let desiredAccessPolicy = await desiredBrokerAccessPolicy()
        guard generation == brokerLifecycleGeneration else { return }
        ...
        // The bind interface and the key requirement are baked into the
        // listener at start, exactly like the port, so changing either one
        // needs the same stop-then-start.
        if brokerBoundPort != desiredPort || brokerActiveAccessPolicy != desiredAccessPolicy {
            await stopBrokerServer(generation: generation)
            guard generation == brokerLifecycleGeneration else { return }
            await startBrokerServer(
                port: desiredPort,
    ...
    /// The access policy the server should be running with right now: the
    /// saved network/key settings, plus the Keychain-held key when the mode
    /// needs one.
    ///
    /// A mode that needs a key with no key provisioned yields `apiKey == nil`,
    /// which ``BrokerAccessPolicy/authorizes(headers:isLoopbackPeer:)`` fails
    /// closed on. That is the correct half-configured behaviour: refuse the
    /// requests that need a key rather than serve them unauthenticated.
    private func desiredBrokerAccessPolicy() async -> BrokerAccessPolicy {
        let mode = settings.broker.apiKeyMode
        let key = mode == .none ? nil : await loadBrokerAPIKey()
        return BrokerAccessPolicy(
            networkAccess: settings.broker.networkAccess,
            apiKeyMode: mode,
            apiKey: key
        )
    }
    ...
    private func startBrokerServer(
        port: UInt16,
        accessPolicy: BrokerAccessPolicy,
        generation: Int
    ) async {
        let server = brokerServerFactory(brokerService, port, accessPolicy)
        brokerLoopbackServer = server
        brokerBoundPort = nil
        brokerActiveAccessPolicy = accessPolicy
    ...
    func refreshChatGPTUsage() async {
        ...
        do {
            let usage = try await chatGPTUsageService.fetchUsage()
            ...
            await chatGPTUsageCacheRepository.save(usage)
    ...
    func clearChatGPTSessionCookie() async throws {
        ...
        await chatGPTUsageCacheRepository.clear()
        hasChatGPTSessionCookie = false
    ...
    /// Refreshes the remote preset manifest — extra named rule profiles the
    ...
    ///
    /// No-op when the feature is off, the URL is missing or not `https`, or
    /// (unless `force`) the last check is still within the freshness window.
    /// On failure the previous presets are kept — stale-but-usable beats
    /// empty — and a short, sanitised error is stored for the settings UI.
    func refreshPresetManifest(force: Bool = false) async {
        guard let presetManifestService else { return }
        let config = settings.broker.presetManifest
        guard config.isEnabled else { return }
        // Trimmed exactly like `BrokerPresetManifestCard.isURLValid`: a
        // pasted URL routinely carries leading/trailing whitespace, and
        // `URL(string:)` treats that as part of the string rather than
        // ignoring it, so an untrimmed value here would show a green
        // (valid-looking) field in Settings and then silently no-op forever.
        let trimmedURLString = config.urlString.trimmingCharacters(in: .whitespacesAndNewlines)
        guard let url = URL(string: trimmedURLString), url.scheme?.lowercased() == "https" else { return }

        if !force, let lastCheckedAt = config.lastCheckedAt,
           Date().timeIntervalSince(lastCheckedAt) < Self.presetManifestRefreshInterval {
            return
        }

        do {
            let outcome = try await presetManifestService.fetch(from: url, etag: config.etag)
            switch outcome {
            case .notModified:
                settings.broker.presetManifest.lastCheckedAt = Date()
                settings.broker.presetManifest.lastError = nil
            case .updated(let manifest, let etag):
                settings.broker.updateRemotePresets(manifest.presets)
                settings.broker.presetManifest.etag = etag
                settings.broker.presetManifest.lastCheckedAt = Date()
                settings.broker.presetManifest.lastError = nil
            }
        } catch {
            settings.broker.presetManifest.lastCheckedAt = Date()
            settings.broker.presetManifest.lastError = Self.sanitizedManifestError(error)
        }
    }

    /// Clears the stored ETag and last-check timestamp for the preset
    /// manifest. Called by `BrokerPresetManifestCard` whenever the URL field
    /// commits a change.
    ///
    ...
    /// throttle.
    func presetManifestURLChanged() {
        settings.broker.presetManifest.etag = nil
        settings.broker.presetManifest.lastCheckedAt = nil
    }

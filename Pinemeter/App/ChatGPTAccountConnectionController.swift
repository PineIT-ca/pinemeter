...
actor ChatGPTAccountConnectionController {
    ...
    func connect(
        ...
        for candidate in validated {
            ...
            } else if let allowedAccountIds, !allowedAccountIds.contains(candidate.accountId) {
                continue
        ...
        if candidates.isEmpty, excludedCandidateCount == 0, !excludesEveryAccount,
           ...
           !preserveExistingAccounts || existingAccounts.isEmpty {
        ...
        guard !candidates.isEmpty else {
            await progress(nil)
            throw excludedCandidateCount > 0 || excludesEveryAccount
                ? SessionKeyImportError.allDiscoveredAccountsExcluded
                : SessionKeyImportError.invalidImportedChatGPTSessionCookie
        }
        ...
        if preserveExistingAccounts,
           let existingPrimaryId,
           !candidates.contains(where: { $0.accountId == existingPrimaryId }) {
            await progress(nil)
            throw SessionKeyImportError.primaryAccountNeedsReconnect(
                provider: "ChatGPT",
                label: existingAccounts.first { $0.isPrimary }?.displayLabel ?? "primary account"
            )
        }
        ...
        // deleted.
        var staleAccountIds = preserveExistingAccounts
            ? []
            : Set(existingAccounts.filter { !$0.isPrimary }.map(\.id))
        ...
        if preserveExistingAccounts {
            accounts.append(contentsOf: existingAccounts.filter { existing in
                !accounts.contains { $0.id == existing.id }
            })
            accounts.sort { lhs, rhs in
                if lhs.isPrimary != rhs.isPrimary { return lhs.isPrimary }
                return lhs.displayLabel.localizedCaseInsensitiveCompare(rhs.displayLabel) == .orderedAscending
        ...
        return Connection(
            ...
            connected: (validated.isEmpty ? candidates : validated.filter { candidate in
                if candidate.identity.stableId == nil {
                    return candidates.contains { $0.cookieHeader == candidate.cookieHeader }
                }
                return seenAccounts.contains(candidate.accountId)
            }).map {
                ImportedChatGPTSessionCookie(
                    cookieHeader: $0.cookieHeader,

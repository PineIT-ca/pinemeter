...
actor ClaudeAccountConnectionController {
    ...
    func connect(
        ...
        for candidate in validated {
            ...
            } else if let allowedAccountIds, !allowedAccountIds.contains(candidate.organization.uuid) {
                continue
        ...
        if preserveExistingAccounts,
           let existingPrimaryOrganizationId,
           !candidates.contains(where: { $0.organization.organizationUUID == existingPrimaryOrganizationId }) {
            await progress(nil)
            throw SessionKeyImportError.primaryAccountNeedsReconnect(
                provider: "Claude",
                label: accountsBeforePrimarySave.first { $0.isPrimary }?.displayLabel ?? "primary account"
            )
        }
        ...
        var staleAccountIds = preserveExistingAccounts
            ? []
            : Set(await currentAccounts().filter { !$0.isPrimary }.map(\.id))
        ...
        if preserveExistingAccounts {
            accounts.append(contentsOf: accountsBeforePrimarySave.filter { existing in
        ...
        return Connection(
            result: ClaudeAccountsImportResult(
                ...
                connected: validated.filter { seenOrganizations.contains($0.organization.uuid) }.map {

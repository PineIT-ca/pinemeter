    ...
    private func observeOverlayEvents() {
        ...
        brokerAlertObserver = NotificationCenter.default.addObserver(
            ...
            MainActor.assumeIsolated {
                ...
                self.presentBrokerAlert(decision)
            }
        }
    }

    private func presentBrokerAlert(_ decision: BrokerDecision) {
        NSApp.activate(ignoringOtherApps: true)
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = "Broker Route Degraded"
        alert.informativeText = [
            "\(decision.caller) requested \(decision.role).",
            decision.model,
            decision.degradedReason ?? decision.reason,
        ].joined(separator: "\n\n")
        alert.addButton(withTitle: "Fix Routes")

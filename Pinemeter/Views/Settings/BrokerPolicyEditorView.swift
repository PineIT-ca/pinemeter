...
struct BrokerPolicyEditorView: View {
    ...
    private var roleSidebar: some View {
        ...
        // caption ("native/claude-fable-5" truncated), which is what makes the
            ...
            HStack(spacing: Self.rowControlSpacing) {
                if routeBinding.wrappedValue == .t3 {
                    ...
                    Picker("", selection: instanceBinding) {
                        ...
                        // Between "Default" (one fixed lane) and a named row
                        // (one fixed lane) sits the choice that names none:
                        // whichever instance serving this model has the most
                        // headroom when the pick is made.
                        Text("Any instance")
                            .tag(String?.some(BrokerCandidate.anyInstance))
                    ...
                    .help(
                        instanceBinding.wrappedValue == BrokerCandidate.anyInstance
                            ? "T3 instance: whichever instance serving this model has the most "
                                + "headroom when the pick is made"
                            : "T3 instance: \(instanceName)"
                    )
    ...
    private func reachability(for candidate: BrokerCandidate) -> Bool? {
        ...
        // `t3:*` has no single instance to report on, so the dot answers the
        // question the row actually poses: is there anything here to route to.
        // Red only when every instance it could expand to is known-unreachable.
        if candidate.isAnyInstance {
            let health = appModel.brokerUIState?.routeHealth ?? []
            let known = policy.instances(serving: candidate.model).compactMap { instance in
                health.first { $0.instanceId == instance.id }?.reachable
            }
            return known.isEmpty ? nil : known.contains(true)
        }
    ...
    /// instance's name, "Any instance" for the `t3:*` sentinel, or "Default"
    /// when the candidate carries no qualifier (or names an instance the policy
    ...
    static func instanceDisplayName(_ instanceID: String?, in policy: BrokerPolicy) -> String {
        ...
        if instanceID == BrokerCandidate.anyInstance { return "Any instance" }

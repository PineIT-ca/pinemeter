...
struct BrokerPolicyEditorView: View {
    ...
    func updateCandidate(role: String, index: Int, transform: (BrokerCandidate) -> BrokerCandidate) {
        ...
        chain[index] = transform(chain[index]).clampingEffortToModelSupport(in: policy.models)
    ...
    private func candidateRow(role: String, index: Int) -> some View {
        ...
        let effortSupport = BrokerEffort.support(forModel: modelBinding.wrappedValue, in: policy.models)
        let effortValueForAccessibility = currentCandidate.map {
            Self.effortAccessibilityValue(for: $0, in: policy.models)
        }
    ...
    /// Chain models lead, alias/catalog-only ids trail: `addRole`/`addCandidate` seed
    ...
    static func knownModelIDs(in policy: BrokerPolicy) -> [String] {
        ...
        let catalogOnly = Set(policy.agentModelAliases.keys)
            .union(policy.models.keys).subtracting(chainModels)
        return chainModels.sorted() + catalogOnly.sorted()
    }
    ...
    static func effortAccessibilityValue(
        for candidate: BrokerCandidate,
        in models: [String: BrokerModelSpec]
    ) -> String {
        let support = BrokerEffort.support(forModel: candidate.model, in: models)
        guard let effort = candidate.effort, support.selectableLevels.contains(effort) else {
    ...
    func addCandidate(role: String) {
        ...
        chain.append(BrokerCandidate(
            route: .auto,
            model: defaultModel
        ).clampingEffortToModelSupport(in: policy.models))
        writeChain(role: role, chain)
    }

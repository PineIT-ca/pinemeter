        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if showsUndoBanner {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text("Restore your previous \(appModel.settings.broker.routingRecovery?.profileName ?? "profile") rules, including local edits. Your accounts stay connected.\(placement == .popover ? " Dismissing keeps Undo Update in Broker settings." : "")")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)

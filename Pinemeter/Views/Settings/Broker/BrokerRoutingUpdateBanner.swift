...
/// Persistent entry point shared by the popover and every Broker window pane.
struct BrokerRoutingUpdateBanner: View {
    /// Where the banner is shown. The popover is a glanceable surface, so the
    /// undo offer can be dismissed there; the Broker window is the durable place
    ...
    private var undoBody: String {
        ...
        return "Restore your previous \(name) rules, including local edits. Your accounts stay connected.\(placement == .popover ? " Dismissing keeps Undo Update in the Broker window." : "")"
    }
                ...
                VStack(alignment: .leading, spacing: 10) {
                    if placement == .popover {
                        HStack(alignment: .firstTextBaseline) {
                            ...
                            .help("Dismiss. Undo Update stays in the Broker window.")

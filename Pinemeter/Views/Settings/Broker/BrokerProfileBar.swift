...
struct BrokerProfileBar: View {
                ...
                if settings.activeProfileHasUpdatedRules {
                    ...
                    .help(
                        "This profile's saved rules changed since you loaded it. "
                            + "Use Review Update at the top of this window to apply them."
                    )
                }

                // Who decides this profile's rules. Sits beside "Updated"
                // because the two answer the same question from opposite
                // ends: that one says newer rules are waiting, this one says
                // the manifest is where this profile's rules come from at all.
                // Not a claim about the rules currently on screen — while an
                // update is pending those are still the shipped ones.
                if let activeProfile, settings.manifestSuppliesRules(id: activeProfile.id) {
                    BrokerChip(
                        text: "Manifest",
    ...
    private var profileMenu: some View {
        Menu {
            // The manifest's rules for a built-in, where it publishes any:
            // an updated built-in keeps its slot, its name and its symbol,
            // and picking it applies the updated rules.
            Section("Built-in") {
                ForEach(settings.effectiveBuiltInProfiles) { profile in
                    profileMenuItem(profile)
                }
            }
            // Presets the manifest adds rather than updates. Same apply flow
            // as a built-in (`profileMenuItem` doesn't distinguish), but they
            // sit in their own section so it stays visible that these came
            // from outside the app rather than shipping with it.
            if !settings.additionalRemotePresets.isEmpty {
                Section("From Manifest") {
                    ForEach(settings.additionalRemotePresets) { profile in
                        profileMenuItem(profile)
                    }
                }
            }
    ...
    private func profileMenuItem(_ profile: BrokerAgentProfile) -> some View {
        let isFromManifest = settings.manifestSuppliesRules(id: profile.id)
        return Button {
        ...
        .help(isFromManifest ? "\(profile.detail) \(Self.manifestSourceHelp)" : profile.detail)
    }

    /// One line of help for anything the manifest publishes rules for.
    ///
    /// Worded as "publishes", not "these rules came from the manifest": while
    /// an update is pending the loaded rules are still the shipped ones, and
    /// the sentence has to stay true in both states. The second half is the
    /// part people actually need — a manifest refresh never re-routes
    /// anything on its own.
    static let manifestSourceHelp =
        "The preset manifest publishes this profile's rules, so they can change "
            + "without an app update. Nothing routes differently until you pick it."

    /// The menu row's text.
    ///
    /// A built-in that the manifest publishes rules for still renders in the
    /// "Built-in" section under its compiled name, so without the suffix the
    /// row gives no hint that its rules came off the network — the exact
    /// question someone asks when routing changed and the app did not. Marked
    /// in every section rather than only that one: the label answers "where
    /// did these rules come from" wherever the row is read, including in the
    /// bar's own accessibility value.
    static func menuTitle(
        for profile: BrokerAgentProfile, isActive: Bool, isFromManifest: Bool
    ) -> String {
        var title = isActive ? "\u{2713} \(profile.name)" : profile.name
        if isFromManifest {

...
struct BrokerThresholdsCard: View {
    ...
    /// Quota is use-it-or-lose-it, so the account whose window resets soonest
    /// is the one holding stock that is about to be thrown away.
    private var expiringQuotaRow: some View {
        HStack(spacing: 10) {
            VStack(alignment: .leading, spacing: 1) {
                Text("Spend expiring quota first")
                    .font(.callout)
                Text("When two accounts can serve the same model, prefer the one whose "
                    + "window resets soonest, so quota is spent before it is lost. "
                    + "Off ranks on remaining headroom alone.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            ...
            Toggle("", isOn: thresholds.preferExpiringQuota)
                .labelsHidden()
                .accessibilityLabel("Spend expiring quota first")
        }
    }

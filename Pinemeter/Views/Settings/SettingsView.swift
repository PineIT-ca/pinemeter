...
struct SettingsView: View {
    ...
    // real selected tab, the way `BrokerSettingsTab`'s pane persistence did
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            ...
            BrokerSettingsTab(appModel: appModel)
        ...
        // The Broker tab's role/chain editor is the widest content in the
        // window: a role list beside a chain row carrying route, model,
        // instance and effort controls. At the old 460pt minimum those
        // controls had to be squeezed onto two lines with hand-computed
        // indents; 620 gives them room and leaves the other tabs, whose
        // content was already wider than 460, with fewer horizontal scrolls.
        //
        // The height floor is set by the Broker tab too: its fixed status
        // header and pane picker cost around 150pt before any pane draws, and
        // below 600 the Routing pane loses so much of the chain editor that
        // the scroll is all the user sees.
        .frame(
            minWidth: 620,
            maxWidth: .infinity,
            minHeight: 600,
        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 16) {
                if !appModel.isReady {
                    ...
                    subscriptionResetAnnouncementSection
                    barColorsSection
                    fableUsageSection
    ...
    private var accountsTab: some View {
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                Text("Connect the accounts you use, give them recognizable names, and keep their usage in one place.")
    ...
    private var scanButton: some View {
        Button(action: {
            ...
            HStack(spacing: 5) {
                ...
                Text(isImportingProviderSessions ? (appModel.importProgress ?? "Scanning\u{2026}") : "Connect browser accounts")
            }
        }
    ...
    private var accountCardsRow: some View {
        ...
        // Adaptive rows keep every account and the Gemini connection action
        // reachable in the pane's single vertical scroll view.
        VStack {
    ...
    private func chatGPTAccountCard(_ account: ChatGPTAccount) -> some View {
        ...
        let statusText = appModel.settings.isChatGPTUsageShown ? Self.accountStatusText(health) : "Monitoring off"
        ...
        return VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 4) {
                ...
                Circle()
                    .fill(appModel.settings.isChatGPTUsageShown ? credentialStatusColor(for: health) : .secondary)
                    .frame(width: 8, height: 8)
                    .help(accountError ?? statusText)
            }
            ...
            if !appModel.settings.isChatGPTUsageShown {
                Text("Turn on ChatGPT usage in General settings to resume monitoring.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
            ...
            if let accountError {
                CopyableErrorText(accountError, font: .caption2, foregroundStyle: .orange)
            }

            Spacer(minLength: 4)

            HStack(spacing: 8) {
                excludeButton(.chatGPT(id: account.id, label: account.displayLabel))
                removeButton(.chatGPT(id: account.id, label: account.displayLabel))
                Spacer()
            }
        }
        .padding(10)
    ...
    private func geminiAccountCard(_ account: GeminiAccount) -> some View {
        ...
        let statusText = Self.accountStatusText(health)
        ...
        return VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 4) {
                ...
                Circle()
                    .fill(credentialStatusColor(for: health))
                    .frame(width: 8, height: 8)
                    .help(accountError ?? statusText)
            }
            ...
            HStack(spacing: 8) {
                removeButton(.gemini(id: account.id, label: account.displayLabel))
                Spacer()
            }
        }
        .padding(10)
    ...
    /// Manual entry card for connecting one more Gemini key. A Google AI
    /// Studio key has no browser cookie to scan, so paste is the only route.
    private var addGeminiKeyCard: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Gemini")
                .font(.caption2)
                .foregroundStyle(.secondary)

            Text(appModel.settings.geminiAccounts.isEmpty && !appModel.hasGeminiAPIKey ? "Connect Gemini" : "Add another key")
                .font(.callout.weight(.semibold))
    ...
    private func claudeAccountCard(_ account: ClaudeAccount) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            ...
            if let accountError {
                CopyableErrorText(accountError, font: .caption, foregroundStyle: .orange)
            }
    ...
    private func providerCard(for provider: CredentialProvider) -> some View {
        ...
        let statusText = provider == .chatGPT && !appModel.settings.isChatGPTUsageShown
            ? "Monitoring off. Turn on ChatGPT usage in General settings."
            : Self.accountStatusText(health)
        ...
        return VStack(alignment: .leading, spacing: 6) {
            ...
            if let status {
                HStack(spacing: 8) {
                    if provider == .chatGPT {
                        excludeButton(.chatGPT(
                            id: ChatGPTAccount.unidentifiedId,
                            label: displayLabel
                        ))
                    }
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Show ChatGPT Usage")
                        .font(.subheadline)
                    Text("Optional. Connect ChatGPT in Accounts settings, then show ChatGPT plan quota usage in the popover.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    // MARK: - Subscription Reset Announcement Section

    private var subscriptionResetAnnouncementSection: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text("Subscription Reset Announcement")
                    .font(.subheadline)
                Text("Controls reset text shown below quota bars")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            Picker("Reset time display", selection: $appModel.settings.subscriptionResetAnnouncementMode) {
    ...
    private var thresholdsSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                .accessibilityValue("\(Int(warningThresholdValue)) percent")
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                .accessibilityValue("\(Int(criticalThresholdValue)) percent")
            ...
            HStack(alignment: .top, spacing: 32) {
                VStack(spacing: 16) {
                    ...
                    VStack(spacing: 6) {
                        ...
                        if let versionLabel = BuildInfo.versionLabel() {
                            Text(versionLabel)
                                .font(.caption)
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Toggle("Include Beta Updates", isOn: $appModel.settings.includeBetaUpdates)
                ...
                VStack(alignment: .leading, spacing: 18) {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Text("Sessions are imported from your browsers and stored in the macOS Keychain. Pinemeter sends credentials to their providers to retrieve usage data.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
...
private extension View {
    /// A settings pane's body: one column, padded, and centred once the window
    /// is wider than the column.
    ///
    /// A settings row is a label on the leading edge and a control on the
    /// trailing one, so an uncapped pane in a resizable window reads as two
    /// far-apart columns and stretches every caption into one very long line.
    /// 720pt is a little wider than the 680pt the window opens at, so nothing
    /// changes at the default size and only a deliberately widened window sees
    /// the cap. The Broker tab is exempt: its chain rows use the width.
    func settingsColumn(maxWidth: CGFloat = 720) -> some View {
        self

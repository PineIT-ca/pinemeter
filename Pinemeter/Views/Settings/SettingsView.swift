...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        // Broker configuration has its own window, so the remaining tabs
        // can use a smaller minimum size.
        .frame(
            minWidth: 540,
            maxWidth: .infinity,
            minHeight: 480,
...
extension View {
    ...
    /// 720pt is wider than the 600pt the window opens at, so nothing
    /// changes at the default size and only a deliberately widened window sees
    /// the cap. Broker configuration uses its own window.

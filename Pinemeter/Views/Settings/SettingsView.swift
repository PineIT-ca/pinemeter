...
struct SettingsView: View {
    ...
    static func selectBrokerTab() {
        TestSafeDefaults.standardOrIsolated.set(
            Tab.broker.rawValue,
            forKey: selectedTabDefaultsKey
        )
    }
    ...
    private func chatGPTAccountCard(_ account: ChatGPTAccount) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 4) {
                ...
                Circle()
                    .fill(chatGPTStatusColor(for: account, primaryStatus: primaryStatus))
                    .frame(width: 8, height: 8)
                    .help(accountError ?? primaryStatus?.stateText ?? "Connected")
            }
    ...
    private func geminiAccountCard(_ account: GeminiAccount) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 4) {
                ...
                Circle()
                    .fill(geminiStatusColor(for: account, primaryStatus: primaryStatus))
                    .frame(width: 8, height: 8)
                    .help(accountError ?? primaryStatus?.stateText ?? "Connected")
            }
    ...
    private var addGeminiKeyCard: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(appModel.settings.geminiAccounts.isEmpty ? "Gemini" : "Add key")
                .font(.callout.weight(.semibold))
    ...
    private func chatGPTStatusColor(
        ...
        return appModel.chatGPTAccountErrors[account.id] == nil ? .green : .orange
    }

    private func geminiStatusColor(
        for account: GeminiAccount,
        primaryStatus: AppProviderCredentialStatus?
    ) -> Color {
        if account.isPrimary {
            return credentialStatusColor(for: primaryStatus?.state.health ?? .unknown)
        }
        return appModel.geminiAccountErrors[account.id] == nil ? .green : .orange
    }
    ...
    private var subscriptionResetAnnouncementSection: some View {
        HStack {
            ...
            Picker("", selection: $appModel.settings.subscriptionResetAnnouncementMode) {

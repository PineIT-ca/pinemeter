...
struct BrokerSettingsTab: View {
    ...
    private static func restoredPane() -> Pane {
        TestSafeDefaults.standardOrIsolated
            ...
            .flatMap(Pane.init(rawValue:)) ?? .routing
    }
    ...
    var body: some View {
        ...
        .onChange(of: appModel.settings.broker.apiKeyMode) { _, newValue in
            Task {
                // Provisioning first: `ensureBrokerAPIKey` applies the change
                // itself when it generates a key, so the server never comes
                // back up demanding a key that does not exist yet.
                if newValue == .none {

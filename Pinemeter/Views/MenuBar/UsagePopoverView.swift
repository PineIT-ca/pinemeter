...
enum QuotaChartLayout {
    ...
    static func columnWidth(for bar: MenuBarQuotaBar) -> CGFloat {
        guard let detail = bar.detail else { return columnWidth }
        let longestLine = detail.components(separatedBy: "\n")
            .max(by: { $0.count < $1.count })
            ?? ""
        let textWidth = (longestLine as NSString).size(withAttributes: [
            .font: NSFont.systemFont(ofSize: NSFont.smallSystemFontSize)
        ]).width
        return max(columnWidth, ceil(textWidth) + 18)
    }
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            if let detail = bar.detail {
                ...
                .accessibilityLabel("Resets \(detail.replacingOccurrences(of: "\n", with: ", "))")
            }
        }

...
struct ChatGPTWHAMUsageResponse: Decodable, Equatable, Sendable {
    ...
    /// Account identity fields. These name *which* login the cookie belongs to,
    /// which is what lets several connected ChatGPT accounts be told apart.
    let userId: String?
    ...
    var identity: ChatGPTAccountIdentity {
...
struct ChatGPTWHAMAdditionalRateLimit: Decodable, Equatable, Sendable {
    ...
    /// Only the nested shape carries a second window; the flat shape never did.
    var resolvedSecondaryWindow: ChatGPTWHAMWindow? {
...
struct ChatGPTWHAMWindow: Decodable, Equatable, Sendable {
    ...
    /// Sub-day windows are session ("5h") windows, up to a fortnight reads as
    /// weekly, and anything longer is the 30-day window ChatGPT meters
    /// free-plan Codex usage over.
    var durationRole: ChatGPTUsageData.MenuBarQuotaRole? {
        ...
        default:
            return .chatGPTMonthly
        }
    }

    /// Short human name for the window length, used to keep two windows on the
    /// same feature apart ("Codex Spark 5h" vs "Codex Spark weekly").
    var durationName: String? {
        ...
        case ..<86_400:
            return "\(Int((limitWindowSeconds / 3_600).rounded()))h"
        case ..<1_209_600:
            return "weekly"
        default:
            return "monthly"
...
extension ChatGPTWHAMUsageResponse {
    func toDomain(lastUpdated: Date = Date()) throws -> ChatGPTUsageData {
        ...
        for additionalLimit in additionalRateLimits ?? [] {
            ...
            let baseLabel = menuBarRole?.menuBarLabel ?? Self.displayLabel(for: rawLabel)
            let baseHeading = menuBarRole?.columnHeading ?? Self.displayLabel(for: rawLabel)
            let subtitle = menuBarRole == nil ? "WHAM: \(rawLabel)" : "WHAM additional limit"

            // Mid-2026 the per-feature meters gained a second window: the
            // Codex-Spark meter now carries a 5h primary AND a weekly
            // secondary, so both are emitted and the window length is appended
            // to keep the two rows apart. Only the first row keeps the shared
            // role -- two rows with the same role collapse in `displayRows`.
            let windows = [
                (window: additionalLimit.resolvedPrimaryWindow, sourceSuffix: ""),
                (window: additionalLimit.resolvedSecondaryWindow, sourceSuffix: ".secondary_window"),
            ].compactMap { entry in
                entry.window.map { (window: $0, sourceSuffix: entry.sourceSuffix) }
            }

            for (index, entry) in windows.enumerated() {
                let suffix = windows.count > 1 ? entry.window.durationName : nil
                if let row = Self.row(
                    from: entry.window,
                    sourceLabel: rawLabel + entry.sourceSuffix,
                    displayLabel: Self.joined(baseLabel, suffix),
                    subtitle: subtitle,
                    menuBarRole: index == 0 ? menuBarRole : nil,
                    menuBarHeading: Self.joined(baseHeading, suffix)
                ) {
                    rows.append(row)
                }
            }

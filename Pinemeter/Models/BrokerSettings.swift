...
/// One remote broker preset manifest and its fetch bookkeeping.
struct BrokerPresetManifestSource: Codable, Equatable, Identifiable, Sendable {
    var id: UUID
    var urlString: String
    var lastCheckedAt: Date?
    var etag: String?
    var lastError: String?
    /// Last validated presets from this source. Keeping them per source is
    /// what lets a 304 or one failed fetch coexist with another source's update.
    var cachedPresets: [BrokerAgentProfile]
    var cachedAgentSetup: BrokerAgentSetupNotice?
    ...
    init(
        ...
        cachedPresets: [BrokerAgentProfile] = [],
        cachedAgentSetup: BrokerAgentSetupNotice? = nil
    ) {
        self.id = id
        ...
        self.cachedPresets = cachedPresets
        self.cachedAgentSetup = cachedAgentSetup
    }
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case cachedPresets = "cached_presets"
        case cachedAgentSetup = "cached_agent_setup"
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        cachedPresets = try container.decodeIfPresent(
            [BrokerAgentProfile].self, forKey: .cachedPresets
        ) ?? []
        cachedAgentSetup = try container.decodeIfPresent(
            BrokerAgentSetupNotice.self, forKey: .cachedAgentSetup
        )
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(cachedPresets, forKey: .cachedPresets)
        try container.encodeIfPresent(cachedAgentSetup, forKey: .cachedAgentSetup)
    }
}

/// Settings for remote broker preset manifests: small GitHub-hosted JSON
/// files (``BrokerPresetManifest``) offering extra named rule profiles the
/// user may explicitly apply. Fetching only downloads presets into
/// ``BrokerSettings/remotePresets`` — it never applies one (see
/// ``BrokerSettings/updateRemotePresets(_:)``).
///
...
struct BrokerPresetManifestConfig: Codable, Equatable, Sendable {
    ...
    var isEnabled: Bool
    /// Fetch order is also merge precedence: the first occurrence of an id wins.
    var sources: [BrokerPresetManifestSource]
    ...
    static let `default` = BrokerPresetManifestConfig(
        isEnabled: true,
        sources: [BrokerPresetManifestSource(urlString: defaultURLString)]
    )

    init(
        isEnabled: Bool = true,
        sources: [BrokerPresetManifestSource] = [
            BrokerPresetManifestSource(urlString: defaultURLString)
        ]
    ) {
    ...
    /// Source-compatible initializer for callers constructing the old shape.
    init(
        isEnabled: Bool = true,
        urlString: String,
        lastCheckedAt: Date? = nil,
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case sources
        case legacyURL = "url"
        case legacyURLString = "url_string"
        case lastCheckedAt = "last_checked_at"
        case etag
        case lastError = "last_error"
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        if let decodedSources = try container.decodeIfPresent(
            [BrokerPresetManifestSource].self, forKey: .sources
        ) {
            // First occurrence of an id wins. A hand-edited or merged save can
            // carry the same id twice, and every lookup here is `firstIndex(by
            // id)`: the second row would never be fetched, never show its own
            // error, and hand `ForEach` a duplicate `Identifiable` id.
            var seen = Set<UUID>()
            sources = decodedSources.filter { seen.insert($0.id).inserted }
            return
        }

        // Saves before multi-source support stored one URL and its state at
        // this level. Preserve every value in exactly one migrated source.
        let decodedURLString = try container.decodeIfPresent(String.self, forKey: .legacyURLString)
            ?? container.decodeIfPresent(String.self, forKey: .legacyURL)
        sources = [BrokerPresetManifestSource(
            urlString: (decodedURLString?.isEmpty == false) ? decodedURLString! : Self.defaultURLString,
            lastCheckedAt: try container.decodeIfPresent(Date.self, forKey: .lastCheckedAt),
            etag: try container.decodeIfPresent(String.self, forKey: .etag),
            lastError: try container.decodeIfPresent(String.self, forKey: .lastError)
        )]
    ...
    // Keep source compatibility while concurrent callers move to `sources`.
    var urlString: String {
        get { sources.first?.urlString ?? "" }
        set { updateFirstSource { $0.urlString = newValue } }
    }

    var lastCheckedAt: Date? {
        get { sources.first?.lastCheckedAt }
        set { updateFirstSource { $0.lastCheckedAt = newValue } }
    }

    var etag: String? {
        get { sources.first?.etag }
        set { updateFirstSource { $0.etag = newValue } }
    }

    var lastError: String? {
        get { sources.first?.lastError }
        set { updateFirstSource { $0.lastError = newValue } }
    }

    private mutating func updateFirstSource(_ update: (inout BrokerPresetManifestSource) -> Void) {
        if sources.isEmpty {
            sources.append(BrokerPresetManifestSource(urlString: Self.defaultURLString))
        }
        update(&sources[0])
    }
}

/// One durable recovery point for the most recently applied routing update.
struct BrokerRoutingRecovery: Codable, Equatable, Sendable {
    let profileID: UUID
    let profileName: String
    let previousRules: BrokerRuleSet
    let previousBaseline: BrokerRuleSet
    let appliedRules: BrokerRuleSet
    var origin: String? = nil
    var source: BrokerManifestHistoryNamespace? = nil
    var revisionFingerprint: String? = nil

    private enum CodingKeys: String, CodingKey {
        case profileID, profileName, previousRules, previousBaseline, appliedRules
        case origin, source, revisionFingerprint
    }

    init(profileID: UUID, profileName: String, previousRules: BrokerRuleSet,
         previousBaseline: BrokerRuleSet, appliedRules: BrokerRuleSet,
         origin: String? = nil, source: BrokerManifestHistoryNamespace? = nil,
         revisionFingerprint: String? = nil) {
        self.profileID = profileID
        self.profileName = profileName
        self.previousRules = previousRules
    ...
    init(from decoder: Decoder) throws {
        ...
        previousRules = try container.decode(BrokerRuleSet.self, forKey: .previousRules)
        previousBaseline = try container.decode(BrokerRuleSet.self, forKey: .previousBaseline)
        appliedRules = try container.decode(BrokerRuleSet.self, forKey: .appliedRules)
        // Presentation metadata must never invalidate otherwise usable settings.
        origin = try? container.decode(String.self, forKey: .origin)
        source = try? container.decode(BrokerManifestHistoryNamespace.self, forKey: .source)
        revisionFingerprint = try? container.decode(String.self, forKey: .revisionFingerprint)
    }
}
...
struct BrokerSettings: Codable, Equatable, Sendable {
    ...
    /// Broker window's `portFormatter` bounds (`BrokerStatusHeader.swift`).
    ...
    var routingRecovery: BrokerRoutingRecovery? = nil

    /// Whether the user dismissed the "previous routing is safe" banner from
    /// the menu bar popover. Dismissal hides it there only: the Broker window
    /// keeps showing it so Undo Update stays reachable for as long as
    /// ``canUndoRoutingUpdate`` holds. Reset whenever a new recovery point is
    /// recorded, so the next update announces itself again.
    var isRoutingRecoveryDismissedInPopover: Bool = false
    ...
    /// Whether a manifest refresh may announce new or updated routing rules.
    var routingUpdateNotificationsEnabled: Bool

    /// Remote preset ids already observed by a completed manifest refresh.
    var seenRemotePresetIDs: Set<UUID>

    /// Last routing-update notification payload successfully delivered.
    var lastRoutingUpdateNotifiedFingerprint: String?

    var effectiveAgentSetup: BrokerAgentSetupNotice? {
        presetManifest.sources.compactMap(\.cachedAgentSetup).max {
    ...
    static let newRolesMigrationID = "new-roles"
    static let automaticModelRoutingMigrationID = "automatic-model-routing"
    static let heavyCodexRouteMigrationID = "heavy-codex-route"
    ...
    static let knownRoutingMigrations: Set<String> = [
        ...
        newRolesMigrationID,
        automaticModelRoutingMigrationID,
        heavyCodexRouteMigrationID,
    ]
    ...
    static let `default` = BrokerSettings(
        ...
        routingUpdateNotificationsEnabled: true,
        seenRemotePresetIDs: [],
        lastRoutingUpdateNotifiedFingerprint: nil,
    ...
    init(
        ...
        routingUpdateNotificationsEnabled: Bool = true,
        seenRemotePresetIDs: Set<UUID> = [],
        lastRoutingUpdateNotifiedFingerprint: String? = nil,
        ...
        self.routingUpdateNotificationsEnabled = routingUpdateNotificationsEnabled
        self.seenRemotePresetIDs = seenRemotePresetIDs
        self.lastRoutingUpdateNotifiedFingerprint = lastRoutingUpdateNotifiedFingerprint
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case isRoutingRecoveryDismissedInPopover = "routing_recovery_dismissed_in_popover"
        case recheckReminderEnabled = "recheck_reminder_enabled"
        case routingUpdateNotificationsEnabled = "routing_update_notifications_enabled"
        case seenRemotePresetIDs = "seen_remote_preset_ids"
        case lastRoutingUpdateNotifiedFingerprint = "last_routing_update_notified_fingerprint"
    ...
    init(from decoder: Decoder) throws {
        ...
        routingUpdateNotificationsEnabled = try container.decodeIfPresent(
            Bool.self, forKey: .routingUpdateNotificationsEnabled
        ) ?? true
        // Absent means a save written before routing-update notifications
        // existed. The presets already on disk were seen long ago, so seed
        // from them; an empty set would announce every cached preset as new
        // on the first refresh after the upgrade.
        if let decodedSeen = try container.decodeIfPresent(
            Set<UUID>.self, forKey: .seenRemotePresetIDs
        ) {
            seenRemotePresetIDs = decodedSeen
        } else {
            seenRemotePresetIDs = Set(remotePresets.map(\.id))
        }
        lastRoutingUpdateNotifiedFingerprint = try container.decodeIfPresent(
            String.self, forKey: .lastRoutingUpdateNotifiedFingerprint
        )
        ...
        if container.contains(.activeProfileID) {
            ...
            let storedRules = policy.ruleSet.modelRouted
        ...
        routingRecovery = try container.decodeIfPresent(BrokerRoutingRecovery.self, forKey: .routingRecovery)
        isRoutingRecoveryDismissedInPopover = try container.decodeIfPresent(
            Bool.self, forKey: .isRoutingRecoveryDismissedInPopover
        ) ?? false
        ...
        if activeProfileRules == nil {
            // Compiled built-ins only, deliberately not `activeProfile`: that
            // resolves a built-in through the manifest, so a save with remote
            // presets and no pin would pin manifest-supplied rules with no
            // click behind them. No such save can exist today (pinning shipped
            // before remote presets did), and this keeps it that way by
            // construction rather than by chronology.
            activeProfileRules = activeProfileID.flatMap { id in
                BrokerAgentProfile.builtIn(id: id) ?? profiles.first { $0.id == id }
            }?.rules
        }
    ...
    private mutating func applyPendingRoutingMigrations() {
        ...
        if !appliedRoutingMigrations.contains(BrokerSettings.standardCodexRouteMigrationID),
           ...
           !standardChain.contains(where: { codexRoutes.contains($0.route) }) {
            // Appends the current Sol tier, so on a bundled seed it collapses
            // into the chain's own gpt-6-sol entry instead of adding a row.
            let codexCandidates = [
                BrokerCandidate(route: .auto, model: "gpt-6-sol", effort: .low),
            ]
            standardChain.append(contentsOf: codexCandidates)
            policy.roles["standard"] = BrokerPolicy.automaticModelChoices(in: standardChain)
            if var pinnedChain = activeProfileRules?.roles["standard"],
               !pinnedChain.contains(where: { codexRoutes.contains($0.route) }) {
                ...
                activeProfileRules?.roles["standard"] = BrokerPolicy.automaticModelChoices(in: pinnedChain)
            }
        }

        if !appliedRoutingMigrations.contains(BrokerSettings.newRolesMigrationID) {
            for role in ["explore", "verification"] {
                guard let shippedChain = BrokerPolicy.bundledDefault.roles[role] else { continue }
                if policy.roles[role] == nil {
                    policy.roles[role] = shippedChain
                }
                if var pinned = activeProfileRules, pinned.roles[role] == nil {
                    pinned.roles[role] = shippedChain
                    activeProfileRules = pinned
                }
                // User-authored profiles are applied wholesale, so a saved
                // profile that predates these roles would delete them from the
                // live policy on its next pick. Seed them there too.
                for index in profiles.indices where profiles[index].rules.roles[role] == nil {
                    profiles[index].rules.roles[role] = shippedChain
                }
            }
        }

        if !appliedRoutingMigrations.contains(BrokerSettings.automaticModelRoutingMigrationID) {
            policy.roles = policy.roles.mapValues(BrokerPolicy.automaticModelChoices)
            if var pinned = activeProfileRules {
                pinned.roles = pinned.roles.mapValues(BrokerPolicy.automaticModelChoices)
                activeProfileRules = pinned
            }
        }

        if !appliedRoutingMigrations.contains(BrokerSettings.heavyCodexRouteMigrationID),
           let codexRoutes = policy.callers["codex"]?.routes,
           var heavyChain = policy.roles["heavy"],
           !heavyChain.contains(where: { codexRoutes.contains($0.route) }) {
            let codexCandidates = [
                BrokerCandidate(route: .auto, model: "gpt-6-sol", effort: .high),
            ]
            heavyChain.append(contentsOf: codexCandidates)
            policy.roles["heavy"] = BrokerPolicy.automaticModelChoices(in: heavyChain)
            if var pinnedChain = activeProfileRules?.roles["heavy"],
               !pinnedChain.contains(where: { codexRoutes.contains($0.route) }) {
                pinnedChain.append(contentsOf: codexCandidates)
                activeProfileRules?.roles["heavy"] = BrokerPolicy.automaticModelChoices(in: pinnedChain)
            }
        }
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(routingUpdateNotificationsEnabled, forKey: .routingUpdateNotificationsEnabled)
        try container.encode(seenRemotePresetIDs, forKey: .seenRemotePresetIDs)
        try container.encodeIfPresent(
            lastRoutingUpdateNotifiedFingerprint,
            forKey: .lastRoutingUpdateNotifiedFingerprint
        )
        try container.encode(appliedRoutingMigrations.sorted(), forKey: .appliedRoutingMigrations)
        ...
        try container.encodeIfPresent(routingRecovery, forKey: .routingRecovery)
        try container.encode(isRoutingRecoveryDismissedInPopover, forKey: .isRoutingRecoveryDismissedInPopover)
    }
}

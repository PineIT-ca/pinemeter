...
enum BrokerInvocation: Codable, Equatable, Sendable {
    ...
    case codexExec(model: String)
    /// A visible thread in the T3 desktop app, on a named provider instance.
    case t3Dispatch(model: String, instanceId: String)
    ...
    var model: String {
        switch self {
        case .agent(let model): return model
        case .codexExec(let model): return model
        case .t3Dispatch(let model, _): return model
        }
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        if case .t3Dispatch(_, let instanceId) = self {
    ...
    init(from decoder: Decoder) throws {
        ...
        case "codex-exec":
            self = .codexExec(model: model)
        case "t3-dispatch":
            let instanceId = try container.decode(String.self, forKey: .instanceId)
            self = .t3Dispatch(model: model, instanceId: instanceId)
        default:
...
struct BrokerDecision: Codable, Equatable, Sendable {
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case reason, source, oracle, degraded, candidatesTried
    }

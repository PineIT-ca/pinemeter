...
extension UsageLimit {
    ...
    ///
    /// The elapsed-time divisor is clamped to
    /// `Constants.Pacing.minimumElapsedFraction`: before that much of the
    /// window has run, the raw ratio is dominated by the divisor rather than by
    /// the burn rate. Clamping rather than abstaining keeps a genuine early
    /// runaway detectable.
    ...
    func isAtRisk(windowDuration: TimeInterval, now: Date = Date()) -> Bool {
        ...
        // Clamped: a window that has barely opened has no meaningful divisor.
        let pace = usagePct / max(timeElapsedPct, Constants.Pacing.minimumElapsedFraction)

        return pace > Constants.Pacing.riskThreshold
    }
}

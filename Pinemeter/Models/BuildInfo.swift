...
enum BuildInfo {
    ...
    /// Allowlisted support details. Never includes account or machine identifiers.
    static func supportDetails(
        bundle: Bundle = .main,
        macOSVersion: OperatingSystemVersion = ProcessInfo.processInfo.operatingSystemVersion
    ) -> String {
        let version = normalized(bundle.infoDictionary?["CFBundleShortVersionString"] as? String) ?? "Unknown"
        let build = normalized(bundle.infoDictionary?["CFBundleVersion"] as? String) ?? "Unknown"
        return "Version: \(version)\nBuild: \(build)\nmacOS: \(macOSVersion.majorVersion).\(macOSVersion.minorVersion).\(macOSVersion.patchVersion)"
    }

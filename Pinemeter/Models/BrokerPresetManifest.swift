...
//  named rule profiles (``BrokerAgentProfile``) the user may explicitly apply.
//  An entry either adds a profile of its own or, by carrying a built-in's id,
//  publishes updated rules for that built-in — which is how routing reaches an
//  existing install without an app update. Fetched frequently by
//  ``PresetManifestService`` and never routes anything by itself — see
...
struct BrokerAgentSetupNotice: Codable, Equatable, Sendable {
    let revision: Int
...
struct BrokerPresetManifest: Equatable, Sendable {
    ...
    var agentSetup: BrokerAgentSetupNotice?
    ...
    init(
        ...
        agentSetup: BrokerAgentSetupNotice? = nil,
        manifestRevision: Int? = nil,
    ...
    static let maxSetupSummaryScalars = 240
    static let maxSummaryScalars = 240
    ...
    static func decode(from data: Data) throws -> BrokerPresetManifest {
        ...
        // A built-in id is allowed here, and is the point: it publishes new
        // rules for that built-in, which is the only way an install already
        // sitting on one can be re-routed without an app update. What the
        // entry cannot do is take the built-in's identity — name, detail and
        // symbol stay compiled (``BrokerSettings/effectiveBuiltInProfiles``),
        // so a manifest can never rename "Balanced" into something it is not,
        // and cannot route by itself either: the updated rules reach the
        // policy only when the user adopts them.
        ...
        return BrokerPresetManifest(
            schemaVersion: schemaVersion,
            presets: accepted,
            agentSetup: decodeAgentSetup(object["agent_setup"] as? [String: Any]),
            manifestRevision: (object["manifest_revision"] as? Int).flatMap { $0 >= 0 ? $0 : nil },
            publishedAt: (object["published_at"] as? String).flatMap(validPublishedDate),
            summary: (object["summary"] as? String).flatMap {
                let sanitized = sanitize($0, maxScalars: maxSummaryScalars)
                return sanitized.isEmpty ? nil : sanitized
            }
        )
    }

    private static func decodeAgentSetup(_ object: [String: Any]?) -> BrokerAgentSetupNotice? {
        guard let object,
              let revision = object["revision"] as? Int,
              revision >= 0 else { return nil }

        let changedAt = (object["changed_at"] as? String).flatMap(validISODate)
        let summary = (object["summary"] as? String).flatMap {
            let sanitized = sanitize($0, maxScalars: maxSetupSummaryScalars)
            return sanitized.isEmpty ? nil : sanitized
        }
        return BrokerAgentSetupNotice(revision: revision, changedAt: changedAt, summary: summary)
    }

    private static func validISODate(_ value: String) -> String? {
    ...
    static func validAgentSetupDate(_ value: String) -> Bool {
        validISODate(value) != nil
    }

    private static func validPublishedDate(_ value: String) -> Date? {
        ...
        guard let date = dayFormatter.date(from: value),
              dayFormatter.string(from: date) == value else { return nil }
        return date
    }
    ...
    /// the string.
    static func sanitize(_ raw: String, maxScalars: Int) -> String {

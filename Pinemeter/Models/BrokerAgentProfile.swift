...
extension BrokerAgentProfile {
    ...
    static let conserveQuota: BrokerAgentProfile = {
        ...
        rules.setChain(for: judgmentRoles, [
            ...
            BrokerCandidate(route: .native, model: "claude-opus-5", effort: .medium),
...
extension BrokerSettings {
    ...
    var routingUpdateChanges: [String] {
        ...
        func describe(_ chain: [BrokerCandidate]) -> String {
            guard !chain.isEmpty else { return "No models" }
            return chain.map { candidate in
                candidate.id + (candidate.effort.map { " (\($0.rawValue))" } ?? "")
            }.joined(separator: ", ")
        }
        var changes = Set(old.roles.keys).union(new.roles.keys).sorted().compactMap { role -> String? in
            guard old.roles[role] != new.roles[role] else { return nil }
            return "\(role.capitalized)\nLast applied: \(describe(old.roles[role] ?? []))\nNew: \(describe(new.roles[role] ?? []))"
        }
        if old.thresholds != new.thresholds { changes.append("Quota limits updated.") }
        if old.models != new.models { changes.append("Model availability, quota tracking, or reasoning options updated.") }
        if old.callers != new.callers || old.allowForcedDegraded != new.allowForcedDegraded {
            changes.append("Agent access or fallback permissions updated.")
        }
        if old.instanceByModel != new.instanceByModel || old.defaultInstance != new.defaultInstance
            ...
            changes.append("Model connection preferences updated.")
        }
        return changes
    }

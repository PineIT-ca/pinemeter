...
struct BrokerRuleSet: Codable, Equatable, Sendable {
    ...
    /// Every T3 instance id these rules name, in any of the three places a
    /// rule can name one. Drives the "this profile references instances this
    /// Mac doesn't have" caption after a load or import.
    var referencedInstanceIDs: [String] {
        var ids = Set(instanceByModel.values)
        ids.insert(defaultInstance)
        for chain in roles.values {
            for candidate in chain {
                if let instance = candidate.instance { ids.insert(instance) }
            }
        }
        return ids.sorted()
    }
...
struct BrokerAgentProfile: Codable, Equatable, Sendable, Identifiable {
    ...
    init(from decoder: Decoder) throws {
        ...
        isBuiltIn = false
        rules = try container.decodeIfPresent(BrokerRuleSet.self, forKey: .rules) ?? .default
    }
...
extension BrokerAgentProfile {
    ...
    private static let ruleSetKeys: Set<String> = [
    ...
    static func decodeExported(from data: Data) throws -> BrokerAgentProfile {
        guard let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              ...
              !ruleSetKeys.isDisjoint(with: rules.keys)
        else {
...
extension BrokerAgentProfile {
    ...
    static let conserveQuota: BrokerAgentProfile = {
        ...
        // Second-account and cheaper lanes first, so the primary subscription
        // is the last thing spent rather than the first.
...
private extension BrokerRuleSet {
    ...
    mutating func setChain(for roleNames: [String], _ chain: [BrokerCandidate]) {
        let clamped = chain.map { $0.clampingEffortToModelSupport() }
...
extension BrokerSettings {
    /// Built-ins first, then the user's own profiles by name.
    ...
    /// state (instances, ignore list, usage lanes) is preserved.
    mutating func applyProfile(id: UUID) {
        guard let profile = BrokerAgentProfile.builtIn(id: id) ?? profiles.first(where: { $0.id == id })
        else { return }
    ...
    mutating func importProfile(_ imported: BrokerAgentProfile) -> UUID {
        let existingIDs = Set(profiles.map(\.id)).union(BrokerAgentProfile.builtIns.map(\.id))
    ...
    func uniqueProfileName(_ base: String, excluding excludedID: UUID? = nil) -> String {
        ...
        let taken = Set(
            (profiles.filter { $0.id != excludedID }.map(\.name)
                + BrokerAgentProfile.builtIns.map(\.name)).map { $0.lowercased() }
        )

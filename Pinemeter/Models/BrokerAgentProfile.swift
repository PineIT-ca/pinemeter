...
struct BrokerRuleSet: Codable, Equatable, Sendable {
    ...
    /// Portable quota and effort capabilities keyed by model slug.
    var models: [String: BrokerModelSpec]
    ...
    init(
        ...
        models: [String: BrokerModelSpec] = BrokerPolicy.default.models,
    ...
    init(from decoder: Decoder) throws {
        ...
        models = try container.decodeIfPresent([String: BrokerModelSpec].self, forKey: .models)
            ?? defaults.models
        guard BrokerModelSpec.hasValidKeys(models) else {
            throw DecodingError.dataCorruptedError(
                forKey: .models, in: container, debugDescription: "Invalid portable model catalog"
            )
        }
...
extension BrokerPolicy {
    ...
    mutating func apply(_ rules: BrokerRuleSet) {
        assignExactRules(rules.modelRouted)
    }

    /// Restoration must preserve explicit routes, order, and captured edits.
    mutating func assignExactRules(_ rules: BrokerRuleSet) {
        roles = rules.roles
...
struct BrokerRoutingRoleChange: Equatable, Sendable {
    let role: String
    let previous: [BrokerCandidate]
    let proposed: [BrokerCandidate]
    let proposedUsesDefaultRules: Bool
}

struct BrokerRoutingThresholdChange: Equatable, Sendable {
    enum Field: CaseIterable, Equatable, Sendable {
        case sessionPct
        case weeklyPct
        case sonnetWeeklyPct
        case fableWeeklyPct
        case chatgptWeeklyPct
        case stalenessSeconds
        case preferExpiringQuota

        var label: String {
            switch self {
            case .sessionPct: return "Session"
            case .weeklyPct: return "Weekly"
            case .sonnetWeeklyPct: return "Sonnet weekly"
            case .fableWeeklyPct: return "Fable weekly"
            case .chatgptWeeklyPct: return "ChatGPT weekly"
            case .stalenessSeconds: return "Data staleness"
            case .preferExpiringQuota: return "Prefer expiring quota"
            }
        }

        fileprivate func value(in thresholds: BrokerThresholds) -> String {
            switch self {
            case .sessionPct: return Self.percent(thresholds.sessionPct)
            case .weeklyPct: return Self.percent(thresholds.weeklyPct)
            case .sonnetWeeklyPct: return Self.percent(thresholds.sonnetWeeklyPct)
            case .fableWeeklyPct: return Self.percent(thresholds.fableWeeklyPct)
            case .chatgptWeeklyPct: return Self.percent(thresholds.chatgptWeeklyPct)
            case .stalenessSeconds: return "\(Self.number(thresholds.stalenessSeconds)) seconds"
            case .preferExpiringQuota: return thresholds.preferExpiringQuota ? "On" : "Off"
            }
        }
...
enum BrokerRoutingChange: Equatable, Sendable {
    case role(BrokerRoutingRoleChange)
    case thresholds([BrokerRoutingThresholdChange])
    case modelCapabilities
    case callerPolicy
    case connections

    fileprivate var ordinaryDescription: String {
        switch self {
        case .role(let change):
            return "\(change.role.capitalized)\nLast applied: \(Self.describe(change.previous))\nNew: \(Self.describe(change.proposed))"
        case .thresholds:
            return "Quota limits updated."
        case .modelCapabilities:
            return "Model availability, quota tracking, or reasoning options updated."
        case .callerPolicy:
            return "Agent access or fallback permissions updated."
        case .connections:
            return "Model connection preferences updated."
        }
    }

    static func describe(_ chain: [BrokerCandidate], includesRoute: Bool = true) -> String {
        guard !chain.isEmpty else { return "No models" }
        return chain.map { candidate in
            (includesRoute ? candidate.id : candidate.model)
                + (candidate.effort.map { " (\($0.rawValue))" } ?? "")
        }.joined(separator: ", ")
    }
}
...
extension BrokerAgentProfile {
    ...
    static let ruleSetKeys: Set<String> = [
        ...
        "agent_model_aliases", "models", "instance_by_model", "default_instance",
    ]
...
extension BrokerAgentProfile {
    ...
    static let judgmentRoles = ["design", "planning", "research", "review"]
    ...
    static let conserveQuota: BrokerAgentProfile = {
        ...
        // T3 and cheaper lanes go first. Opus medium stays ahead of Haiku for
        // judgment because Haiku scores poorly there, while Terra and Luna
        // reduce Codex credit burn for execution and short standard work.
        rules.setChain(for: judgmentRoles, [
            ...
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .medium),
            BrokerCandidate(route: .codex, model: "gpt-5.6-sol", effort: .high),
            BrokerCandidate(route: .native, model: "claude-haiku-4-5-20251001"),
        ])
        rules.models.removeValue(forKey: "gpt-6-astra")
        rules.instanceByModel.removeValue(forKey: "gpt-6-astra")
        rules.setChain(for: ["explore"], [
            BrokerCandidate(route: .native, model: "claude-haiku-4-5-20251001"),
            BrokerCandidate(route: .t3, model: "gpt-5.6-luna", effort: .medium),
            BrokerCandidate(route: .codex, model: "gpt-5.6-luna", effort: .medium),
            BrokerCandidate(route: .t3, model: "gpt-5.6-terra", effort: .medium),
            BrokerCandidate(route: .codex, model: "gpt-5.6-terra", effort: .medium),
        ])
        rules.setChain(for: ["verification"], [
            BrokerCandidate(route: .t3, model: "gpt-5.6-terra", effort: .medium),
            BrokerCandidate(route: .codex, model: "gpt-5.6-terra", effort: .medium),
        ...
        rules.setChain(for: ["heavy"], [
            BrokerCandidate(route: .native, model: "claude-sonnet-5", effort: .high),
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .high),
            BrokerCandidate(route: .t3, model: "gpt-5.6-sol", effort: .high),
            BrokerCandidate(route: .codex, model: "gpt-5.6-sol", effort: .high),
        ])
        rules.setChain(for: ["standard"], [
            BrokerCandidate(route: .native, model: "claude-haiku-4-5-20251001"),
            BrokerCandidate(route: .t3, model: "gpt-5.6-luna", effort: .high),
            BrokerCandidate(route: .codex, model: "gpt-5.6-luna", effort: .high),
            BrokerCandidate(route: .t3, model: "gpt-5.6-terra", effort: .medium),
            BrokerCandidate(route: .codex, model: "gpt-5.6-terra", effort: .medium),
            BrokerCandidate(route: .native, model: "claude-sonnet-5", effort: .medium),
        ])
    ...
    static let maxQuality: BrokerAgentProfile = {
        var rules = BrokerRuleSet.default
        // Max Quality keeps the strongest judgment chain and raises Sol to
        // xhigh for heavy work, where harder coding benefits most from effort.
        rules.setChain(for: ["design"], [
            BrokerCandidate(route: .native, model: "claude-fable-5-1"),
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .xhigh),
            BrokerCandidate(route: .codex, model: "gpt-5.6-sol", effort: .high),
        ])
        rules.setChain(for: ["planning"], [
            BrokerCandidate(route: .native, model: "claude-fable-5-1"),
            BrokerCandidate(route: .codex, model: "gpt-6-astra", effort: .low),
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .xhigh),
        ])
        rules.setChain(for: ["research"], [
            BrokerCandidate(route: .native, model: "claude-fable-5-1"),
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .xhigh),
            BrokerCandidate(route: .codex, model: "gpt-5.6-sol", effort: .high),
        ])
        rules.setChain(for: ["review"], [
            BrokerCandidate(route: .codex, model: "gpt-6-astra", effort: .low),
            BrokerCandidate(route: .native, model: "claude-fable-5-1"),
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .xhigh),
        ])
        rules.setChain(for: ["explore"], [
            BrokerCandidate(route: .native, model: "claude-sonnet-5", effort: .medium),
            BrokerCandidate(route: .t3, model: "gpt-5.6-terra", effort: .high),
            BrokerCandidate(route: .codex, model: "gpt-5.6-terra", effort: .high),
            BrokerCandidate(route: .native, model: "claude-haiku-4-5-20251001"),
        ])
        rules.setChain(for: ["verification"], [
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .high),
        ...
        rules.setChain(for: ["heavy"], [
            BrokerCandidate(route: .codex, model: "gpt-6-astra", effort: .medium),
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .xhigh),
            BrokerCandidate(route: .native, model: "claude-fable-5-1"),
            BrokerCandidate(route: .native, model: "claude-sonnet-5", effort: .xhigh),
            BrokerCandidate(route: .t3, model: "gpt-5.6-sol", effort: .xhigh),
            BrokerCandidate(route: .codex, model: "gpt-5.6-sol", effort: .xhigh),
        ])
        rules.setChain(for: ["standard"], [
            BrokerCandidate(route: .native, model: "claude-sonnet-5", effort: .high),
            BrokerCandidate(route: .native, model: "claude-opus-5-5", effort: .high),
        ...
        // A ceiling gates what the broker DISPATCHES, not what gets spent:
        // threads already running keep consuming, and Codex CLI use outside the
        // broker consumes the same window. So the true usage overshoots the
        // ceiling by whatever is in flight, and a ceiling set at 97 leaves no
        // room to absorb that — on 2026-09-17 the ChatGPT lane stopped taking
        // picks at 97% and still reached 100%.
        //
        // The ChatGPT window is the one lane every other role competes for
        // (`execution` and `standard` reach it through gpt-5.6-sol), so it
        // returns to the shipped default of 90 and keeps a reserve. The Claude
        // ceilings stay high: there are two accounts behind them, so exhausting
        // one is not the same kind of event.
        rules.thresholds = BrokerThresholds(
            sessionPct: 97,
            weeklyPct: 95,
            sonnetWeeklyPct: 97,
        ...
        // A degraded pick is not a silent downgrade: the contract makes the
        // calling harness confirm one with the user before dispatching it, so
        // the only thing the hard stop buys is removing that choice. `review`
        // is the highest-volume judgment role and its chain draws on three
        // quota pools in total, so it is the one that actually reaches
        // exhaustion — and a downgrade the user agreed to beats dead-ending
        // the completion gate. The other judgment roles run rarely enough that
        // the hard stop still costs less than a quietly worse plan, so they
        // keep it.
        rules.allowForcedDegraded = Dictionary(
            uniqueKeysWithValues: judgmentRoles.map { ($0, $0 == "review") }
        )
        return BrokerAgentProfile(
            id: maxQualityID,
            name: "Max Quality",
    ...
    static let offloadExecution: BrokerAgentProfile = {
        var rules = BrokerRuleSet.default
        // Codex stays first for writing code. Terra leads standard work at
        // half Sol's credit rate, while Sol remains the compatibility fallback.
        rules.setChain(for: ["execution"], [
            BrokerCandidate(route: .codex, model: "gpt-5.6-sol", effort: .high),
            BrokerCandidate(route: .t3, model: "gpt-5.6-sol", effort: .high),
            BrokerCandidate(route: .native, model: "claude-sonnet-5", effort: .high),
        ...
        rules.setChain(for: ["explore"], [
            ...
            BrokerCandidate(route: .t3, model: "gpt-5.6-terra", effort: .medium),
            BrokerCandidate(route: .native, model: "claude-haiku-4-5-20251001"),
            BrokerCandidate(route: .native, model: "claude-sonnet-5", effort: .low),
        ])
        rules.setChain(for: ["verification"], [
            BrokerCandidate(route: .codex, model: "gpt-5.6-terra", effort: .high),
            BrokerCandidate(route: .t3, model: "gpt-5.6-terra", effort: .high),
...
extension BrokerRuleSet {
    var fillingMissingRoles: BrokerRuleSet {
        var rules = self
        for (role, chain) in BrokerPolicy.bundledDefault.roles where rules.roles[role] == nil {
            rules.roles[role] = chain
        }
        return rules
    }
    ...
    mutating func setChain(for roleNames: [String], _ chain: [BrokerCandidate]) {
        let clamped = BrokerPolicy.automaticModelChoices(in: chain)
            .map { $0.clampingEffortToModelSupport(in: models) }
...
extension BrokerSettings {
    /// The built-ins in menu order, each carrying the manifest's rules for it
    /// when the manifest publishes an entry under its id.
    ///
    /// Rules only. Name, detail and symbol stay as compiled, so an updated
    /// built-in is still recognisably the profile the user picked and a
    /// manifest cannot rebrand one; and `isBuiltIn` stays true, so the
    /// "can't save into this" rules the bar already applies are unchanged.
    /// This is what lets shipped routing be corrected between app releases:
    /// the profile the user sits on gains updated rules, the existing pin
    /// comparison lights up "Updated", and adopting them stays one explicit
    /// click (``activeProfileHasUpdatedRules``).
    var effectiveBuiltInProfiles: [BrokerAgentProfile] {
        BrokerAgentProfile.builtIns.map { builtIn in
            guard let override = remotePresets.first(where: { $0.id == builtIn.id }) else {
                return builtIn
            }
            var updated = builtIn
            updated.rules = override.rules
            return updated
        }
    }

    /// Manifest presets that add a profile of their own, i.e. everything the
    /// "From Manifest" section should list. An entry that updates a built-in
    /// is excluded: it already renders in its built-in slot, and listing it
    /// twice would offer the same id as two different-looking choices.
    var additionalRemotePresets: [BrokerAgentProfile] {
        let builtInIDs = Set(BrokerAgentProfile.builtIns.map(\.id))
        return remotePresets.filter { !builtInIDs.contains($0.id) }
    }

    /// Built-ins (manifest-updated where applicable), then the manifest's own
    /// presets in manifest order, then the user's profiles by name.
    var allProfiles: [BrokerAgentProfile] {
        effectiveBuiltInProfiles
            + additionalRemotePresets
    ...
    ///
    /// Built-ins resolve through ``effectiveBuiltInProfiles``, so an id the
    /// manifest publishes rules for answers with those rules — the whole
    /// mechanism behind an "Updated" chip on a built-in.
    var activeProfile: BrokerAgentProfile? {
        guard let activeProfileID else { return nil }
        return effectiveBuiltInProfiles.first { $0.id == activeProfileID }
    ...
    ///
    /// True for a built-in the manifest publishes rules for as well. Both
    /// answers already forbid the same edits, so the overlap costs nothing
    /// and the stricter one is the safe way round.
    func isRemotePreset(id: UUID) -> Bool {
        remotePresets.contains { $0.id == id }
    }

    /// Whether the manifest, not this app version, decides `id`'s rules.
    ///
    /// False for a built-in the manifest republishes unchanged: the manifest
    /// ships every built-in so that any of them CAN be corrected between
    /// releases, and badging all four "Manifest" in the common case where
    /// they match would make the badge mean nothing on the day one of them
    /// actually differs. True for every preset the manifest adds, which has
    /// no shipped rules to agree with.
    func manifestSuppliesRules(id: UUID) -> Bool {
        guard let published = remotePresets.first(where: { $0.id == id }) else { return false }
        guard let builtIn = BrokerAgentProfile.builtIn(id: id) else { return true }
        return published.rules != builtIn.rules
    }
    ...
    /// Compare published rules with the loaded baseline, not unsaved edits.
    /// Model names and efforts come from the manifest, including future models.
    var routingUpdateSemanticChanges: [BrokerRoutingChange] {
        guard activeProfileHasUpdatedRules,
              let old = activeProfileRules, let new = activeProfile?.rules else { return [] }
        return routingChanges(from: old, to: new)
    }

    /// Compatibility text for notifications and non-review consumers.
    /// The review sheet consumes ``routingUpdateSemanticChanges`` directly.
    var routingUpdateChanges: [String] {
        routingUpdateSemanticChanges.map(\.ordinaryDescription)
    }

    func routingChanges(
        from old: BrokerRuleSet,
        to new: BrokerRuleSet,
        defaultedRoles: Set<String> = []
    ) -> [BrokerRoutingChange] {
        var changes = Set(old.roles.keys)
            .union(new.roles.keys)
            .union(defaultedRoles)
            .sorted()
            .compactMap { role -> BrokerRoutingChange? in
                guard old.roles[role] != new.roles[role] || defaultedRoles.contains(role) else {
                    return nil
                }
                return .role(BrokerRoutingRoleChange(
                    role: role,
                    previous: old.roles[role] ?? [],
                    proposed: new.roles[role] ?? [],
                    proposedUsesDefaultRules: defaultedRoles.contains(role)
                ))
            }
        let thresholdChanges = BrokerRoutingThresholdChange.Field.allCases.compactMap { field in
            let previous = field.value(in: old.thresholds)
            let proposed = field.value(in: new.thresholds)
            return previous == proposed ? nil : BrokerRoutingThresholdChange(
                field: field,
                previousValue: previous,
                proposedValue: proposed
            )
        }
        if !thresholdChanges.isEmpty { changes.append(.thresholds(thresholdChanges)) }
        if old.models != new.models { changes.append(.modelCapabilities) }
        if old.callers != new.callers || old.allowForcedDegraded != new.allowForcedDegraded {
            changes.append(.callerPolicy)
        }
        if old.instanceByModel != new.instanceByModel || old.defaultInstance != new.defaultInstance
            || old.agentModelAliases != new.agentModelAliases {
    ...
    mutating func applyProfile(id: UUID) {
        guard let profile = effectiveBuiltInProfiles.first(where: { $0.id == id })
            ?? remotePresets.first(where: { $0.id == id })
            ?? profiles.first(where: { $0.id == id })
        else { return }
        let previousRules = policy.ruleSet
        let previousBaseline = activeProfileRules
        let isUpdate = activeProfileID == id && activeProfileHasUpdatedRules
        policy.apply(profile.rules)
        if isUpdate, let previousBaseline {
            routingRecovery = BrokerRoutingRecovery(
                profileID: id, profileName: profile.name,
                previousRules: previousRules, previousBaseline: previousBaseline,
                appliedRules: policy.ruleSet
            )
            // A new recovery point is a new offer, so the popover shows it
            // again even if the previous one was dismissed there.
            isRoutingRecoveryDismissedInPopover = false
        }
        activeProfileID = profile.id
        activeProfileRules = policy.ruleSet
    }

    var canUndoRoutingUpdate: Bool {
        guard let recovery = routingRecovery else { return false }
        return activeProfileID == recovery.profileID && activeProfileRules == recovery.appliedRules
    }

    /// Structural validation deliberately ignores current quotas and reachability.
    func revisionRestoreError(id: UUID, rules: BrokerRuleSet) -> String? {
        guard activeProfileID == id, activeProfile != nil else {
            return "The active profile has changed"
        }
        let rules = rules.fillingMissingRoles
        if let unknown = unknownInstanceReferences(in: rules).first {
            return "It refers to an instance this Mac doesn't have (\(unknown))"
        }
        var proposed = policy
        proposed.assignExactRules(rules)
        for role in rules.roles.keys.sorted() {
            let routes = BrokerEngine.expandingModelChoices(
                rules.roles[role] ?? [], policy: proposed, oracle: nil, now: Date(timeIntervalSince1970: 0)
            )
            let routable = routes.contains { candidate in
                switch candidate.route {
                case .auto: return false
                case .native: return proposed.agentModelAliases[candidate.model] != nil
                case .codex:
                    return proposed.usageLanes[candidate.id] != nil
                        || proposed.models[candidate.model]?.quota.map {
                            if case .chatGPT = $0 { return true }
                            return false
                        } == true
                case .t3:
                    return proposed.instances(serving: candidate.model).contains {
                        $0.id == proposed.resolvedInstance(for: candidate)
                    }
                }
            }
            if !routable { return "The \(role) role would have no model to route to" }
        }
        return nil
    }

    /// Applies only validated rules to the active profile, replacing the existing undo point.
    @discardableResult
    mutating func restoreProfileRules(id: UUID, rules: BrokerRuleSet) -> Bool {
        guard revisionRestoreError(id: id, rules: rules) == nil,
              let profile = activeProfile, let baseline = activeProfileRules else { return false }
        let rules = rules.fillingMissingRoles
        guard rules != policy.ruleSet else { return false }
        let recovery = BrokerRoutingRecovery(
            profileID: id, profileName: profile.name,
            previousRules: policy.ruleSet, previousBaseline: baseline,
            appliedRules: rules, origin: "revision"
        )
        policy.assignExactRules(rules)
        activeProfileRules = rules
        routingRecovery = recovery
        isRoutingRecoveryDismissedInPopover = false
        return true
    }

    /// Restore rules and their original edit baseline without rewinding accounts or connections.
    @discardableResult
    mutating func undoRoutingUpdate(discardingEdits: Bool = false) -> Bool {
        guard canUndoRoutingUpdate, let recovery = routingRecovery,
              discardingEdits || policy.ruleSet == recovery.appliedRules else { return false }
        if recovery.origin == "revision" {
            policy.assignExactRules(recovery.previousRules)
        } else {
            policy.apply(recovery.previousRules)
        }
        activeProfileRules = recovery.previousBaseline
        routingRecovery = nil
        isRoutingRecoveryDismissedInPopover = false
        return true
    }
    ...
    mutating func updateRemotePresets(_ presets: [BrokerAgentProfile]) {
        ...
        // themselves. A collision with a BUILT-IN id is the opposite case and
        // is allowed on purpose (it publishes that built-in's rules, with its
        // identity kept compiled); this filter guards the one identity space
        // a manifest author cannot see and must never be able to claim.

...
extension BrokerPolicy {
    ...
    static let bundledDefault: BrokerPolicy = {
        ...
        // `review` is deliberately not on `fableChain`. The reviewer completion
        // gate fires on every multi-file or security-relevant change, which
        // makes review the highest-volume judgment role by a wide margin.
        // Leading it with Opus keeps review well above the execution tier while
        // leaving the reasoning-tier budget for planning and architecture, which
        // run far less often and carry the decisions review only checks. Fable
        // stays next in line, so a capped or unavailable Opus still falls back
        // into the reasoning tier rather than down to Sonnet.

...
struct BrokerEffort: RawRepresentable, Codable, Hashable, Sendable {
    let rawValue: String
    ...
    init?(rawValue: String) {
        ...
        guard rawValue != "none", (1...24).contains(scalars.count),
    ...
    static let low = BrokerEffort(rawValue: "low")!
    static let medium = BrokerEffort(rawValue: "medium")!
    static let high = BrokerEffort(rawValue: "high")!
    static let xhigh = BrokerEffort(rawValue: "xhigh")!
    static let builtInLevels: [BrokerEffort] = [.low, .medium, .high, .xhigh]
    ...
    init(from decoder: Decoder) throws {
        ...
        guard let value = BrokerEffort(rawValue: rawValue) else {
            throw DecodingError.dataCorruptedError(
                in: container, debugDescription: "Invalid broker effort '\(rawValue)'"
            )
        }
        self = value
    }
...
enum BrokerEffortSupport: Equatable, Sendable {
    ...
    /// default).
    case supported(levels: [BrokerEffort], nilLabel: String)
    ...
    var nilLabel: String {
        switch self {
        case .supported(_, let nilLabel): return nilLabel
    ...
    var selectableLevels: [BrokerEffort] {
        if case .supported(let levels, _) = self { return levels }
        return []
    }
}
...
extension BrokerEffort {
    ...
    /// generic "Provider default" label alongside the built-in levels (D-02, D-06).
    static func support(forModel model: String) -> BrokerEffortSupport {
        ...
        if id.hasPrefix("claude-fable-5") {
            return .supported(levels: builtInLevels, nilLabel: "Adaptive (high)")
        }
        // Opus 5.5 lowered the unset default one level, to medium. This
        // branch must precede the `claude-opus-5` prefix below, which
        // would otherwise claim the id and report the wrong default.
        if id.hasPrefix("claude-opus-5-5") {
            return .supported(levels: builtInLevels, nilLabel: "Default (medium)")
        }
        if id.hasPrefix("claude-opus-5") || id.hasPrefix("claude-sonnet-5") {
            return .supported(levels: builtInLevels, nilLabel: "Default (high)")
        }
        // The local Codex catalog reports Sol's default as low, while the API
        // and Codex docs report medium. Shipped Sol candidates stay explicit.
        if id.hasPrefix("gpt-5.6-") {
            return .supported(levels: builtInLevels, nilLabel: "Default (medium)")
        }
        // The gpt-6 tier (Astra, Sol, Luna) defaults to medium.
        if id.hasPrefix("gpt-6-") {
            return .supported(levels: builtInLevels, nilLabel: "Default (medium)")
        }
        return .supported(levels: builtInLevels, nilLabel: "Provider default")
    }

    static func support(
        forModel model: String,
        in models: [String: BrokerModelSpec]
    ) -> BrokerEffortSupport {
        if let capability = models[model]?.effort {
            return capability.levels.isEmpty
                ? .unsupported
                : .supported(levels: capability.levels, nilLabel: capability.nilLabel)
        }
        return support(forModel: model)
    }
}
...
struct BrokerCandidate: Codable, Hashable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // A malformed effort fails loud, exactly like an unknown route. Valid
        // future levels decode here and are checked against model capability
        // before dispatch.
...
extension BrokerCandidate {
    ...
    func clampingEffortToModelSupport(in models: [String: BrokerModelSpec]) -> BrokerCandidate {
        guard let effort else { return self }
        let support = BrokerEffort.support(forModel: model, in: models)
        guard !support.supportsEffort || !support.selectableLevels.contains(effort) else {
            return self
        }
...
struct BrokerThresholds: Codable, Equatable, Sendable {
    ...
    /// Whether equivalent routes are ranked by how soon their quota expires
    /// rather than by remaining headroom alone.
    ///
    /// Quota is use-it-or-lose-it: an account whose weekly window resets in
    /// 14 hours loses every unspent point at that reset, while one resetting
    /// in six days keeps its own. Ranking on headroom alone spends the durable
    /// pool first and lets the perishable one evaporate, so this is on by
    /// default. The ceilings above and the pacing gates still decide whether a
    /// lane is usable at all; this only orders the ones that are.
    var preferExpiringQuota: Bool
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case preferExpiringQuota = "prefer_expiring_quota"
    }
    ...
    init(
        ...
        self.preferExpiringQuota = preferExpiringQuota
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        preferExpiringQuota = try container.decodeIfPresent(
            Bool.self, forKey: .preferExpiringQuota
        ) ?? defaults.preferExpiringQuota
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(preferExpiringQuota, forKey: .preferExpiringQuota)
    }
}
...
struct BrokerEffortCapability: Codable, Equatable, Sendable {
    let levels: [BrokerEffort]
    let nilLabel: String

    private enum CodingKeys: String, CodingKey {
        case levels
        case nilLabel = "default_label"
    }

    init(levels: [BrokerEffort], nilLabel: String = "Provider default") {
        var seen = Set<BrokerEffort>()
        self.levels = Array(levels.filter { seen.insert($0).inserted }.prefix(8))
        let label = BrokerPresetManifest.sanitize(nilLabel, maxScalars: 40)
        self.nilLabel = label.isEmpty ? "Provider default" : label
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let decoded = try container.decode([BrokerEffort].self, forKey: .levels)
        guard decoded.count <= 8 else {
            throw DecodingError.dataCorruptedError(
                forKey: .levels, in: container, debugDescription: "A model may declare at most 8 effort levels"
            )
        }
        var seen = Set<BrokerEffort>()
        levels = decoded.filter { seen.insert($0).inserted }
        let rawLabel = try container.decodeIfPresent(String.self, forKey: .nilLabel) ?? "Provider default"
        let label = BrokerPresetManifest.sanitize(rawLabel, maxScalars: 40)
        nilLabel = label.isEmpty ? "Provider default" : label
    }

    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(levels, forKey: .levels)
        try container.encode(nilLabel, forKey: .nilLabel)
    }
}

struct BrokerModelSpec: Codable, Equatable, Sendable {
    let quota: BrokerUsageLane?
    let effort: BrokerEffortCapability?
    ...
    init(quota: BrokerUsageLane? = nil, effort: BrokerEffortCapability? = nil) {
        switch quota {
        case .claudeAccount(_, let labelContains, let isPrimary):
            self.quota = .claudeAccount(
                accountId: nil,
                labelContains: labelContains.map { BrokerPresetManifest.sanitize($0, maxScalars: 60) },
                isPrimary: isPrimary
            )
        case .chatGPT(let labelContains):
            self.quota = .chatGPT(
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let decodedQuota = try container.decodeIfPresent(BrokerUsageLane.self, forKey: .quota)
        switch decodedQuota {
        case .claudeAccount(let accountId, let labelContains, let isPrimary):
            guard accountId?.isEmpty != false else {
                throw DecodingError.dataCorruptedError(
                    forKey: .quota, in: container,
                    debugDescription: "Portable model quota cannot carry account_id"
                )
            }
            let label = labelContains.map { BrokerPresetManifest.sanitize($0, maxScalars: 60) }
            quota = .claudeAccount(accountId: nil, labelContains: label, isPrimary: isPrimary)
        case .chatGPT(let labelContains):
            let label = labelContains.map { BrokerPresetManifest.sanitize($0, maxScalars: 60) }
            quota = .chatGPT(labelContains: label)
        case nil:
            quota = nil
        }
        effort = try container.decodeIfPresent(BrokerEffortCapability.self, forKey: .effort)
    }
    ...
    static func hasValidKeys(_ models: [String: BrokerModelSpec]) -> Bool {
        models.count <= 64 && models.keys.allSatisfy { model in
            (1...128).contains(model.unicodeScalars.count)
                && BrokerPresetManifest.sanitize(model, maxScalars: 128) == model
        }
    }
}
...
struct BrokerPolicy: Codable, Equatable, Sendable {
    /// The routes this build can invoke. `auto` is a policy-only model choice;
    /// the engine expands it to one of the three executable routes before it
    /// evaluates or returns a decision. The reference's proxy route is
    ...
    enum Route: String, Codable, Sendable, CaseIterable {
        ...
        /// Only executable routes belong in route pickers and public docs.
        static let allCases: [Route] = [.native, .t3, .codex]
    }

    /// Caller assumed when a client omits the `caller` argument.
    static let defaultCaller = "claude-code"

    /// Compatibility aliases for role names removed from the shipped policy.
    static let roleAliases: [String: String] = ["architecture": "planning"]
    ...
    /// Portable model capability metadata supplied by built-ins or manifests.
    ...
    /// Resolves a compatibility alias only when no explicit chain exists.
    func resolveRole(_ role: String) -> String {
        if roles[role] != nil { return role }
        if let target = Self.roleAliases[role], roles[target] != nil { return target }
        return role
    }
    ...
    /// The quota lane that gates a candidate: its exact `usage_lanes` row, a
    /// T3 instance's bound Claude account, or a shared ChatGPT model lane.
    ///
    ...
    func usageLane(for candidate: BrokerCandidate) -> BrokerUsageLane? {
        ...
        if candidate.route == .t3 {
            ...
            if let bound = t3Instances.first(where: { $0.id == instance })?.boundAccountId,
               !bound.isEmpty {
                return .claudeAccount(accountId: bound, labelContains: nil, isPrimary: nil)
            }
            if let shared = usageLanes["t3/\(candidate.model)"], case .chatGPT = shared {
                return shared
            }
        }
        guard candidate.route == .codex || candidate.route == .t3,
              let quota = models[candidate.model]?.quota else { return nil }
        // Shared Claude headroom cannot establish which account an unbound
        // T3 instance uses. Keep that instance quota-blind until it is bound.
        if candidate.route == .t3, case .claudeAccount = quota { return nil }
        return quota
    }

    /// Converts a route-qualified chain into one policy row per model.
    /// Model order and the first row's effort survive the conversion.
    static func automaticModelChoices(in chain: [BrokerCandidate]) -> [BrokerCandidate] {
        var seen = Set<String>()
        return chain.compactMap { candidate in
            guard seen.insert(candidate.model).inserted else { return nil }
            return BrokerCandidate(route: .auto, model: candidate.model, effort: candidate.effort)
        }
    }
    ...
    func allowsForcedDegraded(role: String) -> Bool {
        allowForcedDegraded[resolveRole(role)] ?? true
    }
...
extension BrokerPolicy {
    ...
    /// Seed policy distilled from `policy/model-routing.yaml`. Roles rank
    /// models only. Discovery and live quota decide the executable route,
    /// instance and account for each pick.
    ///
    ...
    /// and `docs/2026-09-03-routing-manifest-review/2026-09-03-routing-manifest-review.md`.
    /// Those documents carry the sourcing and recorded conflicts. An unset
    ...
    static let bundledDefault: BrokerPolicy = {
        ...
        // Planning leads with Opus 5.5 because it is the stated default and
        // has the best measured cost per completed task. It supersedes
        // Opus 5 in every shipped chain; Opus 5 stays in the catalog and
        // alias map so saved policies and manual picks keep working.
        let planningChain: [BrokerCandidate] = [
            BrokerCandidate(route: .auto, model: "claude-opus-5-5", effort: .xhigh),
            BrokerCandidate(route: .auto, model: "gpt-6-astra", effort: .low),
            BrokerCandidate(route: .auto, model: "claude-fable-5-1"),
            BrokerCandidate(route: .auto, model: "claude-sonnet-5", effort: .xhigh),
        ]
        // Design and research keep Fable first because measured gains are
        // strongest there. Research uses high effort to reduce quota burn.
        let designChain: [BrokerCandidate] = [
            BrokerCandidate(route: .auto, model: "claude-fable-5-1"),
            BrokerCandidate(route: .auto, model: "gpt-6-astra", effort: .low),
            BrokerCandidate(route: .auto, model: "claude-opus-5-5", effort: .xhigh),
            BrokerCandidate(route: .auto, model: "claude-sonnet-5", effort: .xhigh),
        ]
        let researchChain: [BrokerCandidate] = [
            BrokerCandidate(route: .auto, model: "claude-fable-5-1"),
            BrokerCandidate(route: .auto, model: "claude-opus-5-5", effort: .high),
            BrokerCandidate(route: .auto, model: "gpt-6-sol", effort: .medium),
            BrokerCandidate(route: .auto, model: "claude-sonnet-5", effort: .high),
        ]
        // Review stays on Opus 5.5 because it is the highest-volume judgment role
        // and Anthropic's named review model. High holds review accuracy while
        // lowering quota burn; Fable stays second before Sonnet.
        let reviewChain: [BrokerCandidate] = [
            BrokerCandidate(route: .auto, model: "claude-opus-5-5", effort: .high),
            BrokerCandidate(route: .auto, model: "gpt-6-astra", effort: .low),
            BrokerCandidate(route: .auto, model: "claude-fable-5-1"),
            BrokerCandidate(route: .auto, model: "claude-sonnet-5", effort: .high),
        ...
        // Explore is the read-only scanning tier. Haiku leads, while Sonnet
        // uses low effort for high-volume subagent work. Luna is the fast,
        // cheap GPT tier and suits read-only scanning.
        roles["explore"] = [
            BrokerCandidate(route: .auto, model: "claude-haiku-4-5-20251001"),
            BrokerCandidate(route: .auto, model: "gpt-6-luna", effort: .medium),
            BrokerCandidate(route: .auto, model: "claude-sonnet-5", effort: .low),
        ]
        // Verification stays mid tier because failures in test-running steps
        // can invalidate the whole trajectory.
        roles["verification"] = [
            BrokerCandidate(route: .auto, model: "claude-sonnet-5", effort: .high),
            BrokerCandidate(route: .auto, model: "gpt-6-sol", effort: .medium),
            BrokerCandidate(route: .auto, model: "claude-haiku-4-5-20251001"),
        ...
        // Heavy work raises effort because the hardest coding guidance and
        // unattended completion rates improve with it. GPT-6 Sol runs at high,
        // one step below the Claude candidates.
        roles["heavy"] = [
            BrokerCandidate(route: .auto, model: "gpt-6-astra", effort: .medium),
            BrokerCandidate(route: .auto, model: "claude-opus-5-5", effort: .xhigh),
            BrokerCandidate(route: .auto, model: "claude-sonnet-5", effort: .xhigh),
            BrokerCandidate(route: .auto, model: "gpt-6-sol", effort: .high),
        ]
        // Standard work keeps Sonnet first, then Sol at low effort, the
        // cheapest Sol setting, for short, well-scoped work.
        roles["standard"] = [
            BrokerCandidate(route: .auto, model: "claude-sonnet-5", effort: .high),
            BrokerCandidate(route: .auto, model: "gpt-6-sol", effort: .low),
        ...
        return BrokerPolicy(
            ...
            callers: [
                ...
                "codex": BrokerCallerPolicy(routes: [.codex, .t3]),
            ],
            t3: .default,
            t3Instances: [
            ...
            usageLanes: [
                "t3/claude-fable-5-1": .claudeAccount(
                    accountId: nil, labelContains: nil, isPrimary: true
                ),
            ],
            models: [
                "gpt-5.6-sol": BrokerModelSpec(
                    quota: .chatGPT(labelContains: "codex weekly"),
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
                ),
                "gpt-5.6-terra": BrokerModelSpec(
                    quota: .chatGPT(labelContains: "codex weekly"),
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
                ),
                "gpt-5.6-luna": BrokerModelSpec(
                    quota: .chatGPT(labelContains: "codex weekly"),
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
                ),
                "gpt-6-astra": BrokerModelSpec(
                    quota: .chatGPT(labelContains: "codex weekly"),
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
                ),
                "gpt-6-sol": BrokerModelSpec(
                    quota: .chatGPT(labelContains: "codex weekly"),
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
                ),
                "gpt-6-luna": BrokerModelSpec(
                    quota: .chatGPT(labelContains: "codex weekly"),
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
                ),
                "claude-fable-5-1": BrokerModelSpec(
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Adaptive (high)")
                ),
                "claude-fable-5": BrokerModelSpec(
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Adaptive (high)")
                ),
                "claude-opus-5-5": BrokerModelSpec(
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (medium)")
                ),
                "claude-opus-5": BrokerModelSpec(
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (high)")
                ),
                "claude-sonnet-5": BrokerModelSpec(
                    effort: BrokerEffortCapability(levels: BrokerEffort.builtInLevels, nilLabel: "Default (high)")
                ),
                "claude-haiku-4-5-20251001": BrokerModelSpec(
                    effort: BrokerEffortCapability(levels: [])
                ),
            ],
            agentModelAliases: [
